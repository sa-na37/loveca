# -*- coding: utf-8 -*-
# BUILD_TAG: discard_pending_rules_20260708a
...
_EFFECT_RULES = [
    ...
    {"id": "draw_until_hand_size_n", "pattern": r"^(?:自分の)?手札が(?P<n>\d+)枚になるまでカードを引く。$", "op": "draw_until_hand_size"},
    {"id": "draw_then_draw_if_self_moved_this_turn", "pattern": r"^カードを(?P<n>\d+)枚引く。このターン、このメンバーがエリアを移動して(?:いる|いた)場合、さらにカードを(?P<bonus_n>\d+)枚引く。$", "op": "draw_then_draw_if_self_moved_this_turn"},
    ...
    {"id": "draw2_discard2_if_entry_not_from_hand", "pattern": r"^このメンバーが手札以外からステージに登場している場合、カードを(?P<n>\d+)枚引き、手札を(?P<m>\d+)枚控え室に置く。$", "op": "draw_discard_if_entry_origin", "entry_origin": "not_hand"},
    {"id": "draw_if_other_stage_member_moved_this_turn", "pattern": r"^このターン、自分のステージにいるほかのメンバーがエリアを移動している場合、カードを(?P<n>\d+)枚引く。$", "op": "draw_if_other_stage_member_moved_this_turn"},
    {"id": "draw_then_gain_icons_if_entry_from_waiting", "pattern": r"^カードを(?P<n>\d+)枚引く。その後、控え室から登場している場合、ライブ終了時まで、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)を得る。$", "op": "draw_then_gain_icons_if_entry_origin", "entry_origin": "green"},
    ...
    {"id": "retrieve_waiting_live_score_le_1", "pattern": r"^自分の控え室からスコア(?P<score_lim>\d+)以下のライブカードを1枚手札に加える。$", "op": "retrieve_from_waiting_room", "card_kind": "LIVE", "n": 1},
    ...
    {"id": "look_top_live_total_plus_n_choose_one_rest_waiting", "pattern": r"^自分のデッキの上から、自分のライブの合計スコアに(?P<plus_n>\d+)を足した数に等しい枚数見る。その中からカードを1枚手札に加える。残りを控え室に置く。$", "op": "look_top_live_total_plus_n_choose_one_rest_waiting"},
    {"id": "look_top_named_members_optional_then_opponent_wait_cost_blade", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中から(?P<names>「[^」]+」(?:か「[^」]+」)*)のメンバーカードを1枚公開して手札に加えてもよい。残りを控え室に置く。その後、相手のステージにいる、これにより公開したカードのコスト以下で、かつ元々持つ<\(ブレード\)>の数が(?P<blade_lim>\d+)つ以下のメンバーをすべてウェイトにする。$", "op": "look_top_named_members_optional_then_opponent_wait_cost_blade"},
    ...
    {"id": "look_top_k_reorder_keep_any_rest_waiting", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中から好きな枚数を好きな順(?:番|場)でデッキの上に置き、残りを控え室に置く。$", "op": "look_top_reorder_keep_any"},
    {"id": "look_top_k_reorder_all_on_top", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。それらを好きな順番でデッキの上に置く。$", "op": "look_top_reorder_all"},
    {"id": "look_top1_optional_mill", "pattern": r"^自分のデッキの上からカードを1枚見る。そのカードを控え室に置いてもよい。$", "op": "self_top1_optional_green"},
    {"id": "look_top_stage_member_count_plus_keep_one_top_rest_waiting", "pattern": r"^自分のデッキの上から、自分のステージにいるメンバーの数に(?P<plus_n>\d+)を足した数に等しい枚数見る。その中から1枚をデッキの一番上に置き、残りを控え室に置く。$", "op": "look_top_stage_member_count_plus_keep_one_top_rest_waiting"},
    {"id": "reveal_top_by_all_stage_members_live_score_rest_waiting", "pattern": r"^自分のデッキの上から、自分と相手のステージにいるメンバー1人につき、1枚公開する。それらの中にあるライブカード1枚につきこのカードのスコアを\+(?P<score_n>\d+)する。その後、これにより公開したカードを控え室に置く。$", "op": "reveal_top_by_all_stage_members_live_score_rest_waiting"},
    {"id": "look_top_stage_group_count_keep_one_top_rest_waiting_reveal_score_if_live", "pattern": r"^自分のステージにいる『(?P<group>[^』]+)』のメンバー1人につき、自分のデッキの上からカードを1枚見る。その中から1枚をデッキの上に置き、残りを控え室に置く。自分のデッキの一番上のカードを1枚公開する。これによりライブカードを公開したとき、このカードのスコアを\+(?P<score_n>\d+)する。$", "op": "look_top_stage_group_count_keep_one_top_rest_waiting_reveal_score_if_live"},
    {"id": "reveal_until_live_or_cost_ge_member_to_hand_rest_waiting_choice", "pattern": r"^ライブカードかコスト(?P<cost_n>\d+)以上のメンバーカードのどちらか1つを選ぶ。選んだカードが公開されるまで、自分のデッキの一番上からカードを1枚ずつ公開する。そのカードを手札に加え、これにより公開されたほかのすべてのカードを控え室に置く。$", "op": "reveal_until_live_or_cost_ge_member_to_hand_rest_waiting_choice"},
    {"id": "reveal_top1_cost_le_member_hand_then_self_position_change", "pattern": r"^自分のデッキの一番上のカードを公開する。公開したカードがコスト(?P<cost_n>\d+)以下のメンバーカードの場合、公開したカードを手札に加え、このメンバーはポジションチェンジする。それ以外の場合、公開したカードを控え室に置く。$", "op": "reveal_top1_cost_le_member_hand_then_self_position_change"},
    {"id": "choose_number_reveal_top_member_cost_compare_hand_or_blade", "pattern": r"^数1つを選ぶ。自分のデッキの一番上のカードを公開する。公開したカードがメンバーカードで、かつコストが選んだ数以上の場合、公開したカードを手札に加える。選んだ数以下の場合、ライブ終了時まで、(?P<blades>(?:<\(ブレード\)>)+)を得る。$", "op": "choose_number_reveal_top_member_cost_compare_hand_or_blade"},
    {"id": "reveal_until_live_to_hand_rest_waiting", "pattern": r"^ライブカードが公開されるまで、自分のデッキの一番上のカードを公開し続ける。そのライブカードを手札に加え、これにより公開されたほかのすべてのカードを控え室に置く。$", "op": "reveal_until_match_to_hand_rest_waiting", "card_kind": "LIVE"},
    {"id": "mill_top_k_to_waiting", "pattern": r"^(?:自分の)?デッキの上からカードを(?P<k>\d+)枚控え室に置く。$", "op": "mill_top_to_green"},
    {"id": "mill_top_k_then_retrieve_waiting_group_type", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。その後、自分の控え室から『(?P<group>[^』]+)』の(?P<kind>ライブ|メンバー)カードを1枚手札に加える。$", "op": "mill_top_then_retrieve_from_waiting"},
    {"id": "mill_top_k_then_retrieve_waiting_type", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。その後、自分の控え室から(?P<kind>ライブ|メンバー)カードを1枚手札に加える。$", "op": "mill_top_then_retrieve_from_waiting"},
    {"id": "mill_top_k_then_waiting_live_to_deck_nth_optional", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。その後、自分の控え室からライブカード1枚を自分のデッキの一番上から(?P<deck_n>\d+)枚目に置いてもよい。$", "op": "mill_top_then_waiting_live_to_deck_nth_optional"},
    {"id": "mill_top_k_then_retrieve_if_distinct_group_live_names_at_least", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。その後、自分の控え室にあるカード名の異なる『(?P<group>[^』]+)』のライブカードが(?P<count_n>\d+)枚以上ある場合、自分の控え室から『(?P=group)』のライブカードを1枚手札に加える。$", "op": "mill_top_then_retrieve_if_distinct_group_live_names_at_least"},
    {"id": "look_top_k_optional_member_gain_icons_if_group_picked", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中からメンバーカードを1枚公開して手札に加えてもよい。残りを控え室に置く。これにより『(?P<group>[^』]+)』のカードを手札に加えた場合、ライブ終了時まで、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)を得る。$", "op": "look_top_optional_member_gain_icons_if_group_picked"},
    {"id": "choose_heart_color_reveal_top5_all_match_group_pick_gain_blades", "pattern": r"^好きなハートの色を1つ指定する。その後、自分のデッキの上からカードを(?P<k>\d+)枚公開する。公開されたカードの中に指定した色のハートを持つメンバーカードと必要ハートに指定した色を含むライブカードが合計(?P<need_n>\d+)枚含まれる場合、その中から『(?P<group>[^』]+)』のカードを1枚手札に加え、ライブ終了時まで、(?P<blades>(?:<\(ブレード\)>)+)を得る。公開した残りのカードを控え室に置く。$", "op": "choose_heart_color_reveal_topk_all_match_group_pick_gain_blades"},
    {"id": "stage_named_exists_reveal_topk_named_pick_gain_picked_hearts", "pattern": r"^自分のステージに「(?P<stage_name>[^」]+)」がいる場合、自分のデッキの上からカードを(?P<k>\d+)枚公開する。自分はそれらの中から「(?P<pick_name>[^」]+)」のカードを1枚選ぶ。ライブ終了時まで、自分のステージにいる「(?P<target_name>[^」]+)」1人は、これにより選んだカードが持つ色のハートを1つずつ得る。公開したカードをすべて控え室に置く。$", "op": "stage_named_exists_reveal_topk_named_pick_gain_picked_hearts"},
    {"id": "optional_repeat_mill_top1_gain_blade_wait_if_live", "pattern": r"^自分のデッキの一番上のカードを控え室に置いてもよい。そうした場合、ライブ終了時まで、(?P<blades><\(ブレード\)>)を得る。これにより控え室に置いたカードがライブカードの場合、このメンバーをウェイトにする。自分はこの手順をさらに(?P<extra_n>\d+)回まで繰り返してもよい。$", "op": "optional_repeat_mill_top1_gain_blade_wait_if_live"},
    {"id": "stage_group_optional_mill_top_k", "pattern": r"^自分のステージに『(?P<group>[^』]+)』のメンバーがいる場合、自分のデッキの上からカードを(?P<k>\d+)枚控え室に置いてもよい。$", "op": "stage_group_optional_mill_top_k"},
    {"id": "mill_top_k_draw_if_contains_live", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。それらの中にライブカードがある場合、カードを(?P<draw_n>\d+)枚引く。$", "op": "mill_top_conditional_followup", "cond": "contains_live", "then": "draw"},
    {"id": "mill_top_k_draw_if_all_members", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。それらがすべてメンバーカードの場合、カードを(?P<draw_n>\d+)枚引く。$", "op": "mill_top_conditional_followup", "cond": "all_members", "then": "draw"},
    {"id": "mill_top_k_gain_icons_if_contains_live", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。それらの中にライブカードがある場合、ライブ終了時まで、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)(?:を(?P<icons2>(?:<(?:\([^)]+\)|[^<>]+)>)+))?を得る。$", "op": "mill_top_conditional_followup", "cond": "contains_live", "then": "gain_icons"},
    {"id": "mill_top_k_gain_icons_if_all_members", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。それらがすべてメンバーカードの場合、ライブ終了時まで、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)を得る。$", "op": "mill_top_conditional_followup", "cond": "all_members", "then": "gain_icons"},
    {"id": "mill_top_k_gain_icons_if_all_member_heart_color", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。それらがすべて(?P<member_heart_icon><(?:\([^)]+\)|[^<>]+)>)を持つメンバーカードの場合、ライブ終了時まで、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)を得る。$", "op": "mill_top_conditional_followup", "cond": "all_member_heart_color", "then": "gain_icons"},
    {"id": "mill_top_k_gain_icons_if_all_group_cards", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚控え室に置く。それらがすべて『(?P<group>[^』]+)』のカードの場合、ライブ終了時まで、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)を得る。$", "op": "mill_top_conditional_followup", "cond": "all_group_cards", "then": "gain_icons"},
    {"id": "topdeck_green_any_upto1", "pattern": r"^自分の控え室からカードを1枚までデッキの一番上に置く。$", "op": "topdeck_from_green", "card_kind": "ANY", "allow_less": True},
    {"id": "topdeck_green_any_upto1_waiting_card", "pattern": r"^自分の控え室にあるカード1枚をデッキの一番上に置いてもよい。$", "op": "topdeck_from_green", "card_kind": "ANY", "allow_less": True},
    ...
    {"id": "live_storage_count_choose_group_live_no_live_start_topdeck_gain_icons", "pattern": r"^自分のライブカード置き場にカードが(?P<count_n>\d+)枚以上ある場合、その中から<ライブ開始時>能力を持たない『(?P<group>[^』]+)』のライブカードを1枚選び、デッキの一番上に置いてもよい。そうした場合、ライブ終了時まで、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>と?)+)を得る。$", "op": "live_storage_count_choose_group_live_no_live_start_topdeck_gain_icons"},
    ...
    {"id": "draw_then_hand_top", "pattern": r"^カードを(?P<draw_n>\d+)枚引き、自分の手札を(?P<top_n>\d+)枚デッキの一番上に置く。$", "op": "draw_then_hand_to_deck_top"},
    ...
    {"id": "energy_put_active_n", "pattern": r"^自分のエネルギーデッキから、エネルギーカードを(?P<n>\d+)枚アクティブ状態で置く。$", "op": "energy_put_active"},
    ...
    {"id": "energy_zone_to_under_self_optional", "pattern": r"^自分のエネルギー置き場にあるエネルギー(?P<n>\d+)枚をこのメンバーの下に置いてもよい。$", "op": "energy_zone_to_under_self_optional"},
    {"id": "energy_zone_to_under_self_optional_then_draw", "pattern": r"^自分のエネルギー置き場にあるエネルギー(?P<n>\d+)枚をこのメンバーの下に置いてもよい。そうした場合、カードを(?P<draw_n>\d+)枚引く。$", "op": "energy_zone_to_under_self_optional"},
    {"id": "energy_zone_to_under_self_optional_then_draw_stage_blades", "pattern": r"^自分のエネルギー置き場にあるエネルギー(?P<n>\d+)枚をこのメンバーの下に置いてもよい。そうした場合、カードを(?P<draw_n>\d+)枚引き、ライブ終了時まで、自分のステージにいるメンバーは(?P<blades>(?:<\(ブレード\)>)+)を得る。$", "op": "energy_zone_to_under_self_optional"},
    ...
    {"id": "gain_blade_if_self_moved_this_turn", "pattern": r"^このターン、このメンバーがエリアを移動して(?:いる|いた)場合、ライブ終了時まで、(?P<blades>(?:<\(ブレード\)>)+)を得る。$", "op": "gain_blade_if_self_moved_this_turn"},
    {"id": "moved_stage_members_gain_icons_until_end_live", "pattern": r"^ライブ終了時まで、自分のステージにいる、このターン中にエリアを移動したメンバーは(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)を得る。$", "op": "moved_stage_members_gain_icons_until_end_live"},
    {"id": "moved_stage_group_members_gain_icons_until_end_live", "pattern": r"^ライブ終了時まで、自分のステージにいる、このターン中にエリアを移動したすべての『(?P<group>[^』]+)』メンバーは、(?P<icons>(?:<(?:\([^)]+\)|[^<>]+)>)+)を得る。$", "op": "moved_stage_members_gain_icons_until_end_live"},
    {"id": "choose_heart_for_moved_stage_members_until_end_live", "pattern": r"^(?P<choices>(?:か?<(?:\([^)]+\)|[^<>]+)>)+)のうち、1つを選ぶ。ライブ終了時まで、自分のステージにいる、このターン中にエリアを移動しているすべてのメンバーは、選んだハートを1つ得る。$", "op": "choose_heart_for_moved_stage_members_until_end_live"},
    {"id": "choose_heart_for_entered_stage_members_except_group_until_end_live", "pattern": r"^(?P<choices>(?:か?<(?:\([^)]+\)|[^<>]+)>)+)のうち、1つを選ぶ。ライブ終了時まで、自分のステージにいるこのターンに登場したメンバーのうち、『(?P<exclude_group>[^』]+)』以外のすべてのメンバーは選んだハートを1つ得る。$", "op": "choose_heart_for_entered_stage_members_except_group_until_end_live"},
    {"id": "entered_or_moved_stage_group_members_reduce_required_any_per_member", "pattern": r"^自分のステージにいる、このターン中に登場、またはエリアを移動した『(?P<group>[^』]+)』のメンバー1人につき、このカードを成功させるための必要ハートを(?P<anys>(?:<\(任意\)>)+)減らす。$", "op": "entered_or_moved_stage_group_members_reduce_required_any_per_member"},
    {"id": "live_success_score_if_self_moved_by_group_effect", "pattern": r"^このターン、自分の『(?P<group>[^』]+)』のカードの効果によってこのメンバーがエリアを移動していた場合、ライブの合計スコアを\+(?P<n>\d+)する。$", "op": "live_success_score_if_self_moved_by_group_effect"},
    ...
    {"id": "position_change_self", "pattern": r"^このメンバーをポジションチェンジする。$", "op": "position_change_self"},
    ...
    {"id": "optional_discard_one_from_hand_then_effect_direct", "pattern": r"^手札を1枚控え室に置いてもよい。(?P<after>.+)$", "op": "optional_discard_one_from_hand_then_effect", "prefix_text": "手札を1枚控え室に置いてもよい。", "condition_text": "手札を1枚控え室に置いた場合", "timing": "任意コスト"},
    {"id": "discarded_group_top4_choose2_else_retrieve_live", "pattern": r"^これにより控え室に置いたカードが『(?P<group>[^』]+)』のカードの場合、自分のデッキの上からカードを(?P<k>\d+)枚見る。その中からカードを(?P<n>\d+)枚手札に加える。残りを控え室に置く。『(?P=group)』のカード以外の場合、自分の控え室からライブカードを1枚手札に加える。$", "op": "discarded_group_topk_choose_n_else_retrieve_live"},
    ...
    # Area-targeted group member icon gain. This keeps self-wait/energy/hand-cost
    ...
    {"id": "look_top_k_optional_cost_ge_group_member", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中からコスト(?P<cost_min>\d+)以上の『(?P<group>[^』]+)』のメンバーカードを1枚公開して手札に加えてもよい。残りを控え室に置く。$", "op": "look_top_choose_filtered", "optional": True, "card_kind": "MEMBER"},
    ...
    {"id": "look_top_k_optional_member_heart_any_color_upto_n", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中から(?P<member_heart_icons>(?:<(?:\([^)]+\)|[^<>]+)>か?)+)を持つメンバーカードを(?P<pick_count>\d+)枚まで公開して手札に加えてもよい。残りを控え室に置く。$", "op": "look_top_choose_filtered", "optional": True, "card_kind": "MEMBER", "min_pick_count": 0},
    {"id": "look_top_k_optional_distinct_group_upto_n", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中から各グループ名につき1枚ずつ公開し、(?P<pick_count>\d+)枚まで手札に加えてもよい。残りを控え室に置く。$", "op": "look_top_choose_filtered", "optional": True, "filter_mode": "distinct_group", "min_pick_count": 0},
    {"id": "look_top_k_optional_cost_le_group_member_stage_or_hand", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中からコスト(?P<cost_max>\d+)以下の『(?P<group>[^』]+)』のメンバーカードを1枚まで公開する。そのカードを自分のステージのメンバーのいないエリアに登場させるか、手札に加える。残りを控え室に置く。$", "op": "look_top_choose_filtered", "optional": True, "card_kind": "MEMBER", "after_pick": "stage_or_hand_empty_area"},
    {"id": "look_top_k_optional_member_heart_all_colors", "pattern": r"^自分のデッキの上からカードを(?P<k>\d+)枚見る。その中から(?P<member_heart_icons>(?:<(?:\([^)]+\)|[^<>]+)>と?)+)をすべて持つメンバーカードを1枚公開して手札に加えてもよい。残りを控え室に置く。$", "op": "look_top_choose_filtered", "optional": True, "card_kind": "MEMBER", "filter_mode": "member_heart_all_colors"},
...
def _ordered_heart_counts(counts: Dict[str, int]) -> Dict[str, int]:
    ...
    UI overlays use JSON/dict insertion order, so keep required-heart display
    aligned with normal six-color LIVE cards: 桃→赤→黄→緑→青→紫→任意.
    """
    src = {str(k).lower(): int(v or 0) for k, v in dict(counts or {}).items()}
    out: Dict[str, int] = {}
    for k in _HEART_DISPLAY_ORDER:
        v = int(src.get(k, 0) or 0)
        if v > 0:
            out[k] = v
    for k in sorted([k for k in src.keys() if k not in _HEART_DISPLAY_ORDER]):
        v = int(src.get(k, 0) or 0)
        if v > 0:
            out[k] = v
    return out
...
def _match_effect_template(effect_text: str):
    ...
    m_baton_lower_enter = re.match(r"^このメンバーよりコストが低い『(?P<group>[^』]+)』のメンバーからバトンタッチして登場した場合、(?P<inner>.+)$", s)
    if m_baton_lower_enter and _match_effect_template(str(m_baton_lower_enter.group('inner') or '').strip()):
        return ({'id': 'enter_baton_from_lower_cost_group_member_wrapper', 'op': 'enter_baton_from_lower_cost_group_member_wrapper'}, {})
    m_baton_lower_any_enter = re.match(r"^このメンバーよりコストが低いメンバーからバトンタッチして登場した場合、(?P<inner>.+)$", s)
    if m_baton_lower_any_enter and _match_effect_template(str(m_baton_lower_any_enter.group('inner') or '').strip()):
        return ({'id': 'enter_baton_from_lower_cost_member_wrapper', 'op': 'enter_baton_from_lower_cost_member_wrapper'}, {})
    ...
    if _is_named_hand_cost_result_effect(s):
        return ({'id': 'named_hand_cost_result_effect', 'op': 'named_hand_cost_result_effect'}, {})
    ...
    s_stage_cond = _normalize_icon_token_text(s).replace('\n','')
    m_stage_count = re.match(r'^自分のステージにメンバーが\d+人以上いる場合、(?P<inner>.+)$', s_stage_cond)
    if m_stage_count and _match_effect_template(str(m_stage_count.group('inner') or '').strip()):
        return ({'id': 'stage_member_count_gte_wrapper', 'op': 'stage_member_count_gte_wrapper'}, {})
    m_stage_group = re.match(r'^自分のステージに『[^』]+』のメンバーがいる場合、(?P<inner>.+)$', s_stage_cond)
    if m_stage_group and _match_effect_template(str(m_stage_group.group('inner') or '').strip()):
        return ({'id': 'stage_has_group_or_unit_member_wrapper', 'op': 'stage_has_group_or_unit_member_wrapper'}, {})
    m_stage_group_entered = re.match(r'^このターン、自分のステージに『[^』]+』のメンバーが登場している場合、(?P<inner>.+)$', s_stage_cond)
    if m_stage_group_entered and _match_effect_template(str(m_stage_group_entered.group('inner') or '').strip()):
        return ({'id': 'stage_group_entered_this_turn_wrapper', 'op': 'stage_group_entered_this_turn_wrapper'}, {})
    m_stage_enter_count = re.match(r'^このターン、自分のステージにメンバーが\d+回以上登場している場合、(?P<inner>.+)$', s_stage_cond)
    if m_stage_enter_count and _match_effect_template(str(m_stage_enter_count.group('inner') or '').strip()):
        return ({'id': 'stage_enter_count_gte_this_turn_wrapper', 'op': 'stage_enter_count_gte_this_turn_wrapper'}, {})
    m_stage_enter_exact_event = re.match(r'^このターン、自分のステージにメンバーが\d+回登場したとき、(?P<inner>.+)$', s_stage_cond)
    if m_stage_enter_exact_event and _match_effect_template(str(m_stage_enter_exact_event.group('inner') or '').strip()):
        return ({'id': 'stage_enter_count_exact_event_wrapper', 'op': 'stage_enter_count_exact_event_wrapper'}, {})
    m_self_wait_then = re.match(r'^このメンバーをウェイトにする。その後、(?P<inner>.+)$', s_stage_cond)
    if m_self_wait_then and _match_effect_template(str(m_self_wait_then.group('inner') or '').strip()):
        return ({'id': 'self_wait_then_effect_wrapper', 'op': 'self_wait_then_effect_wrapper'}, {})
    m_stage_moved_event = re.match(r'^このメンバーがエリアを移動(?:したとき|するたび)、(?P<inner>.+)$', s_stage_cond)
    if m_stage_moved_event and _match_effect_template(str(m_stage_moved_event.group('inner') or '').strip()):
        return ({'id': 'stage_movement_self_event_wrapper', 'op': 'stage_movement_self_event_wrapper'}, {})
    m_enter_or_stage_moved_event = re.match(r'^このメンバーが登場か、エリアを移動(?:したとき|するたび)、(?P<inner>.+)$', s_stage_cond)
    if m_enter_or_stage_moved_event and _match_effect_template(str(m_enter_or_stage_moved_event.group('inner') or '').strip()):
        return ({'id': 'enter_or_stage_movement_self_event_wrapper', 'op': 'enter_or_stage_movement_self_event_wrapper'}, {})
    m_main_phase_stage_moved_event = re.match(r'^自分のメインフェイズの間、このメンバーがエリアを移動したとき、(?P<inner>.+)$', s_stage_cond)
    if m_main_phase_stage_moved_event and _match_effect_template(str(m_main_phase_stage_moved_event.group('inner') or '').strip()):
        return ({'id': 'main_phase_stage_movement_self_event_wrapper', 'op': 'main_phase_stage_movement_self_event_wrapper'}, {})
    m_moved_or_energy_event = re.match(r'^自分のカードの効果によって、このメンバーがエリアを移動するか自分のエネルギー置き場にエネルギーが置かれたとき、(?P<inner>.+)$', s_stage_cond)
    if m_moved_or_energy_event and _match_effect_template(str(m_moved_or_energy_event.group('inner') or '').strip()):
        return ({'id': 'stage_movement_or_energy_placed_self_event_wrapper', 'op': 'stage_movement_or_energy_placed_self_event_wrapper'}, {})
    candidates = [s]
...
def _current_yell_reveal_count(gs: 'GameState', base_count: int) -> int:
    """Return YELL reveal count after live-until-end modifiers.

    Some live-start effects reduce the number of cards revealed by YELL for the
    current live (for example PL!SP-bp2-010). Keep the modifier in state so both
    the normal YELL step and additional YELL effects read the same value.
    """
    try:
        base = int(base_count or 0)
    except Exception:
        base = 0
    try:
        delta = int(getattr(gs, 'yell_reveal_count_delta_this_live', 0) or 0)
    except Exception:
        delta = 0
...
def _perform_additional_yell(gs: 'GameState', rng: random.Random, cards_db: Dict[str, CardInfo], n: int, reason: str = '') -> List[str]:
    ...
    base_n = max(0, int(n or 0))
    n = _current_yell_reveal_count(gs, base_n)
    if n != base_n:
        gs.log.append(f"[YELL+] reveal count modifier: {base_n} -> {n} ({reason or 'additional'})")
    if n <= 0:
        return []
    revealed: List[str] = []
    for i in range(n):
    ...
    for cn in revealed:
        ...
        if c:
            draw_n += _ci_yell_draw_icon_count(c)
    gs.log.append(f"[YELL+-DEBUG] draw_icon_by_card={_yell_draw_icon_debug_parts(cards_db, revealed)} total={draw_n}")
    gs.log.append(f"[YELL+-DEBUG] deck_before_draw n={len(getattr(gs, 'deck', []) or [])} top={list(getattr(gs, 'deck', []) or [])[:6]}")
    if draw_n > 0:
        got, drawn_cards = _draw_cards_for_yell(gs, draw_n, rng, reason='additional_yell')
    else:
        got, drawn_cards = 0, []
    ...
    _append_effect_event(gs, 'additional_yell', source_cn=str(reason or ''), detail=f'revealed {len(revealed)} cards; draw icons {draw_n}; drew {got}', revealed=revealed, drawn=drawn_cards, requested_count=base_n, effective_count=n)
    try:
...
def _cost_requires_self_to_green(cost_text: str) -> bool:
    t = str(cost_text or "").strip()
    if "このメンバー" not in t or "控え室" not in t or "置" not in t:
        return False
    return bool(
        re.search(r"このメンバー(?:を|は)?(?:ステージから)?控え室に置", t)
        or re.search(r"このメンバーをステージから控え室", t)
    )
...
def _ci_has_live_start_ability(ci: Optional[CardInfo]) -> bool:
    if not ci:
        return False
...
def _green_candidates(gs: 'GameState', cards_db: Dict[str, CardInfo], kind: str, group: str = "", score_lim: Optional[int] = None) -> List[str]:
    out = []
    for cn in list(gs.green_room):
        ...
        if group and not _ci_matches_group_or_unit(ci, group):
            continue
        if kind == 'LIVE' and score_lim is not None:
            try:
                score = int(getattr(ci, 'score', 0) or 0)
            except Exception:
                score = 0
            if score > int(score_lim):
                continue
        out.append(cn)
    return out
def _success_has_group(gs: 'GameState', cards_db: Dict[str, CardInfo], group: str) -> bool:
    ...
    for cn in list(getattr(gs, 'success_zone', []) or []):
        ...
        if group in str(getattr(ci, 'group', '') or ''):
            return True
    return False
def _enqueue_discard_from_hand(
    gs: 'GameState',
    n: int,
    label: str = "",
    ctx: Optional[Dict[str, Any]] = None,
    after_effect_template: str = '',
    after_ctx: Optional[Dict[str, Any]] = None,
    after_source_cn: str = '',
    optional: bool = False,
    ...
    after_ctx2 = dict(after_ctx or ctx or {})
    source_cn = str(after_source_cn or (ctx or {}).get('source_cn', '') or '')
    multi_text = text + (('支払わない場合はスキップ。' if text.endswith(('。', '\n')) else '。支払わない場合はスキップ。') if (optional or allow_skip) else ('。' if not text.endswith('。') else ''))
    if n > 1:
        gs.pending.append({
            'kind': 'choose_member_from_green_multi_up_to',
            'source_zone': 'hand',
            'action': 'discard_from_hand',
            'min_picks': 0 if (optional or allow_skip) else n,
            'max_picks': min(n, len(getattr(gs, 'hand', []) or [])),
            'exact_or_zero': bool(optional or allow_skip),
            'text': _auto_effect_detail_block(ctx, multi_text),
            'options': list(gs.hand),
            'after_effect_template': str(after_effect_template or ''),
            'after_ctx': after_ctx2,
            'after_source_cn': source_cn,
            'source_cn': source_cn,
            'skip_if_no_picks': bool(optional or allow_skip),
            'auto_effect_detail': auto_detail,
            'suppress_card_text': bool(auto_detail),
        })
        gs.log.append(f'[PENDING] discard_from_hand multi n={n} hand={len(gs.hand)}')
        return
    gs.pending.append({
        'kind': 'discard_from_hand',
        'remaining': n,
        'text': _auto_effect_detail_block(ctx, text),
        'options': list(gs.hand),
        'after_effect_template': str(after_effect_template or ''),
        'after_ctx': after_ctx2,
        'after_source_cn': source_cn,
...
def _auto_effect_detail_block(ctx: Optional[Dict[str, Any]], action_text: str = '') -> str:
    ...
    timing = str(ctx.get('effect_timing', '') or ctx.get('timing', '') or '').strip()
    if not timing:
        timing = '自動効果'
...
def _display_text_with_icons(text: str) -> str:
    t = str(text or '').replace('＜', '<').replace('＞', '>')
    try:
        return _normalize_icon_token_text(t)
    except Exception:
        return t

def _card_display_name(cards_db: Dict[str, CardInfo], cn: str) -> str:
    cn0 = str(cn or '').strip()
    ci = _get_card(cards_db, cn0) if cn0 else None
    if ci:
        name = str(getattr(ci, 'name', '') or getattr(ci, 'cardname', '') or '').strip()
        if name:
            return name
    return cn0

def _card_display_name_with_no(cards_db: Dict[str, CardInfo], cn: str) -> str:
    cn0 = str(cn or '').strip()
    name = _card_display_name(cards_db, cn0)
    if cn0 and name and name != cn0:
        return f'{name}（{cn0}）'
    return name or cn0

def _card_display_names(cards_db: Dict[str, CardInfo], cns: List[str]) -> str:
    vals = [_card_display_name_with_no(cards_db, cn) for cn in list(cns or [])]
    vals = [v for v in vals if v]
    return '、'.join(vals) if vals else 'なし'

def _mill_followup_condition_label(cond: str, gd: Dict[str, str]) -> str:
    cond0 = str(cond or '').strip()
    if cond0 == 'contains_live':
        return '控え室に置いたカードの中にライブカードがある'
    if cond0 == 'all_members':
        return '控え室に置いたカードがすべてメンバーカードである'
    if cond0 == 'all_member_heart_color':
        icon = _display_text_with_icons(str(gd.get('member_heart_icon', '') or '指定ハート'))
        return f'控え室に置いたカードがすべて{icon}を持つメンバーカードである'
    if cond0 == 'all_group_cards':
        group = str(gd.get('group', '') or '').strip()
        return f'控え室に置いたカードがすべて『{group}』のカードである' if group else '控え室に置いたカードがすべて指定グループのカードである'
    return cond0 or '-'

def _enqueue_choose_from_green(gs: 'GameState', cards_db: Dict[str, CardInfo], kind: str, n: int = 1, group: str = "", ctx: Optional[Dict[str, Any]] = None, allow_less: bool = False, resume: Optional[Dict[str, Any]] = None, after_effect_template: str = '', after_ctx: Optional[Dict[str, Any]] = None, score_lim: Optional[int] = None) -> None:
    n = int(n or 1)
    score_limit = None
    try:
        if score_lim is not None:
            score_limit = int(score_lim)
    except Exception:
        score_limit = None
    cands = _green_candidates(gs, cards_db, kind=kind, group=group, score_lim=score_limit)
    if not cands:
    ...
    if group:
        action_text += f'（{group}）'
    if kind == 'LIVE' and score_limit is not None:
        action_text += f'（スコア{score_limit}以下）'
    auto_detail = str((ctx or {}).get('auto_effect_detail', '') or '').strip()
    prm = {
...
def _enqueue_topdeck_from_green(gs: 'GameState', cards_db: Dict[str, CardInfo], kind: str, n: int, group: str = '', allow_less: bool = False, ctx: Optional[Dict[str, Any]] = None, effect_text: str = '') -> None:
    ...
    src = str((ctx or {}).get('source_cn', '') or '').strip()
    effect = _display_text_with_icons(str(effect_text or '').strip())
    base_text = f'控え室の{kind}を{n}枚{("まで" if allow_less else "")}デッキ上に置く：1枚目=一番上。/ skipで終了' if allow_less else f'控え室の{kind}を{n}枚デッキ上（1枚目=一番上）'
    if src or effect:
        lines = []
        if src:
            lines.append(f'【{src}】')
        if effect:
            lines.append(f'効果：{effect}')
        lines.append(f'処理：{base_text}')
        pending_text = '\n'.join(lines)
    else:
        pending_text = base_text
    gs.pending.append({
...
def _enqueue_hand_to_deck_top(gs: 'GameState', cards_db: Dict[str, CardInfo], kind: str = 'ANY', n: int = 1, group: str = '', source_cn: str = '') -> None:
    kind = str(kind or 'ANY').upper()
    n = int(n or 0)
    if n <= 0:
        return
    cands = _hand_candidates_by_kind(gs, cards_db, kind=kind, group=group)
    if len(cands) < n:
        gs.log.append(f'[ERR] hand_to_deck_top: not enough {kind} cards in hand (need {n}, have {len(cands)})')
        return
    gs.pending.append({
        'kind': 'hand_to_deck_top',
        'text': f'手札の{kind}カードを{n}枚デッキの一番上に置く',
        'options': list(cands),
        'remaining': n,
        'picked': [],
        'want_kind': kind,
    ...
    gs.log.append(f'[PENDING] hand_to_deck_top: kind={kind} n={n} cands={len(cands)}')


def _topdeck_modal_common_from_ctx(ctx: Optional[Dict[str, Any]] = None, effect_text: str = '', trigger: str = '') -> Dict[str, Any]:
    """Common payload for deck-top effect pending.

    BUILD_TAG: engine_topdeck_pending_contract_cleanup_20260629ae
    Deck-top effect helpers are legacy entry points, but any pending they create
    must still satisfy the shared effect-popup contract: source card, current
    resolving effect text, and trigger metadata are carried on the pending.
    """
    try:
        ctx2 = dict(ctx or {})
    except Exception:
        ctx2 = {}
    src = str(ctx2.get('source_cn') or ctx2.get('cn') or '').strip()
    detail = str(
        ctx2.get('detail_text')
        or ctx2.get('effect_text')
        or effect_text
        or ''
    ).strip()
    if not detail:
        auto_detail = str(ctx2.get('auto_effect_detail') or '').strip()
        if '効果：' in auto_detail:
            detail = auto_detail.split('効果：', 1)[1].strip()
        elif auto_detail:
            detail = auto_detail
    trig = str(trigger or ctx2.get('trigger') or ctx2.get('timing') or '').strip()
    if not trig:
        auto_detail = str(ctx2.get('auto_effect_detail') or '')
    ...
    if trig:
        ...
        payload['ctx'].setdefault('trigger', trig)
    return payload

def _enqueue_choose_from_topk(gs: 'GameState', k: int, rng: Optional[random.Random] = None, ctx: Optional[Dict[str, Any]] = None) -> None:
    k = int(k or 0)
    if k <= 0:
        return
    ...
    common_modal = _topdeck_modal_common_from_ctx(ctx)
    gs.pending.append({
        **common_modal,
...
def _enqueue_choose_n_from_topk(gs: 'GameState', k: int, n: int, rng: Optional[random.Random] = None, ctx: Optional[Dict[str, Any]] = None) -> None:
    k = int(k or 0)
    n = int(n or 0)
    if k <= 0 or n <= 0:
        return
    _rule_refresh_for_top_access(gs, rng, k, reason='look_top_choose_n')
    if not gs.deck:
        gs.log.append('[INFO] look_top_choose_n: deck empty')
        return
    ...
    gs.pending.append({
        **common_modal,
        'kind': 'choose_from_topk',
        'text': f'デッキ上から{len(pool)}枚を見る：その中から{take_n}枚を手札に加え、残りを控え室に置く（0/{take_n}枚選択済み）',
        'options': list(pool),
        'pool': list(pool),
        'display_cards': list(pool),
        'candidates': list(pool),
        'pick_count': take_n,
        'min_pick_count': take_n,
        'max_pick_count': take_n,
        'picked': [],
    })
    gs.log.append(f'[PENDING] choose {take_n} from top {len(pool)} (rest -> waiting room)')
def _enqueue_choose_from_topk_filtered(
    ...
    filter_group2: str = '',  # secondary group for OR filters
    ...
    """Look at top-k cards, let user pick matching filtered cards, rest to green."""
    ...
    if not gs.deck:
        ...
        return
    pool = [gs.deck.pop(0) for _ in range(min(k, len(gs.deck)))]
    def _group_keys(ci: Optional[CardInfo]) -> List[str]:
        raw = str(getattr(ci, 'group', '') or '').replace('／', '/')
    ...
    def _matches(cn: str) -> bool:
        ...
        if mode == 'distinct_group':
            return bool(_group_keys(ci))
        if mode == 'member_heart_or_live_req':
            ...
            return False
        if mode == 'member_heart_all_colors':
            return bool(
                _is_member_ci(ci)
                and member_heart_colors_any
                and all(_ci_member_heart_count(ci, col) > 0 for col in member_heart_colors_any)
            )
        if mode == 'group_no_ability_or_body':
            group1 = str(filter_group or '').strip()
            group2 = str(filter_group2 or filter_group or '').strip()
            abilities = list(getattr(ci, 'abilities', []) or [])
            has_no_ability = len(abilities) == 0
            has_always = any(
                '常時' in str(ab.get('ability_type', '') or '')
                for ab in abilities if isinstance(ab, dict)
            )
            return bool(
                (has_no_ability and group1 and _ci_matches_group_or_unit(ci, group1))
                or (has_always and group2 and _ci_matches_group_or_unit(ci, group2))
            )
        if mode == 'group_member_or_bladeheart_group_member':
            group1 = str(filter_group or '').strip()
            group2 = str(filter_group2 or filter_group or '').strip()
            if not _is_member_ci(ci):
                return False
            return bool(
                (group1 and _ci_matches_group_or_unit(ci, group1))
                or (group2 and _ci_matches_group_or_unit(ci, group2) and _ci_has_blade_heart_payload(ci))
            )
        if filter_kind:
            ...
            if filter_kind == 'MEMBER' and not _is_member_ci(ci):
                return False
        if filter_group:
            if not _ci_matches_group_or_unit(ci, str(filter_group).strip()):
                return False
        if filter_names:
    ...
    if member_heart_colors_any and filter_mode != 'member_heart_all_colors':
    ...
    if filter_mode == 'member_heart_all_colors' and member_heart_colors_any:
        label_parts.append('ハート' + '/'.join(member_heart_colors_any) + 'すべて')
    if filter_mode == 'group_no_ability_or_body':
        label_parts.append('能力なし/常時能力あり')
    if filter_mode == 'group_member_or_bladeheart_group_member':
        label_parts.append('指定グループメンバー/ブレードハート持ちメンバー')
    if filter_mode == 'distinct_group':
        label_parts.append('各グループ名1枚ずつ')
    if filter_kind:
    ...
    if not candidates:
        ...
        return
    pick_count = max(1, int(pick_count or 1))
    min_pick_count = max(0, int(min_pick_count or 0))
    if min_pick_count > pick_count:
        min_pick_count = pick_count
    max_pick_count = min(pick_count, len(candidates))
    if pick_count > 1:
        label_pick = f'{max_pick_count}枚まで' if optional or min_pick_count == 0 else f'{max_pick_count}枚'
    else:
        label_pick = '1枚'
    suffix = '（スキップ可）' if optional and pick_count <= 1 else ''
    ...
    gs.pending.append({
        **common_modal,
        'kind': 'choose_from_topk',
        'text': f'デッキ上{len(pool)}枚から{label}を{label_pick}手札へ{suffix}',
        'options': opts,
        'pool': list(pool),
        'display_cards': list(pool),
        ...
        'optional': optional,
        'pick_count': int(max_pick_count),
        'min_pick_count': int(min_pick_count),
        'max_pick_count': int(max_pick_count),
        'unique_by_group': bool(filter_mode == 'distinct_group'),
        'after_pick': str(after_pick or ''),
    })
    gs.log.append(f'[PENDING] look_top_filtered: pool={len(pool)} candidates={len(candidates)} optional={optional} picks={min_pick_count}..{max_pick_count}')
def _enqueue_look_top_3way_split(
    gs: 'GameState', k: int, rng: Optional[random.Random], ctx: Optional[Dict[str, Any]] = None,
) -> None:
...
def _enqueue_reorder_from_topk_keep_any(gs: 'GameState', k: int, rng: Optional[random.Random] = None, ctx: Optional[Dict[str, Any]] = None) -> None:
    k = int(k or 0)
    if k <= 0:
        return
...
def _enqueue_reorder_from_topk_all(gs: 'GameState', k: int, rng: Optional[random.Random] = None, ctx: Optional[Dict[str, Any]] = None) -> None:
    k = int(k or 0)
    if k <= 0:
        return
    _rule_refresh_for_top_access(gs, rng, k, reason='look_top_reorder_all')
    if not gs.deck:
        gs.log.append('[INFO] look_top_reorder_all: deck empty')
        return
    pool: List[str] = []
    for _ in range(min(k, len(gs.deck))):
        pool.append(gs.deck.pop(0))
    ...
    gs.pending.append({
        **common_modal,
        'kind': 'reorder_topk_all',
        'text': f'デッキ上から{len(pool)}枚を見る：好きな順番でデッキ上に戻す（1枚目=一番上）',
        'options': list(pool),
        'pool': list(pool),
        'kept': [],
    })
    gs.log.append(f'[PENDING] reorder top {len(pool)} all cards')
...
def _apply_effect_by_rule(gs: 'GameState', rng: random.Random, cards_db: Dict[str, CardInfo], rule: Dict[str, Any], gd: Dict[str, str], ctx: Dict[str, Any]) -> None:
    ...
    if op == 'draw_until_hand_size':
        ...
        before = len(getattr(gs, 'hand', []) or [])
        n = max(0, target - before)
        ...
        return
    if op == 'draw_then_draw_if_self_moved_this_turn':
        n = int(gd.get('n', 0) or 0)
        ...
        src = str((ctx or {}).get('source_cn', '') or '')
        slot = gs.stage.get(pos) if pos in ('L','C','R') else None
        src_cn = _canon_cardno(src or (getattr(slot, 'cardnumber', '') if slot else '') or '')
        moved_set = {_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_moved_cardnumbers_this_turn', []) or [])}
        moved_set = {x for x in moved_set if x}
        moved_any = bool(getattr(gs, 'stage_moved_this_turn', False))
        moved_ok = bool(src_cn and src_cn in moved_set) or (moved_any and not moved_set)
        bonus_got = draw(gs, bonus_n, rng) if moved_ok and bonus_n > 0 else 0
        gs.log.append(f'[AUTO] draw {n} -> drew {got}; self moved={moved_ok} -> bonus draw {bonus_n if moved_ok else 0} (drew {bonus_got})')
        return
    ...
    if op == 'draw_discard_if_entry_origin':
        ...
        origin = str((ctx or {}).get('entry_origin', 'hand') or 'hand').strip().lower()
        required = str(rule.get('entry_origin', '') or '').strip().lower()
        ok = (required in ('green', 'waiting') and origin in ('green', 'waiting')) or (required == 'not_hand' and origin != 'hand')
        if not ok:
            src = str((ctx or {}).get('source_cn', '') or '')
            gs.log.append(f'[SKIP] {src}: entry origin {origin or "hand"} does not match {required or "condition"}')
            return
        got = draw(gs, n, rng) if n > 0 else 0
        gs.log.append(f'[AUTO] entry origin {origin}: draw {n} -> drew {got}; then discard {m}')
        if m > 0:
            _enqueue_discard_from_hand(gs, m, label=f'手札を{m}枚控え室に置く', ctx=ctx)
        return
    if op == 'draw_then_gain_icons_if_entry_origin':
        n = int(gd.get('n', 0) or 0)
        icons_blob = str(gd.get('icons', '') or '')
        got = draw(gs, n, rng) if n > 0 else 0
        origin = str((ctx or {}).get('entry_origin', 'hand') or 'hand').strip().lower()
        ...
        if not ok:
            src = str((ctx or {}).get('source_cn', '') or '')
            gs.log.append(f'[AUTO] draw {n} -> drew {got}; [SKIP] {src}: entry origin {origin or "hand"} does not match {required or "condition"}')
            return
        pos = str((ctx or {}).get('pos', '') or '').upper()
        slot = gs.stage.get(pos) if pos in ('L', 'C', 'R') else None
        if not slot:
            gs.log.append(f'[WARN] draw_then_gain_icons_if_entry_origin: no source slot after drawing {got}')
            return
        b, hs = _grant_temp_icons_to_slot(slot, icons_blob, 1)
        gs.log.append(f'[AUTO] draw {n} -> drew {got}; entry origin {origin}: {pos} gain {icons_blob} until end_of_live (blades={b}, hearts={hs})')
        return
    ...
    if op == 'draw_then_discard':
        ...
        _enqueue_discard_from_hand(gs, m, label=f'手札を{m}枚控え室に置く', ctx=ctx)
        return
    if op == 'draw_stage_member_count_then_discard':
        n = int(_stage_member_count(gs, cards_db) or 0)
        discard_n = int(rule.get('discard_n', gd.get('discard_n', 1)) or 1)
        got = draw(gs, n, rng) if n > 0 else 0
        gs.log.append(f'[AUTO] stage member count {n} -> draw {n} (drew {got}); then discard {discard_n}')
        if discard_n > 0:
            _enqueue_discard_from_hand(gs, discard_n, label=f'手札を{discard_n}枚控え室に置く', ctx=ctx)
        return
    if op == 'draw_if_other_stage_member_moved_this_turn':
        n = int(gd.get('n', 0) or 0)
        src_pos = str((ctx or {}).get('pos', '') or '').upper()
        moved = _moved_stage_member_positions_this_turn(gs, cards_db, exclude_pos=src_pos)
        if not moved:
            gs.log.append('[SKIP] other stage member moved this turn -> condition not met')
            return
        got = draw(gs, n, rng) if n > 0 else 0
        gs.log.append(f'[AUTO] other moved stage members={moved} -> draw {n} (drew {got})')
        return
    if op == 'discard_from_hand':
        n = int(gd.get('n', 0) or 0)
        _enqueue_discard_from_hand(gs, n, label=f'手札を{n}枚控え室に置く', ctx=ctx)
        return
    if op == 'retrieve_from_waiting_room':
        kind = str(rule.get('card_kind', '') or '').upper() or 'ANY'
        n = int(gd.get('n', rule.get('n', 1)) or 1)
        group = str(gd.get('group', '') or '')
        score_lim = gd.get('score_lim', rule.get('score_lim', None))
    ...
    if op == 'optional_discard_one_from_hand_then_effect':
        ...
        src = str((ctx or {}).get('source_cn', '') or '')
        prefix_text = str(rule.get('prefix_text', '') or '手札を1枚控え室に置いてもよい。そうした場合、')
        condition_text = str(rule.get('condition_text', '') or '手札を1枚控え室に置いた場合')
        timing_text = str(rule.get('timing', '') or '任意コスト')
        detail = _auto_effect_detail_for_condition(ctx, after, condition_text, timing=timing_text)
    ...
    if op == 'topdeck_from_green':
        ...
        if kind not in ('LIVE','MEMBER','ANY'):
            ...
            return
        _enqueue_topdeck_from_green(gs, cards_db, kind=kind, n=n, group=group, allow_less=allow_less, ctx=ctx, effect_text=str((ctx or {}).get('effect_text', '') or ''))
        return
    if op == 'topdeck_green_live_group_upto1_then_draw_if_opponent_wait_exists':
        group = str(gd.get('group', '') or '')
        src = str((ctx or {}).get('source_cn', '') or '')
        _enqueue_topdeck_from_green(gs, cards_db, kind='LIVE', n=1, group=group, allow_less=True, ctx=ctx, effect_text=str((ctx or {}).get('effect_text', '') or ''))
        gs.pending.append({
            'kind': 'confirm_effect',
    ...
    if op == 'choose_player_deck_top_action':
        ...
        return
    if op == 'live_storage_count_choose_group_live_no_live_start_topdeck_gain_icons':
        need = int(gd.get('count_n', 0) or 0)
        group = str(gd.get('group', '') or '').strip()
        icons_blob = str(gd.get('icons', '') or '')
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        set_zone = list(getattr(gs, 'set_zone', []) or [])
        if len(set_zone) < need:
            gs.log.append(f'[SKIP] {src}: live storage count {len(set_zone)}/{need}')
            return
        cands = []
        for cn in set_zone:
            ci = _get_card(cards_db, cn)
            if not (ci and _is_live_ci(ci)):
                continue
            if group and not _ci_matches_group_or_unit(ci, group):
                continue
            if _ci_has_live_start_ability(ci):
                continue
            cands.append(cn)
        if not cands:
            gs.log.append(f'[INFO] {src}: no 『{group}』 LIVE without live-start ability in live storage')
            return
        gs.pending.append({
            'kind': 'live_storage_live_to_deck_top_gain_icons',
            'text': f'{src or "この能力"}：ライブカード置き場から<ライブ開始時>能力を持たない『{group}』ライブカードを1枚選び、デッキの一番上に置いてもよい。',
        ...
        gs.log.append(f'[PENDING] {src}: choose live storage 『{group}』 LIVE without live-start -> deck top ({len(cands)} candidates)')
        return
    ...
    if op == 'draw_then_hand_to_deck_bottom':
        ...
        return
    if op == 'draw_then_hand_to_deck_top':
        draw_n = int(gd.get('draw_n', 0) or 0)
        top_n = int(gd.get('top_n', 1) or 1)
        got = draw(gs, draw_n, rng)
        gs.log.append(f'[AUTO] draw_then_hand_to_deck_top: drew {got}; choose {top_n} from hand -> deck top')
        _enqueue_hand_to_deck_top(gs, cards_db, kind='ANY', n=top_n, source_cn=str((ctx or {}).get('source_cn', '') or ''))
        return
    ...
    if op == 'look_top_choose':
        ...
        _enqueue_choose_from_topk(gs, k, rng, ctx=ctx)
        return
    if op == 'look_top_choose_n':
        k = int(gd.get('k', 0) or 0)
        n = int(gd.get('n', 0) or 0)
        _enqueue_choose_n_from_topk(gs, k, n, rng, ctx=ctx)
        return
    if op == 'look_top_live_total_plus_n_choose_one_rest_waiting':
        plus_n = int(gd.get('plus_n', 0) or 0)
        live_total = int(getattr(gs, 'last_attempt_final_score', 0) or 0)
        k = max(0, live_total + plus_n)
        src = str((ctx or {}).get('source_cn', '') or '')
        gs.log.append(f'[AUTO] {src}: live total score {live_total}+{plus_n} -> look top {k}')
        _enqueue_choose_from_topk(gs, k, rng, ctx=ctx)
        return
    if op == 'look_top_choose_if':
        ...
        _enqueue_choose_from_topk(gs, k, rng, ctx=ctx)
        return
    if op == 'mill_top_to_green':
        k = int(gd.get('k', 0) or 0)
        if k <= 0:
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='mill_top_to_green')
        moved = []
        for _ in range(min(k, len(gs.deck))):
            moved.append(gs.deck.pop(0))
        gs.green_room.extend(moved)
        gs.log.append(f'[AUTO] mill_top_to_green: moved top {len(moved)}/{k} cards to waiting room')
        return
    if op == 'reveal_until_match_to_hand_rest_waiting':
        kind = str(rule.get('card_kind', '') or '').upper()
        src = str((ctx or {}).get('source_cn', '') or '')
        revealed: List[str] = []
        picked = ''
        # Continue across a normal rule refresh, but stop if no card is available.
        while True:
            if not gs.deck:
                _rule_refresh_main_deck(gs, rng, reason='reveal_until_match')
            if not gs.deck:
                break
            cn = gs.deck.pop(0)
            revealed.append(cn)
            ...
            ok = False
            if kind == 'LIVE':
                ok = bool(ci and _is_live_ci(ci))
            elif kind == 'MEMBER':
                ok = bool(ci and _is_member_ci(ci))
            else:
                ok = True
            if ok:
                picked = cn
        ...
        if picked:
            ...
            rest = [cn for cn in revealed if cn != picked]
            gs.green_room.extend(rest)
            gs.log.append(f'[AUTO] {src}: reveal_until {kind or "ANY"} -> hand {picked}; rest {len(rest)} -> waiting room')
            picked_label = _card_display_name_with_no(cards_db, picked)
            rest_label = _card_display_names(cards_db, rest)
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '公開結果確認',
                'text': f'{src or "この能力"}：条件に合うカードが公開されるまでデッキを公開しました。\n結果：{picked_label} を手札に加え、ほか{len(rest)}枚を控え室に置きました。\n控え室：{rest_label}',
                'display_cards': list(revealed),
                'options': ['ok'],
            })
        else:
            gs.green_room.extend(revealed)
            gs.log.append(f'[AUTO] {src}: reveal_until {kind or "ANY"} found no match; revealed {len(revealed)} -> waiting room')
            revealed_label = _card_display_names(cards_db, revealed)
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '公開結果確認',
                'text': f'{src or "この能力"}：条件に合うカードは公開されませんでした。\n結果：公開した{len(revealed)}枚を控え室に置きました。\n控え室：{revealed_label}',
                'display_cards': list(revealed),
                'options': ['ok'],
            })
        return
    if op == 'mill_top_then_retrieve_from_waiting':
        k = int(gd.get('k', 0) or 0)
        if k <= 0:
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='mill_top_then_retrieve_from_waiting')
        moved = []
        for _ in range(min(k, len(gs.deck))):
            moved.append(gs.deck.pop(0))
        gs.green_room.extend(moved)
        ...
        group = str(gd.get('group', '') or '').strip()
        gs.log.append(f'[AUTO] mill_top_then_retrieve_from_waiting: moved top {len(moved)}/{k}; retrieve kind={kind} group={group or "-"}')
        _enqueue_choose_from_green(gs, cards_db, kind=kind, n=1, group=group, ctx=ctx)
        return
    if op == 'mill_top_then_waiting_live_to_deck_nth_optional':
        k = int(gd.get('k', 0) or 0)
        deck_n = int(gd.get('deck_n', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        if k <= 0 or deck_n <= 0:
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='mill_top_then_waiting_live_to_deck_nth_optional')
        moved = []
        for _ in range(min(k, len(gs.deck))):
            moved.append(gs.deck.pop(0))
        gs.green_room.extend(moved)
        cands = _green_candidates_for_kind(gs, cards_db, kind='LIVE')
        gs.log.append(f'[AUTO] {src}: mill_top_then_waiting_live_to_deck_nth_optional: moved top {len(moved)}/{k}; deck_n={deck_n}; live candidates={len(cands)}')
        if not cands:
            gs.log.append(f'[INFO] {src}: no LIVE in waiting room for optional deck placement')
            return
        gs.pending.append({
            'kind': 'choose_live_from_green_to_deck_nth',
            'text': f'{src or "この能力"}：控え室のライブカード候補を表示しています。デッキの上から{deck_n}枚目に置くライブカードを1枚選んでください。置かない場合はスキップ。',
            'options': ['skip'] + list(cands),
            'deck_n': deck_n,
            'source_cn': src,
            'allow_skip': True,
        ...
        return
    if op == 'mill_top_then_retrieve_if_distinct_group_live_names_at_least':
        k = int(gd.get('k', 0) or 0)
        group = str(gd.get('group', '') or '').strip()
        need = int(gd.get('count_n', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        if k <= 0:
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='mill_top_then_retrieve_if_distinct_group_live_names')
        moved = []
        for _ in range(min(k, len(gs.deck))):
            moved.append(gs.deck.pop(0))
        gs.green_room.extend(moved)
        live_cands = _green_candidates_for_kind(gs, cards_db, kind='LIVE', group=group)
        distinct_names = set()
        for cn in live_cands:
            ci = _get_card(cards_db, cn)
            nm = str(getattr(ci, 'name', '') or cn)
            if nm:
                distinct_names.add(nm)
        gs.log.append(f'[AUTO] {src}: mill {len(moved)}/{k}; distinct 『{group}』 LIVE names={len(distinct_names)}/{need}')
        if len(distinct_names) >= need:
            _enqueue_choose_from_green(gs, cards_db, kind='LIVE', n=1, group=group, ctx=ctx)
        else:
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '条件未達確認',
                'text': f'{src or "この能力"}：デッキ上{len(moved)}枚を控え室に置きました。控え室のカード名の異なる『{group}』ライブカードは{len(distinct_names)}/{need}枚のため、回収しません。',
                'display_cards': list(moved),
                'options': ['ok'],
            })
        return
    if op == 'look_top_optional_member_gain_icons_if_group_picked':
        k = int(gd.get('k', 0) or 0)
        group = str(gd.get('group', '') or '').strip()
        icons = str(gd.get('icons', '') or '')
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        if k <= 0:
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='look_top_optional_member_gain_icons')
        pool = [gs.deck.pop(0) for _ in range(min(k, len(gs.deck)))]
        candidates = [cn for cn in pool if (_get_card(cards_db, cn) and _is_member_ci(_get_card(cards_db, cn)))]
        if not candidates:
            gs.green_room.extend(pool)
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '公開結果確認',
                'text': f'{src or "この能力"}：デッキ上{len(pool)}枚にメンバーカードがないため、すべて控え室に置きました。',
                'display_cards': list(pool),
                'options': ['ok'],
            })
            gs.log.append(f'[AUTO] {src}: no member in top {len(pool)} -> waiting room')
            return
        gs.pending.append({
            'kind': 'choose_from_topk',
            'text': f'{src or "この能力"}：デッキ上{len(pool)}枚からメンバーカード1枚を公開して手札に加えてもよい。『{group}』ならライブ終了時までアイコンを得ます。',
            'options': list(candidates) + ['skip'],
            'pool': list(pool),
            'display_cards': list(pool),
            'display_pool_all': list(pool),
            'candidates': list(candidates),
            ...
            'min_pick_count': 0,
            'max_pick_count': 1,
            'after_pick': 'gain_icons_if_picked_group',
            'after_group': group,
            'after_icons': icons,
            'source_cn': src,
            'pos': pos,
        })
        gs.log.append(f'[PENDING] {src}: optional top {len(pool)} member pick; gain icons if group={group}')
        return
    if op == 'look_top_named_members_optional_then_opponent_wait_cost_blade':
        k = int(gd.get('k', 0) or 0)
        names = re.findall(r'「([^」]+)」', str(gd.get('names', '') or ''))
        blade_lim = int(gd.get('blade_lim', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        if k <= 0:
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='look_top_named_members_opponent_wait')
        pool = [gs.deck.pop(0) for _ in range(min(k, len(gs.deck)))]
        def _named_member(cn: str) -> bool:
            ci = _get_card(cards_db, cn)
            nm = str(getattr(ci, 'name', '') or getattr(ci, 'cardname', '') or '') if ci else ''
            return bool(ci and _is_member_ci(ci) and any(n in nm for n in names))
        candidates = [cn for cn in pool if _named_member(cn)]
        if not candidates:
            gs.green_room.extend(pool)
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '公開結果確認',
                'text': f'{src or "この能力"}：対象名のメンバーカードがないため、公開した{len(pool)}枚を控え室に置きました。',
                'display_cards': list(pool),
                'options': ['ok'],
            })
            gs.log.append(f'[AUTO] {src}: named member not found in top {len(pool)} -> waiting room')
            return
        gs.pending.append({
            'kind': 'choose_from_topk',
            'text': f'{src or "この能力"}：対象名のメンバーカード1枚を手札に加えてもよい。選んだ場合、そのコスト以下かつ元々のブレード{blade_lim}以下の相手メンバーをすべてウェイトにします。',
            'options': list(candidates) + ['skip'],
            'pool': list(pool),
            'display_cards': list(pool),
            'display_pool_all': list(pool),
            ...
            'min_pick_count': 0,
            'max_pick_count': 1,
            'after_pick': 'opponent_wait_cost_blade_by_picked',
            'blade_lim': blade_lim,
            'source_cn': src,
        })
        gs.log.append(f'[PENDING] {src}: optional named member pick then opponent wait by picked cost')
        return
    if op == 'choose_heart_color_reveal_topk_all_match_group_pick_gain_blades':
        k = int(gd.get('k', 0) or 0)
        need_n = int(gd.get('need_n', 0) or 0)
        group = str(gd.get('group', '') or '').strip()
        blades = str(gd.get('blades', '') or '')
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        gs.pending.append({
            'kind': 'choose_heart_color_reveal_topk_all_match_group_pick_gain_blades',
            'text': f'{src or "この能力"}：指定するハート色を選んでください。',
            'options': ['pink', 'red', 'yellow', 'green', 'blue', 'purple'],
            'k': k,
            'need_n': need_n,
            'group': group,
            'blades': blades,
        ...
        gs.log.append(f'[PENDING] {src}: choose heart color then reveal top {k}')
        return
    if op == 'stage_named_exists_reveal_topk_named_pick_gain_picked_hearts':
        stage_name = str(gd.get('stage_name', '') or '')
        pick_name = str(gd.get('pick_name', '') or '')
        target_name = str(gd.get('target_name', '') or '')
        k = int(gd.get('k', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        stage_positions = []
        for pp, slot in (getattr(gs, 'stage', {}) or {}).items():
            ci = _get_card(cards_db, getattr(slot, 'cardnumber', '') or '') if slot else None
            nm = str(getattr(ci, 'name', '') or getattr(ci, 'cardname', '') or '') if ci else ''
            if stage_name and stage_name in nm:
                stage_positions.append(pp)
        if not stage_positions:
            gs.log.append(f'[SKIP] {src}: no stage member named {stage_name}')
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='stage_named_reveal_topk_gain_picked_hearts')
        pool = [gs.deck.pop(0) for _ in range(min(k, len(gs.deck)))]
        candidates = []
        for cn in pool:
            ci = _get_card(cards_db, cn)
            nm = str(getattr(ci, 'name', '') or getattr(ci, 'cardname', '') or '') if ci else ''
            if pick_name and pick_name in nm:
        ...
        if not candidates:
            ...
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '公開結果確認',
                'text': f'{src or "この能力"}：公開した{len(pool)}枚に「{pick_name}」のカードがないため、すべて控え室に置きました。',
                'display_cards': list(pool),
                'options': ['ok'],
            })
            gs.log.append(f'[AUTO] {src}: no {pick_name} in top {len(pool)} -> waiting room')
            return
        gs.pending.append({
            'kind': 'choose_revealed_for_heart_colors_to_stage_named',
            'text': f'{src or "この能力"}：公開されたカードのうち、「{pick_name}」のカード1枚を選んでください。選んだカードが持つ色のハートを、次の画面で選ぶステージ上の「{target_name}」に付与します。',
        ...
        gs.log.append(f'[PENDING] {src}: choose revealed {pick_name} for heart colors')
        return
    if op == 'optional_repeat_mill_top1_gain_blade_wait_if_live':
        extra_n = int(gd.get('extra_n', 0) or 0)
        blades = str(gd.get('blades', '') or '')
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        gs.pending.append({
            'kind': 'confirm_repeat_mill_top1_gain_blade_wait_if_live',
            'text': f'{src or "この能力"}：デッキ上1枚を控え室に置きますか？（残り{extra_n + 1}回）',
            'options': ['使う', 'スキップ'],
            'remaining': extra_n + 1,
            'blades': blades,
            'source_cn': src,
        ...
        gs.log.append(f'[PENDING] {src}: optional repeat mill top1 up to {extra_n + 1}')
        return
    if op == 'discarded_group_topk_choose_n_else_retrieve_live':
        group = str(gd.get('group', '') or '').strip()
        k = int(gd.get('k', 0) or 0)
        n = int(gd.get('n', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        discarded = str((ctx or {}).get('discarded_cn', '') or '')
        if not discarded:
            discarded_list = list((ctx or {}).get('discarded_cns', []) or [])
            discarded = str(discarded_list[-1] if discarded_list else '')
        ci_disc = _get_card(cards_db, discarded) if discarded else None
        if ci_disc and _ci_matches_group_or_unit(ci_disc, group):
            gs.log.append(f'[AUTO] {src}: discarded {discarded} matches 『{group}』 -> look top {k}, choose {n}')
            _enqueue_choose_n_from_topk(gs, k, n, rng, ctx=ctx)
        else:
            gs.log.append(f'[AUTO] {src}: discarded {discarded or "-"} does not match 『{group}』 -> retrieve LIVE from waiting room')
            _enqueue_choose_from_green(gs, cards_db, kind='LIVE', n=1, ctx=ctx, allow_less=False)
        return
    if op == 'stage_group_optional_mill_top_k':
        group = str(gd.get('group', '') or '').strip()
        k = int(gd.get('k', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        if k <= 0:
            return
        if not _stage_has_group_or_unit_member(gs, cards_db, group):
            gs.log.append(f'[SKIP] {src}: no stage 『{group}』 member -> optional mill skipped')
            return
        gs.pending.append({
            'kind': 'confirm_mill_top_to_green',
            'text': f'【{src or "この能力"}】自分のステージに『{group}』のメンバーがいます。デッキ上からカードを{k}枚控え室に置きますか？',
            'options': ['使う', 'スキップ'],
            'k': k,
            'source_cn': src,
        })
        gs.log.append(f'[PENDING] {src}: optional mill top {k} cards (stage group={group})')
        return
    if op == 'mill_top_conditional_followup':
        k = int(gd.get('k', 0) or 0)
        if k <= 0:
            return
        _rule_refresh_for_top_access(gs, rng, k, reason='mill_top_conditional_followup')
        moved = []
        for _ in range(min(k, len(gs.deck))):
            moved.append(gs.deck.pop(0))
        gs.green_room.extend(moved)
        ...
        moved_infos = [_get_card(cards_db, cn) for cn in moved]
        nonempty = bool(moved)
        ok = False
        if cond == 'contains_live':
            ok = any(_is_live_ci(ci) for ci in moved_infos if ci)
        elif cond == 'all_members':
            ok = nonempty and all(bool(ci and _is_member_ci(ci)) for ci in moved_infos)
        elif cond == 'all_member_heart_color':
            color = _heart_icon_to_color(str(gd.get('member_heart_icon', '') or ''))
            ok = bool(color) and nonempty and all(bool(ci and _is_member_ci(ci) and _ci_member_heart_count(ci, color) > 0) for ci in moved_infos)
        elif cond == 'all_group_cards':
            group = str(gd.get('group', '') or '').strip()
            ok = bool(group) and nonempty and all(bool(ci and _ci_matches_group_or_unit(ci, group)) for ci in moved_infos)
        else:
            gs.log.append(f'[WARN] mill_top_conditional_followup: unsupported cond={cond}')
        gs.log.append(f'[AUTO] mill_top_conditional_followup: moved top {len(moved)}/{k}; cond={cond} -> {ok}')
        src = str((ctx or {}).get('source_cn', '') or (ctx or {}).get('pos', '') or 'この能力')
        result_lines = [
            f'{src}：デッキ上から{len(moved)}/{k}枚を控え室に置きました。',
            f'条件：{_display_text_with_icons(_mill_followup_condition_label(cond, gd))} → {"達成" if ok else "未達"}',
        ]
        if not ok:
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
        ...
        if then == 'draw':
            draw_n = int(gd.get('draw_n', 0) or 0)
            got = draw(gs, draw_n, rng)
            gs.log.append(f'[AUTO] mill_top_conditional_followup: draw {draw_n} -> drew {got}')
            result_lines.append(f'追加処理：カードを{draw_n}枚引く → {got}枚引きました。')
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '自動効果確認',
                'text': '\n'.join(result_lines),
            ...
            return
        if then == 'gain_icons':
            icons_blob = str(gd.get('icons', '') or '') + str(gd.get('icons2', '') or '')
            pos = str((ctx or {}).get('pos', '') or '').upper()
            ok_gain = _grant_stage_member_temp_icons(gs, cards_db, pos, icons_blob, source_cn=src)
            if not ok_gain:
                gs.log.append(f'[WARN] mill_top_conditional_followup: gain icons skipped; source stage position missing ({src})')
            result_lines.append(f'追加処理：{_display_text_with_icons(icons_blob or "アイコン")}を得る → {"適用" if ok_gain else "未適用"}')
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '自動効果確認',
                'text': '\n'.join(result_lines),
            ...
            return
        gs.log.append(f'[WARN] mill_top_conditional_followup: unsupported then={then}')
        return
    if op == 'look_top_choose_filtered':
        ...
        group2 = str(gd.get('group2', '') or rule.get('group2', '') or '')
        ...
        after_pick = str(rule.get('after_pick', '') or '')
        pick_count = int(gd.get('pick_count', 1) or 1)
        min_pick_count = int(rule.get('min_pick_count', 1 if pick_count <= 1 else pick_count) or 0)
        _enqueue_choose_from_topk_filtered(gs, k, rng, cards_db,
                                           filter_kind=kind, filter_group=group,
                                           filter_group2=group2,
                                           ...
                                           min_pick_count=min_pick_count,
                                           after_pick=after_pick,
                                           ctx=ctx)
        return
    if op == 'look_top_3way_split':
        k = int(gd.get('k', 0) or 0)
        _enqueue_look_top_3way_split(gs, k, rng, ctx=ctx)
        return
    if op == 'look_top_reorder_keep_any':
        k = int(gd.get('k', 0) or 0)
        _enqueue_reorder_from_topk_keep_any(gs, k, rng, ctx=ctx)
        return
    if op == 'look_top_reorder_all':
        k = int(gd.get('k', 0) or 0)
        _enqueue_reorder_from_topk_all(gs, k, rng, ctx=ctx)
        return
    if op == 'self_top1_optional_green':
        _rule_refresh_for_top_access(gs, rng, 1, reason='self_top1_optional_green')
        if not gs.deck:
            gs.log.append('[INFO] self_top1_optional_green: deck empty')
            return
        top_cn = gs.deck[0]
        gs.pending.append({
        ...
        gs.log.append(f'[PENDING] self_top1_optional_green: view top1 {top_cn}; choose green/keep')
        return
    if op == 'look_top_stage_member_count_plus_keep_one_top_rest_waiting':
        plus_n = int(gd.get('plus_n', 0) or 0)
        stage_n = 0
        for slot in list((getattr(gs, 'stage', {}) or {}).values()):
            if not slot:
                continue
            ci_slot = _get_card(cards_db, str(getattr(slot, 'cardnumber', '') or ''))
            if ci_slot and _is_member_ci(ci_slot):
                stage_n += 1
        k = max(0, stage_n + plus_n)
        src = str((ctx or {}).get('source_cn', '') or '')
        gs.log.append(f'[AUTO] {src}: look top stage members {stage_n}+{plus_n} -> {k}; keep one on top')
        _enqueue_choose_top_keep_one(gs, k, label=f'{src}: ステージメンバー{stage_n}+{plus_n}', source_cn=src)
        return
    if op == 'reveal_top_by_all_stage_members_live_score_rest_waiting':
        own_n = 0
        for slot in list((getattr(gs, 'stage', {}) or {}).values()):
            if not slot:
                continue
        ...
        src = str((ctx or {}).get('source_cn', '') or '')
        gs.pending.append({
            'kind': 'choose_opponent_stage_count_for_reveal_live_score',
            'text': f'{src or "この能力"}：相手ステージのメンバー数を選んでください。自分側は{own_n}人として、合計枚数を公開します。',
            'options': ['0', '1', '2', '3'],
            'own_stage_count': own_n,
            'score_n': int(gd.get('score_n', 1) or 1),
            'source_cn': src,
            'set_idx': (ctx or {}).get('set_idx', None),
        })
        gs.log.append(f'[PENDING] {src}: choose opponent stage count for reveal live score (own={own_n})')
        return
    if op == 'look_top_stage_group_count_keep_one_top_rest_waiting_reveal_score_if_live':
        group = str(gd.get('group', '') or '').strip()
        score_n = int(gd.get('score_n', 1) or 1)
        k = 0
        for slot in list((getattr(gs, 'stage', {}) or {}).values()):
            if not slot:
                continue
            ci_slot = _get_card(cards_db, str(getattr(slot, 'cardnumber', '') or ''))
            if ci_slot and _is_member_ci(ci_slot) and _ci_matches_group_or_unit(ci_slot, group):
                k += 1
        src = str((ctx or {}).get('source_cn', '') or '')
        followup_ops = [
            {
            ...
            },
            {
                'op': 'show_cards_ack',
                'label': 'デッキトップ公開',
                'text': f'{src or "この能力"}：デッキの一番上を公開しました。',
                'cards_from': 'revealed_top',
            },
        ]
        gs.log.append(f'[AUTO] {src}: stage 『{group}』 members={k}; keep one then reveal score if LIVE')
        _enqueue_choose_top_keep_one(gs, k, label=f'{src}: 『{group}』メンバー{k}人', source_cn=src, set_idx=(ctx or {}).get('set_idx', None), followup_ops=followup_ops)
        return
    if op == 'reveal_until_live_or_cost_ge_member_to_hand_rest_waiting_choice':
        src = str((ctx or {}).get('source_cn', '') or '')
        cost_n = int(gd.get('cost_n', 0) or 0)
        gs.pending.append({
            'kind': 'choose_reveal_until_kind',
            'text': f'{src or "この能力"}：公開する対象を選んでください。',
            'options': ['LIVE', f'MEMBER_COST_GE_{cost_n}'],
            'cost_n': cost_n,
            'source_cn': src,
        })
        gs.log.append(f'[PENDING] {src}: choose reveal-until target LIVE or MEMBER cost>={cost_n}')
        return
    if op == 'reveal_top1_cost_le_member_hand_then_self_position_change':
        cost_n = int(gd.get('cost_n', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        _rule_refresh_for_top_access(gs, rng, 1, reason='reveal_top1_cost_le_member_position')
        if not gs.deck:
            gs.log.append(f'[INFO] {src}: reveal_top1_cost_le_member_position: deck empty')
            return
        top_cn = gs.deck.pop(0)
        ci_top = _get_card(cards_db, top_cn)
        try:
            actual_cost = int(getattr(ci_top, 'cost', 0) or 0) if ci_top else 0
        except Exception:
            actual_cost = 0
        ok = bool(ci_top and _is_member_ci(ci_top) and actual_cost <= cost_n)
        if ok:
            gs.hand.append(top_cn)
            result = f'公開したカード {top_cn} はコスト{actual_cost}のメンバーカードです。手札に加え、このメンバーはポジションチェンジします。'
            if pos in ('L', 'C', 'R') and gs.stage.get(pos):
                gs.pending.append({
                    'kind': 'show_revealed_cards_ack',
                    'label': 'デッキトップ公開',
                    'text': f'{src or "この能力"}：デッキの一番上を公開しました。\n{result}',
                ...
                dst_opts = [p2 for p2 in ('L', 'C', 'R') if p2 != pos]
                gs.pending.append({
                    'kind': 'position_change',
                    'src_pos': pos,
                    'source_cn': src,
                    'text': f'{pos}: {src or "このメンバー"} の移動先を選んでください。移動先にメンバーがいる場合は入れ替わります。',
                    'options': dst_opts,
                })
                gs.log.append(f'[PENDING] {src}: revealed {top_cn} -> hand; position change source={pos}')
            else:
                gs.log.append(f'[WARN] {src}: revealed {top_cn} -> hand; position change skipped (source position missing)')
                gs.pending.append({
                    'kind': 'show_revealed_cards_ack',
                    'label': 'デッキトップ公開',
                    'text': f'{src or "この能力"}：デッキの一番上を公開しました。\n{result}\nただし、このメンバーのステージ位置がないためポジションチェンジは行いません。',
                    'display_cards': [top_cn],
                    'options': ['ok'],
                })
        else:
            gs.green_room.append(top_cn)
            result = f'公開したカード {top_cn} はコスト{cost_n}以下のメンバーカードではありません。控え室に置きます。'
            gs.log.append(f'[AUTO] {src}: revealed {top_cn} -> waiting room (condition failed)')
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': 'デッキトップ公開',
                'text': f'{src or "この能力"}：デッキの一番上を公開しました。\n{result}',
        ...
        return
    if op == 'choose_number_reveal_top_member_cost_compare_hand_or_blade':
        src = str((ctx or {}).get('source_cn', '') or '')
        blade_n = max(1, _count_blade_icons_from_tagblob(str(gd.get('blades', '') or '')))
        gs.pending.append({
            'kind': 'choose_number_reveal_top_member_cost_compare',
            'text': f'{src or "この能力"}：数を1つ選んでください。',
            'options': [str(i) for i in range(0, 16)],
            'blade_n': int(blade_n),
            'source_cn': src,
            'pos': str((ctx or {}).get('pos', '') or '').upper(),
        })
        gs.log.append(f'[PENDING] {src}: choose number for top reveal cost compare')
        return
    ...
    if op == 'energy_zone_to_under_self_optional':
        ...
        draw_n = max(0, int(gd.get('draw_n', 0) or 0))
        blades = str(gd.get('blades', '') or '')
        if n <= 0:
            return
        gs.pending.append({
            'kind': 'pay_or_skip',
            'text': _auto_effect_detail_block(ctx, f'{src or "この能力"}：エネルギー置き場のエネルギー{n}枚をこのメンバーの下に置きますか？'),
            'options': ['pay', 'skip'],
            'cost_kind': 'energy_under_self',
            'cost_n': int(n),
            'after_effect_template': f'カードを{draw_n}枚引く。' if draw_n > 0 else '',
            'ctx': dict(ctx or {}),
            'source_cn': src,
            'stage_blades_after_under': blades,
        })
        gs.log.append(f"[PENDING] {src or '?'}: optional energy x{n} to under at {pos or '?'}")
        return
    if op == 'return_under_energy_any_to_deck_gain_member_icons_per':
        src = str((ctx or {}).get('source_cn', '') or '')
        icons = str(gd.get('icons', '') or '')
        opts = []
        for _pos in ('L', 'C', 'R'):
            _slot = (getattr(gs, 'stage', {}) or {}).get(_pos)
            if _slot and int(getattr(_slot, 'energy_under', 0) or 0) > 0:
                opts.append(_pos)
        if not opts:
            gs.log.append(f"[INFO] {src or '?'}: no stage member with under-energy")
            return
        gs.pending.append({
            'kind': 'choose_stage_member_under_energy_return',
            'text': _auto_effect_detail_block(ctx, f'{src or "この能力"}：下にあるエネルギーをエネルギーデッキに置くメンバーを選んでください。'),
            'options': list(opts) + ['skip'],
            'source_cn': src,
            'icons_per_energy': icons,
            'auto_effect_detail': str((ctx or {}).get('auto_effect_detail', '') or ''),
            'suppress_card_text': bool(str((ctx or {}).get('auto_effect_detail', '') or '')),
        })
        gs.log.append(f"[PENDING] {src or '?'}: choose stage member under-energy to return ({len(opts)} candidates)")
        return
    if op == 'hand_member_cost_le_not_named_to_former_area_then_energy_under':
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        cost_max = max(0, int(gd.get('cost_max', 0) or 0))
        count_n = max(0, int(gd.get('count_n', 1) or 1))
        energy_n = max(0, int(gd.get('energy_n', 1) or 1))
        exclude_name = str(gd.get('exclude_name', '') or '').strip()
        if count_n != 1:
            gs.log.append(f"[WARN] {src or '?'}: hand_member_to_former_area supports count=1 only (got {count_n})")
            return
        if pos not in ('L', 'C', 'R'):
            gs.log.append(f"[WARN] {src or '?'}: former area missing for hand member entry")
            return
        if (getattr(gs, 'stage', {}) or {}).get(pos):
            gs.log.append(f"[WARN] {src or '?'}: former area {pos} is not empty")
            return
        cands: List[str] = []
        for cn0 in list(getattr(gs, 'hand', []) or []):
            ...
            if not ci0 or not _is_member_ci(ci0):
                continue
            name0 = str(getattr(ci0, 'name', '') or getattr(ci0, 'cardname', '') or '')
            if exclude_name and exclude_name in name0:
                continue
            try:
                actual_cost = int(_card_effective_play_cost_from_hand(gs, cards_db, cn0) or getattr(ci0, 'cost', 0) or 0)
            except Exception:
                actual_cost = int(getattr(ci0, 'cost', 0) or 0)
            if actual_cost <= cost_max:
                cands.append(str(cn0))
        if not cands:
            gs.log.append(f"[INFO] {src or '?'}: no hand member candidates cost<={cost_max} not named {exclude_name}")
            gs.pending.append({
                'kind': 'message_ack',
                'source_cn': src,
                'text': _auto_effect_detail_block(ctx, f'条件に合う手札のメンバーがいないため、効果は適用されません。'),
                'options': ['ok'],
            })
            return
        if int(getattr(gs, 'energy_active', 0) or 0) + int(getattr(gs, 'energy_wait', 0) or 0) < energy_n:
            gs.log.append(f"[INFO] {src or '?'}: not enough energy zone cards to place under (need {energy_n})")
            return
        gs.pending.append({
            'kind': 'hand_member_to_former_area_then_energy_under',
            'text': _auto_effect_detail_block(ctx, f'{src or "この能力"}：手札からコスト{cost_max}以下で「{exclude_name}」以外のメンバーを1枚、{pos}に登場させ、その下にエネルギー{energy_n}枚を置きます。'),
            'options': list(cands),
        ...
        gs.log.append(f"[PENDING] {src or '?'}: choose hand member for former area {pos} ({len(cands)} candidates)")
        return
    if op == 'hand_member_cost_le_to_empty_area':
        src = str((ctx or {}).get('source_cn', '') or '')
        cost_max = max(0, int(gd.get('cost_max', 0) or 0))
        count_n = max(0, int(gd.get('count_n', 1) or 1))
        group_name = str(gd.get('group', '') or '').strip()
        optional = bool(rule.get('optional', False))
        if count_n != 1:
            gs.log.append(f"[WARN] {src or '?'}: hand_member_to_empty_area supports count=1 only (got {count_n})")
            return
        empty_positions = _empty_stage_positions(gs)
        if not empty_positions:
            gs.log.append(f"[INFO] {src or '?'}: no empty stage area for hand member entry")
            gs.pending.append({
                'kind': 'message_ack',
                'source_cn': src,
                'text': _auto_effect_detail_block(ctx, 'メンバーのいないエリアがないため、効果は適用されません。'),
                'options': ['ok'],
            })
            return
        cands: List[str] = []
        for cn0 in list(getattr(gs, 'hand', []) or []):
            ...
            if actual_cost <= cost_max:
                cands.append(str(cn0))
        if not cands:
            gs.log.append(f"[INFO] {src or '?'}: no hand member candidates cost<={cost_max} group={group_name or 'ANY'}")
            gs.pending.append({
                'kind': 'message_ack',
                'source_cn': src,
                'text': _auto_effect_detail_block(ctx, f'条件に合う手札のメンバーがいないため、効果は適用されません。'),
        ...
        if optional:
            opts.append('skip')
        gs.pending.append({
            'kind': 'hand_member_to_empty_area',
            'text': _auto_effect_detail_block(ctx, f'{src or "この能力"}：手札からコスト{cost_max}以下の{("「"+group_name+"」" if group_name else "")}メンバーを1枚、メンバーのいないエリアに登場させます。'),
            'options': opts,
            'display_cards': list(cands),
            'source_cn': src,
            'cost_max': int(cost_max),
            'group_name': group_name,
            'empty_positions': list(empty_positions),
            'optional': optional,
            'auto_effect_detail': str((ctx or {}).get('auto_effect_detail', '') or ''),
            'suppress_card_text': bool(str((ctx or {}).get('auto_effect_detail', '') or '')),
        })
        gs.log.append(f"[PENDING] {src or '?'}: choose hand member for empty area ({len(cands)} candidates; empty={empty_positions})")
        return
    if op == 'green_member_cost_le_group_to_former_area':
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        cost_max = max(0, int(gd.get('cost_max', 0) or 0))
        count_n = max(0, int(gd.get('count_n', 1) or 1))
        group_name = str(gd.get('group', '') or '').strip()
        if count_n != 1:
            gs.log.append(f"[WARN] {src or '?'}: green_member_to_former_area supports count=1 only (got {count_n})")
            return
        if pos not in ('L', 'C', 'R'):
            gs.log.append(f"[WARN] {src or '?'}: former area missing for waiting-room member entry")
            return
        if (getattr(gs, 'stage', {}) or {}).get(pos):
            gs.log.append(f"[WARN] {src or '?'}: former area {pos} is not empty")
            return
        ...
        for cn0 in list(getattr(gs, 'green_room', []) or []):
            ...
            if actual_cost <= cost_max:
                cands.append(str(cn0))
        if not cands:
            gs.log.append(f"[INFO] {src or '?'}: no waiting-room member candidates cost<={cost_max} group={group_name}")
            gs.pending.append({
                'kind': 'message_ack',
                'source_cn': src,
                'text': _auto_effect_detail_block(ctx, f'条件に合う控え室のメンバーがいないため、効果は適用されません。'),
                'options': ['ok'],
            })
            return
        gs.pending.append({
            'kind': 'green_member_to_former_area',
            'text': _auto_effect_detail_block(ctx, f'{src or "この能力"}：控え室からコスト{cost_max}以下の「{group_name}」メンバーを1枚、{pos}に登場させます。'),
            'options': list(cands),
            'display_cards': list(cands),
            'source_cn': src,
            'target_pos': pos,
    ...
    if op == 'green_member_cost_le_to_empty_area':
        ...
        count_n = max(0, int(gd.get('count_n', 1) or 1))
        group_name = str(gd.get('group', '') or '').strip()
        energy_icons = str(gd.get('energy_icons', '') or '')
        need_e = int(energy_icons.count('<(E)>') or 0)
        if count_n != 1:
            gs.log.append(f"[WARN] {src or '?'}: green_member_to_empty_area supports count=1 only (got {count_n})")
            return
        empty_positions = _empty_stage_positions(gs)
        if not empty_positions:
            gs.log.append(f"[INFO] {src or '?'}: no empty stage area for waiting-room member entry")
            return
        cands: List[str] = []
        for cn0 in list(getattr(gs, 'green_room', []) or []):
            ci0 = _get_card(cards_db, cn0)
            ...
            if actual_cost <= cost_max:
                cands.append(str(cn0))
        if not cands:
            gs.log.append(f"[INFO] {src or '?'}: no waiting-room member candidates cost<={cost_max} group={group_name or 'ANY'}")
            gs.pending.append({
                'kind': 'message_ack',
                'source_cn': src,
                'text': _auto_effect_detail_block(ctx, f'条件に合う控え室のメンバーがいないため、効果は適用されません。'),
        ...
        if need_e > 0:
            if int(getattr(gs, 'energy_active', 0) or 0) < need_e:
                gs.log.append(f"[ERR] {src or '?'}: insufficient active energy for embedded [E]{need_e} (have {getattr(gs, 'energy_active', 0)})")
                return
            gs.energy_active = int(getattr(gs, 'energy_active', 0) or 0) - need_e
            gs.energy_wait = int(getattr(gs, 'energy_wait', 0) or 0) + need_e
            try:
                _clamp_energy_zone(gs)
            except Exception:
                pass
            gs.log.append(f"[COST] {src or '?'}: paid embedded [E]{need_e} -> energy_active={gs.energy_active} energy_wait={gs.energy_wait}")
        gs.pending.append({
            'kind': 'green_member_to_empty_area',
            'text': _auto_effect_detail_block(ctx, f'{src or "この能力"}：控え室からコスト{cost_max}以下の{("「"+group_name+"」" if group_name else "")}メンバーを1枚、メンバーのいないエリアに登場させます。'),
            'options': list(cands),
            'display_cards': list(cands),
            'source_cn': src,
            'cost_max': int(cost_max),
        ...
        gs.log.append(f"[PENDING] {src or '?'}: choose waiting-room member for empty area ({len(cands)} candidates; empty={empty_positions})")
        return
    if op == 'energy_put_active':
        n = int(gd.get('n', 0) or 0)
        ...
        if add > 0:
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': 'エネルギー設置確認',
                'text': _auto_effect_detail_block(ctx, f'{src or "この効果"}：エネルギーデッキからエネルギーカードを{add}枚アクティブ状態で置きました。'),
                'display_cards': [],
                'options': ['ok'],
                'source_cn': src,
            })
    ...
    if op == 'gain_blade_until_end_live':
        ...
        return
    if op == 'gain_blade_if_self_moved_this_turn':
        blades_blob = gd.get('blades', '')
        b = _count_blade_icons_from_tagblob(blades_blob)
        pos = str(ctx.get('pos', '') or '').upper()
        src = str((ctx or {}).get('source_cn', '') or '')
        slot = gs.stage.get(pos) if pos in ('L','C','R') else None
        if not slot:
            gs.log.append(f'[WARN] {src}: moved-self blade has no source slot')
            return
        src_cn = _canon_cardno(src or getattr(slot, 'cardnumber', '') or '')
        moved_set = {_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_moved_cardnumbers_this_turn', []) or [])}
        moved_set = {x for x in moved_set if x}
        moved_any = bool(getattr(gs, 'stage_moved_this_turn', False))
        moved_ok = bool(src_cn and src_cn in moved_set) or (moved_any and not moved_set)
        if not moved_ok:
            gs.log.append(f'[SKIP] {src_cn or src}: self moved this turn -> condition not met')
            return
        slot.temp_blade += int(b)
        slot.temp_until = 'end_of_live'
        gs.log.append(f'[AUTO] {pos}: self moved this turn -> gain blade +{b} (until end_of_live)')
        return
    if op == 'moved_stage_members_gain_icons_until_end_live':
        icons_blob = str(gd.get('icons', '') or '')
        group_name = str(gd.get('group', '') or '').strip()
        positions = _moved_stage_member_positions_this_turn(gs, cards_db, tag=group_name)
        if not positions:
            gs.log.append('[SKIP] moved stage members gain icons: no moved staged member' + (f' for 『{group_name}』' if group_name else ''))
            return
        applied = []
        for pos2 in positions:
            ...
            if not slot2:
                continue
            b2, hs2 = _grant_temp_icons_to_slot(slot2, icons_blob, 1)
            applied.append(f'{pos2}(blade+{b2}, hearts={hs2})')
        gs.log.append(f'[AUTO] moved stage members {positions} gain {icons_blob} until end_of_live: {applied}')
        return
    if op == 'choose_heart_for_moved_stage_members_until_end_live':
        positions = _moved_stage_member_positions_this_turn(gs, cards_db)
        if not positions:
            gs.log.append('[SKIP] choose heart for moved stage members: no moved staged member')
            return
        choices_blob = str(gd.get('choices', '') or '')
        opts: List[str] = []
        for m0 in re.finditer(r'<(?:\(([^)]+)\)|([^<>]+))>', choices_blob):
            jp = str((m0.group(1) or m0.group(2) or '')).strip()
            if jp in _HEART_JP_MAP and jp not in opts:
                opts.append(jp)
        if not opts:
            opts = ['桃', '赤', '黄', '緑', '青', '紫']
        src = str((ctx or {}).get('source_cn', '') or '')
        gs.pending.append({
            'kind': 'choose_heart_color_for_moved_stage_members',
            'text': _auto_effect_detail_block(ctx, f'このターン中にエリアを移動したステージ上メンバーへ与えるハートを選んでください。対象: {positions}'),
            'options': list(opts),
        ...
        gs.log.append(f'[PENDING] {src}: choose heart color for moved stage members {positions}')
        return
    if op == 'choose_heart_for_entered_stage_members_except_group_until_end_live':
        exclude_group = str(gd.get('exclude_group', '') or '').strip()
        positions = _entered_stage_member_positions_this_turn(gs, cards_db, exclude_tag=exclude_group)
        if not positions:
            gs.log.append(f'[SKIP] choose heart for entered stage members: no entered staged member outside 『{exclude_group}』')
            return
        choices_blob = str(gd.get('choices', '') or '')
        opts: List[str] = []
        for m0 in re.finditer(r'<(?:\(([^)]+)\)|([^<>]+))>', choices_blob):
        ...
        src = str((ctx or {}).get('source_cn', '') or '')
        gs.pending.append({
            'kind': 'choose_heart_color_for_stage_positions',
            'text': _auto_effect_detail_block(ctx, f'このターン中に登場した対象メンバーへ与えるハートを選んでください。対象: {positions}'),
            'options': list(opts),
            'target_positions': list(positions),
            'source_cn': src,
            'auto_effect_detail': str((ctx or {}).get('auto_effect_detail', '') or ''),
        ...
        gs.log.append(f'[PENDING] {src}: choose heart color for entered stage members {positions} excluding {exclude_group}')
        return
    if op == 'live_success_score_if_self_moved_by_group_effect':
        group_name = str(gd.get('group', '') or '').strip()
        n = int(gd.get('n', 0) or 0)
        src = str((ctx or {}).get('source_cn', '') or '')
        pos = str((ctx or {}).get('pos', '') or '').upper()
        if not _self_moved_by_group_effect_this_turn(gs, cards_db, src, group_name):
            gs.log.append(f'[SKIP] {src}: not moved by 『{group_name}』 effect this turn')
            return
        _add_live_success_total_score_bonus(gs, cards_db, n, detail=f'{src} moved by 『{group_name}』 effect this turn', source_cn=src, pos=pos)
        return
    if op == 'entered_or_moved_stage_group_members_reduce_required_any_per_member':
        group_name = str(gd.get('group', '') or '').strip()
        per = len(re.findall(r'<\(任意\)>', str(gd.get('anys', '') or ''))) or 1
        positions = _entered_or_moved_stage_member_positions_this_turn(gs, cards_db, tag=group_name)
        total = int(len(positions) * per)
        src = _canon_cardno(str((ctx or {}).get('source_cn', '') or ''))
        set_idx = (ctx or {}).get('set_idx', None)
        if total <= 0:
        ...
        try:
            if set_idx is not None:
                rmap = dict(getattr(gs, 'live_start_required_any_reduction_by_set_idx', {}) or {})
                k = int(set_idx)
                rmap[k] = int(rmap.get(k, 0) or 0) + total
                gs.live_start_required_any_reduction_by_set_idx = rmap
            elif src:
                rmap = dict(getattr(gs, 'live_start_required_any_reduction_by_cn', {}) or {})
                rmap[src] = int(rmap.get(src, 0) or 0) + total
                gs.live_start_required_any_reduction_by_cn = rmap
        except Exception:
            pass
    ...
    if op == 'position_change_self':
        pos = str((ctx or {}).get('pos', '') or '').upper()
        src = str((ctx or {}).get('source_cn', '') or '')
        if pos not in ('L', 'C', 'R') or not gs.stage.get(pos):
            gs.log.append(f'[WARN] {src}: position_change_self has no source slot')
            return
        dst_opts = [p for p in ('L', 'C', 'R') if p != pos]
        gs.pending.append({
            'kind': 'position_change',
            'src_pos': pos,
            'source_cn': src or getattr(gs.stage.get(pos), 'cardnumber', ''),
            'text': _auto_effect_detail_block(ctx, f'{pos}: {src or "このメンバー"} の移動先を選んでください。移動先にメンバーがいる場合は入れ替わります。'),
            'options': dst_opts,
            'auto_effect_detail': str((ctx or {}).get('auto_effect_detail', '') or ''),
            'suppress_card_text': bool(str((ctx or {}).get('auto_effect_detail', '') or '')),
        })
        gs.log.append(f'[PENDING] {src}: position_change_self source={pos} -> choose destination')
        return
    ...
    if op == 'gain_icons_until_end_live':
        ...
        return
    if op == 'area_group_member_gain_icons_until_end_live':
        area_raw = str(gd.get('area', '') or '').strip()
        group_name = str(gd.get('group', '') or '').strip()
        icons_blob = str(gd.get('icons', '') or '')
        area_to_pos = {
            'センター': 'C',
        ...
        target_pos = area_to_pos.get(area_raw, '')
        slot = gs.stage.get(target_pos) if target_pos in ('L', 'C', 'R') else None
        src = str((ctx or {}).get('source_cn', '') or '')
        if not slot:
            gs.log.append(f'[INFO] {src}: no member in {area_raw or target_pos} area for group icon gain')
            return
        ci_target = _get_card(cards_db, getattr(slot, 'cardnumber', '') or '')
        if not ci_target or not _is_member_ci(ci_target):
            gs.log.append(f'[INFO] {src}: target in {area_raw or target_pos} is not a member for group icon gain')
            return
        group_blob = (str(getattr(ci_target, 'group', '') or '') + ' / ' + str(getattr(ci_target, 'unit', '') or ''))
        if group_name and group_name not in group_blob:
            gs.log.append(f'[INFO] {src}: {target_pos} member does not match 『{group_name}』 for group icon gain')
            return
        b = _count_blade_icons_from_tagblob(icons_blob)
        if b > 0:
            slot.temp_blade += b
        ...
        for col, cnt in heart_counts.items():
            _grant_temp_heart(slot, col, cnt)
        gs.log.append(f'[AUTO] {src}: {target_pos} 『{group_name}』 member gains icons {icons_blob} (until end_of_live; blades={b} hearts={heart_counts})')
        return
...
def try_apply_effect_template(gs: 'GameState', rng: random.Random, cards_db: Dict[str, CardInfo], effect_text: str, ctx: Dict[str, Any]) -> bool:
    ...
    m_baton_lower_enter = re.match(r"^このメンバーよりコストが低い『(?P<group>[^』]+)』のメンバーからバトンタッチして登場した場合、(?P<inner>.+)$", text_norm)
    if m_baton_lower_enter:
        group_name = str(m_baton_lower_enter.group('group') or '').strip()
        inner = str(m_baton_lower_enter.group('inner') or '').strip()
        old_cn = _canon_cardno(str((ctx or {}).get('baton_old_cn', '') or ''))
        pos = str((ctx or {}).get('pos', '') or '').upper()
        try:
            old_cost = int((ctx or {}).get('baton_old_cost', 0) or 0)
        except Exception:
            old_cost = 0
        slot_now = (getattr(gs, 'stage', {}) or {}).get(pos)
        try:
            new_cost = int(_slot_effective_cost(gs, cards_db, pos, slot_now) or 0)
        except Exception:
            ci_now = _get_card(cards_db, str((ctx or {}).get('source_cn', '') or ''))
            new_cost = int(getattr(ci_now, 'cost', 0) or 0) if ci_now else 0
        ci_old = _get_card(cards_db, old_cn)
        ok = bool(old_cn and ci_old and _is_member_ci(ci_old) and _ci_matches_group_or_unit(ci_old, group_name) and old_cost < new_cost)
        if not ok:
            skip_msg = f'enter baton lower-cost group condition not met: old={old_cn or "?"} old_cost={old_cost} new_cost={new_cost} group={group_name}'
            try:
            ...
            return True
        detail = _auto_effect_detail_for_condition(ctx, text_norm, f'バトンタッチ元 {old_cn} のコスト {old_cost} < このメンバーのコスト {new_cost} / 『{group_name}』', '登場時')
        ctx2 = _with_auto_effect_detail(ctx, detail)
        return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))

    m_baton_lower_any_enter = re.match(r"^このメンバーよりコストが低いメンバーからバトンタッチして登場した場合、(?P<inner>.+)$", text_norm)
    if m_baton_lower_any_enter:
        inner = str(m_baton_lower_any_enter.group('inner') or '').strip()
        old_cn = _canon_cardno(str((ctx or {}).get('baton_old_cn', '') or ''))
        pos = str((ctx or {}).get('pos', '') or '').upper()
        try:
            old_cost = int((ctx or {}).get('baton_old_cost', 0) or 0)
        ...
        try:
            ...
            new_cost = int(getattr(ci_now, 'cost', 0) or 0) if ci_now else 0
        ci_old = _get_card(cards_db, old_cn)
        ok = bool(old_cn and ci_old and _is_member_ci(ci_old) and old_cost < new_cost)
        if not ok:
            skip_msg = f'enter baton lower-cost condition not met: old={old_cn or "?"} old_cost={old_cost} new_cost={new_cost}'
            try:
                ctx['_effect_skipped_reason'] = skip_msg
            except Exception:
                pass
            gs.log.append(f'[SKIP] {skip_msg}')
            return True
        detail = _auto_effect_detail_for_condition(ctx, text_norm, f'バトンタッチ元 {old_cn} のコスト {old_cost} < このメンバーのコスト {new_cost}', '登場時')
        ctx2 = _with_auto_effect_detail(ctx, detail)
        return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))
    ...
    # Conditional wrappers based on current own stage members.
    m_stage_count = re.match(r'^自分のステージにメンバーが(?P<count>\d+)人以上いる場合、(?P<inner>.+)$', text_norm)
    if m_stage_count:
        need = int(m_stage_count.group('count') or 0)
        got = int(_stage_member_count(gs, cards_db) or 0)
        inner = str(m_stage_count.group('inner') or '').strip()
        if got >= need:
            gs.log.append(f'[AUTO] stage member count {got}/{need} -> apply inner effect')
            ctx2 = _with_auto_effect_detail(ctx, _auto_effect_detail_for_condition(ctx, text_norm, f'ステージのメンバー {got}/{need}人', '自動効果'))
            return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))
        gs.log.append(f'[SKIP] stage member count {got}/{need} -> condition not met')
        return True

    m_stage_group = re.match(r'^自分のステージに『(?P<tag>[^』]+)』のメンバーがいる場合、(?P<inner>.+)$', text_norm)
    if m_stage_group:
        tag = str(m_stage_group.group('tag') or '').strip()
        inner = str(m_stage_group.group('inner') or '').strip()
        if _stage_has_group_or_unit_member(gs, cards_db, tag):
            gs.log.append(f'[AUTO] stage has 『{tag}』 member -> apply inner effect')
            ctx2 = _with_auto_effect_detail(ctx, _auto_effect_detail_for_condition(ctx, text_norm, f'ステージに『{tag}』メンバーがいる', '自動効果'))
            return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))
        gs.log.append(f'[SKIP] stage has 『{tag}』 member -> condition not met')
        return True

    m_stage_group_entered = re.match(r'^このターン、自分のステージに『(?P<tag>[^』]+)』のメンバーが登場している場合、(?P<inner>.+)$', text_norm)
    if m_stage_group_entered:
        tag = str(m_stage_group_entered.group('tag') or '').strip()
        inner = str(m_stage_group_entered.group('inner') or '').strip()
        if _stage_group_or_unit_entered_this_turn(gs, cards_db, tag):
            gs.log.append(f'[AUTO] this turn stage entered 『{tag}』 member -> apply inner effect')
            ctx2 = _with_auto_effect_detail(ctx, _auto_effect_detail_for_condition(ctx, text_norm, f'このターンに『{tag}』メンバーが登場している', '自動効果'))
            return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))
        gs.log.append(f'[SKIP] this turn stage entered 『{tag}』 member -> condition not met')
        return True

    m_stage_enter_count_gte = re.match(r'^このターン、自分のステージにメンバーが(?P<count>\d+)回以上登場している場合、(?P<inner>.+)$', text_norm)
    if m_stage_enter_count_gte:
        need = int(m_stage_enter_count_gte.group('count') or 0)
        got = int(getattr(gs, 'stage_enter_count_this_turn', 0) or 0)
        inner = str(m_stage_enter_count_gte.group('inner') or '').strip()
        if got >= need:
            gs.log.append(f'[AUTO] this turn stage enter count {got}/{need} -> apply inner effect')
            ctx2 = _with_auto_effect_detail(ctx, _auto_effect_detail_for_condition(ctx, text_norm, f'このターンの登場回数 {got}/{need}', '自動効果'))
            return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))
        gs.log.append(f'[SKIP] this turn stage enter count {got}/{need} -> condition not met')
        return True

    m_stage_enter_count_exact_event = re.match(r'^このターン、自分のステージにメンバーが(?P<count>\d+)回登場したとき、(?P<inner>.+)$', text_norm)
    if m_stage_enter_count_exact_event:
        need = int(m_stage_enter_count_exact_event.group('count') or 0)
        got = int(getattr(gs, 'stage_enter_count_this_turn', 0) or 0)
        inner = str(m_stage_enter_count_exact_event.group('inner') or '').strip()
        if got == need:
            gs.log.append(f'[AUTO] this turn stage enter count reached {got}/{need} -> apply inner effect')
            ctx2 = _with_auto_effect_detail(ctx, _auto_effect_detail_for_condition(ctx, text_norm, f'このターンの登場回数 {got}/{need}', '自動効果'))
            return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))
        gs.log.append(f'[SKIP] this turn stage enter count {got}/{need} -> exact event condition not met')
        return True

    m_self_wait_then = re.match(r'^このメンバーをウェイトにする。その後、(?P<inner>.+)$', text_norm)
    if m_self_wait_then:
        pos = str((ctx or {}).get('pos', '') or '').upper()
        src = str((ctx or {}).get('source_cn', '') or '')
        slot = gs.stage.get(pos) if pos in ('L', 'C', 'R') else None
        if not slot:
            gs.log.append(f'[WARN] {src}: self wait then effect has no source slot')
            return True
        slot.active = False
        inner = str(m_self_wait_then.group('inner') or '').strip()
        gs.log.append(f'[AUTO] {src}: stage {pos} set WAIT; apply follow-up')
        ctx2 = _with_auto_effect_detail(ctx, _auto_effect_detail_for_condition(ctx, text_norm, 'このメンバーをウェイトにした', '自動効果'))
        return bool(try_apply_effect_template(gs, rng, cards_db, inner, ctx2))
...
def _stage_has_group_or_unit_member_cost_at_least(gs: 'GameState', cards_db: Dict[str, CardInfo], tag: str, min_cost: int) -> bool:
    tag = str(tag or '').strip()
    if not tag:
        return False
    need = int(min_cost or 0)
    for pos in ('L', 'C', 'R'):
        slot = (getattr(gs, 'stage', {}) or {}).get(pos)
        if not slot or not getattr(slot, 'cardnumber', ''):
            continue
        ...
        if not ci or not _is_member_ci(ci):
            continue
        if not _ci_matches_group_or_unit(ci, tag):
            continue
        if int(_slot_effective_cost(gs, cards_db, pos, slot) or 0) >= need:
            return True
    return False
...
def _build_choose_effects_prompt_from_ability(
    ...
    *,
    ...
    header_cost = ''
    try:
        header_cl = clauses[header_i] if isinstance(clauses[header_i], dict) else {}
        header_cost = str(header_cl.get('cost_template', '') or '').strip()
    except Exception:
        header_cost = ''
    ...
    m_stage_cost_group = re.search(r"自分のステージにコスト(?P<cost>\d+)以上の『(?P<tag>[^』]+)』のメンバーがいる場合、以下から", header_text)
    if m_stage_cost_group:
        tag = str(m_stage_cost_group.group('tag') or '').strip()
        min_cost = int(m_stage_cost_group.group('cost') or 0)
        if tag and not _stage_has_group_or_unit_member_cost_at_least(gs, cards_db, tag, min_cost):
            gs.log.append(f'[PENDING] {ttl}[{timing}]: choose block condition unmet (no stage 『{tag}』 member cost>={min_cost})')
            return {
                'kind': 'message_ack',
                'label': f'{ttl} {timing} condition unmet',
                'source_cn': src,
                'text': f'【{ttl}】{timing}：自分のステージにコスト{min_cost}以上の『{tag}』のメンバーがいないため、効果は適用されません。',
                'options': ['ok'],
            }
    ...
    if m_stage_cost_group:
        ...
        min_cost = int(m_stage_cost_group.group('cost') or 0)
        if tag:
            msg = f"{ttl}: ステージにコスト{min_cost}以上の『{tag}』のメンバーがいる場合、以下から{'1つ以上' if max_pick>1 else '1つ'}を選ぶ"
    ...
    header_need_e = int(_parse_energy_cost(header_cost) or 0) if header_cost else 0
    if header_need_e > 0:
        prompt['cost_kind'] = 'energy'
...
class GameState:
    ...
    # True only after the current live's normal YELL action itself has completed.
    # Separate this from reveal acknowledgement so zero-card YELL and deferred
    # pre-YELL prompts can be distinguished without an empty NEXT-only pause.
    yell_flow_completed_this_live: bool = False
    ...
    # Rule refresh notice ledger for owner/public UI.  Entries are UI-safe
    # summaries of cards moved from waiting room to main deck by refresh.
    refresh_notice_seq: int = 0
    refresh_notices: List[Dict[str, Any]] = field(default_factory=list)
    ...
    yell_heart_convert_target_color_this_live: str = ""
    yell_reveal_count_delta_this_live: int = 0
    ...
    # Per-turn movement state.  A card is recorded when it changes stage area
    # by position-change style effects.  This supports effects that ask whether
    # a member on stage has moved areas during this turn.
    ...
    # Detailed effect/event ledger for UI debugging.  Keep this compact and
    # bounded; the text log remains the primary historical trace.
    effect_events: List[Dict[str, Any]] = field(default_factory=list)
    # Cards drawn specifically by YELL blade-heart draw icons during this live,
    # in draw order.  Reroll effects that say「そのエールで得たブレードハートを失う」
    # can reverse only the draw icons from the discarded revealed cards.
def _post_process_auto_live_attempt_after_yell(gs: GameState) -> bool:
    """Continue a completed YELL flow directly into the live attempt summary.

    This is engine-owned rather than UI-owned so every client path behaves the
    same.  Revealed cards still require the one-time acknowledgement; zero-card
    YELL can continue immediately.  Any YELL-time auto prompt or deferred auto
    queue blocks progression until it is fully resolved.
    """
    try:
        if str(getattr(gs, 'phase', '') or '') != 'LIVE_ATTEMPT':
            return False
        if not bool(getattr(gs, 'yell_flow_completed_this_live', False)):
            return False
        if list(getattr(gs, 'pending', []) or []):
            return False
        if getattr(gs, '_deferred_auto_queue', None):
            return False
        revealed = list(getattr(gs, '_yell_revealed_this_live', []) or [])
        if revealed and not bool(getattr(gs, 'yell_reveal_acknowledged_this_live', False)):
            return False
        cards_db = getattr(gs, '_cards_db', None)
        if not isinstance(cards_db, dict) or not cards_db:
            return False

        cmd_attempt(gs, cards_db)
        kinds = [str((p or {}).get('kind', '') or '') for p in list(getattr(gs, 'pending', []) or []) if isinstance(p, dict)]
        if 'live_attempt_summary_ack' in kinds or not list(getattr(gs, 'set_zone', []) or []) or bool(getattr(gs, 'cannot_live_until_end_of_live', False)):
            gs.phase = 'LIVE_RESOLVE'
            gs.log.append(f"[PHASE] LIVE_RESOLVE (ACK + next turn) turn={gs.turn}")
            gs.log.append('[AUTO] completed YELL flow -> live attempt summary')
            return True
        # A late live-start prompt can still appear defensively.  Leave phase in
        # LIVE_ATTEMPT so it resolves before attempting again.
        return False
    except Exception as e:
        try:
            gs.log.append(f'[WARN] engine auto live-attempt continuation failed: {e}')
        except Exception:
            pass
        return False
...
def post_process(gs: GameState) -> None:
    """Post-process deferred prompts and timing continuations."""
    ...
    _post_process_auto_live_attempt_after_yell(gs)
...
def snapshot_state(gs: GameState) -> Dict[str, Any]:
    ...
    return {
        ...
        "effect_events": [dict(x) for x in (getattr(gs, "effect_events", []) or []) if isinstance(x, dict)],
        "effect_event_seq": int(getattr(gs, "_effect_event_seq", 0) or 0),
        ...
        "refresh_notice_seq": int(getattr(gs, "refresh_notice_seq", 0) or 0),
        "refresh_notices": [dict(x) for x in (getattr(gs, "refresh_notices", []) or []) if isinstance(x, dict)],
        ...
        "yell_heart_convert_target_color_this_live": str(getattr(gs, "yell_heart_convert_target_color_this_live", "") or ""),
        "yell_reveal_count_delta_this_live": int(getattr(gs, "yell_reveal_count_delta_this_live", 0) or 0),
...
def restore_state(gs: GameState, snap: Dict[str, Any]) -> None:
    ...
    try:
        ...
        gs.used_this_turn = {}
    gs.stage_moved_this_turn = bool(snap.get("stage_moved_this_turn", getattr(gs, "stage_moved_this_turn", False)))
    try:
        gs.stage_moved_cardnumbers_this_turn = [str(x) for x in list(snap.get("stage_moved_cardnumbers_this_turn", getattr(gs, "stage_moved_cardnumbers_this_turn", []) or []) or []) if str(x or '').strip()]
    except Exception:
    ...
    try:
        gs.effect_events = [dict(x) for x in list(snap.get("effect_events", getattr(gs, "effect_events", []) or []) or []) if isinstance(x, dict)]
    except Exception:
        gs.effect_events = []
    try:
        setattr(gs, '_effect_event_seq', int(snap.get('effect_event_seq', getattr(gs, '_effect_event_seq', 0) or 0) or 0))
    except Exception:
        setattr(gs, '_effect_event_seq', 0)
    ...
    gs.refresh_notice_seq = int(snap.get('refresh_notice_seq', getattr(gs, 'refresh_notice_seq', 0) or 0) or 0)
    gs.refresh_notices = [dict(x) for x in (snap.get('refresh_notices', getattr(gs, 'refresh_notices', []) or []) or []) if isinstance(x, dict)]
    try:
    ...
    gs.yell_heart_convert_target_color_this_live = str(snap.get('yell_heart_convert_target_color_this_live', getattr(gs, 'yell_heart_convert_target_color_this_live', '') or '') or '')
    gs.yell_reveal_count_delta_this_live = int(snap.get('yell_reveal_count_delta_this_live', getattr(gs, 'yell_reveal_count_delta_this_live', 0) or 0) or 0)
...
def begin_turn(gs: GameState, rng: Optional[random.Random] = None) -> None:
    ...
    gs.stage_entered_cardnumbers_this_turn = []
    # Keep refresh_notices until UI-side acknowledgement; public viewers may poll late.
    try:
        gs.live_set_limit = 3
    except Exception:
        pass
...
def _append_effect_event(gs: 'GameState', event_type: str, source_cn: str = '', detail: str = '', **data: Any) -> None:
    """Append a bounded, UI-safe effect event for debugging.

    This is intentionally separate from the human-readable text log: log lines
    are good for console traces, while effect_events gives the UI/Codex a small
    structured ledger of important effect resolutions.
    """
    try:
        seq = int(getattr(gs, '_effect_event_seq', 0) or 0) + 1
        setattr(gs, '_effect_event_seq', seq)
        ev: Dict[str, Any] = {
            'seq': int(seq),
            'turn': int(getattr(gs, 'turn', 0) or 0),
            'phase': str(getattr(gs, 'phase', '') or ''),
            'type': str(event_type or ''),
            'source_cn': _canon_cardno(source_cn) if source_cn else '',
        ...
        for k, v in (data or {}).items():
            if isinstance(v, (str, int, float, bool)) or v is None:
                ...
                ev[str(k)] = [str(x) if not isinstance(x, (int, float, bool)) else x for x in list(v)[:20]]
            elif isinstance(v, dict):
                ev[str(k)] = {str(kk): vv for kk, vv in list(v.items())[:20] if isinstance(vv, (str, int, float, bool)) or vv is None}
            else:
                ev[str(k)] = str(v)
        cur = [dict(x) for x in (getattr(gs, 'effect_events', []) or []) if isinstance(x, dict)]
        cur.append(ev)
        gs.effect_events = cur[-80:]
    except Exception:
        pass


def _draw_cards_for_yell(gs: 'GameState', n: int, rng: Optional[random.Random], reason: str = 'yell') -> Tuple[int, List[str]]:
    """Draw cards caused by YELL blade-heart draw icons and return the card list."""
    drawn: List[str] = []
    k = 0
    requested = max(0, int(n or 0))
    for i in range(requested):
        if not gs.deck:
            _rule_refresh_main_deck(gs, rng, reason=f'{reason}@draw')
        if not gs.deck:
            break
        cn = gs.deck.pop(0)
        gs.hand.append(cn)
        ...
        k += 1
        if not gs.deck:
            _rule_refresh_main_deck(gs, rng, reason=f'{reason}@draw_exhaust')
    if k < requested:
        try:
            green_n = len(getattr(gs, 'green_room', []) or [])
        except Exception:
            green_n = 0
        try:
            deck_n = len(getattr(gs, 'deck', []) or [])
        except Exception:
            deck_n = 0
        gs.log.append(f"[YELL-DEBUG] draw shortage requested={requested} drew={k} deck_n={deck_n} green_n={green_n} ({reason})")
    return k, drawn


def _yell_draw_icon_debug_parts(cards_db: Dict[str, CardInfo], revealed: List[str]) -> List[Dict[str, Any]]:
    parts: List[Dict[str, Any]] = []
    for cn in list(revealed or []):
        ci = _get_card(cards_db, cn)
        raw = ''
        tags = ''
        draw = 0
        if ci:
            try:
                ...
                raw = ''
            try:
                tags = str(getattr(ci, 'blade_heart_tags_json', '') or '')
            except Exception:
                tags = ''
            try:
                draw = int(_ci_yell_draw_icon_count(ci) or 0)
            except Exception:
                draw = 0
        parts.append({'cn': str(cn or ''), 'raw': raw, 'tags': tags, 'draw': int(draw or 0)})
    return parts


def _ci_yell_draw_icon_count(ci: Optional[CardInfo]) -> int:
    """Count YELL blade-heart draw icons from both parsed tags and raw text.

    Some card DB exports leave ``blade_heart_tags_json`` empty even when
    ``blade_heart_raw`` contains tokens such as ``<ドロー+1>``.  Rule processing
    must therefore not depend only on the parsed tag column.  Use the maximum of
    tag-based and raw-text counts to avoid double-counting when both are present.
    """
    if not ci:
        return 0
    tag_n = 0
    try:
        tag_n = int(_count_draw_icons(getattr(ci, 'blade_heart_tags_json', '') or '') or 0)
    except Exception:
        tag_n = 0
    raw_n = 0
    try:
        raw = _normalize_icon_token_text(str(getattr(ci, 'blade_heart_raw', '') or ''))
        for m in re.finditer(r'<(?:\(([^)]+)\)|([^<>]+))>', raw):
            inner = str((m.group(1) or m.group(2) or '')).strip()
            key = str(inner or '').strip().replace('＋', '+').replace(' ', '')
            m = re.match(r'^ドロー([+\-]\d+)$', key)
            if m:
                raw_n += max(0, int(m.group(1)))
    except Exception:
        raw_n = 0
    return max(int(tag_n or 0), int(raw_n or 0))


def _note_yell_bladeheart_loss_for_cards(gs: 'GameState', cards_db: Dict[str, CardInfo], revealed_cards: List[str], reason: str = '') -> Dict[str, int]:
    """Record blade-heart loss from a resolved YELL-time reroll effect.

    Rule order is: YELL reveals cards, draw icons from those revealed blade-hearts
    are processed, then the check timing resolves YELL-time automatic abilities.
    Therefore losing the blade-heart at that later check timing must not undo
    cards already drawn from draw icons.  Removing the revealed cards from
    resolve/tracking prevents their heart/score blade-hearts from being counted
    later in the live attempt; draw icons remain already processed.
    """
    draw_icons = 0
    heart_like = 0
    for cn in list(revealed_cards or []):
        ci = _get_card(cards_db, cn)
        if not ci:
            continue
        try:
            draw_icons += int(_ci_yell_draw_icon_count(ci) or 0)
        except Exception:
            pass
        try:
            raw = str(getattr(ci, 'blade_heart_raw', '') or '')
            if raw and raw not in ('なし', '-', '0'):
                heart_like += 1
        except Exception:
            pass
    gs.log.append(f"[YELL] blade-hearts lost after draw-icon processing ({reason or 'reroll'}; draw icons already resolved x{draw_icons})")
    _append_effect_event(gs, 'yell_bladeheart_loss', detail='lost revealed blade-hearts after draw-icon processing; no draw undo', cards=list(revealed_cards or []), lost_draw_icons=draw_icons, reason=reason)
    return {'draw_icons': int(draw_icons), 'cards': len(list(revealed_cards or [])), 'bladeheart_cards': int(heart_like)}


def _yell_draw_notice_payload(cards_db: Dict[str, CardInfo], revealed: Optional[List[str]], draw_icon_count: int = 0, drew_count: int = 0, drawn_cards: Optional[List[str]] = None) -> Dict[str, Any]:
    """Small metadata for the YELL-revealed-cards confirmation popup.

    This is intentionally UI-only metadata.  It does not affect rule state.
    The popup should show that draw icons from this YELL have already been
    processed before the following YELL-time automatic abilities are resolved.
    """
    try:
        n = int(draw_icon_count or 0)
    except Exception:
        n = 0
    if n <= 0:
        return {}
    try:
        drew = int(drew_count or 0)
    except Exception:
        drew = 0
    icon_cards: List[str] = []
    for cn in list(revealed or []):
        ...
        if not ci:
            continue
        try:
            cnt = int(_ci_yell_draw_icon_count(ci) or 0)
        except Exception:
            cnt = 0
        if cnt > 0:
            icon_cards.append(str(cn))
    return {
        'yell_draw_icon_count': int(n),
        'yell_draw_drew_count': int(drew),
        'yell_draw_icon_cards': icon_cards,
        'yell_draw_drawn_cards': [str(x) for x in list(drawn_cards or []) if str(x or '').strip()],
        'yell_draw_processed': True,
        'yell_draw_notice_label': f'ドローアイコン×{n} → {drew}枚ドロー済み',
        'yell_draw_notice_text': f'このエールで公開されたドローアイコン×{n}を先に処理し、カードを{drew}枚引きました。',
    }
...
def _parse_energy_to_deck_cost(cost_text: str) -> int:
    """Parse activated costs like 'エネルギー2枚をエネルギーデッキに置く'.

    This is not an <(E)> payment.  It removes cards from the current energy
    zone and implicitly returns them to the hidden energy deck count.
    """
    t = str(cost_text or '')
    m = re.search(r'エネルギー\s*(\d+)\s*枚をエネルギーデッキに置く', t)
    if not m:
        return 0
    try:
        return max(0, int(m.group(1) or 0))
    except Exception:
        return 0


def pay_energy_to_deck(gs: GameState, n: int, reason: str = '') -> bool:
    """Return n current energy cards to the energy deck, WAIT/REST first.

    The energy deck is represented implicitly by energy_total minus cards in play,
    so reducing active/wait is sufficient.
    """
    n = max(0, int(n or 0))
    if n <= 0:
        return True
    active = int(getattr(gs, 'energy_active', 0) or 0)
    wait = int(getattr(gs, 'energy_wait', 0) or 0)
    if active + wait < n:
        return False
    take_wait = min(wait, n)
    take_active = n - take_wait
    try:
        gs.energy_wait = wait - take_wait
        gs.energy_active = active - take_active
    except Exception:
        return False
    try:
        msg = f"[COST] energy -> energy deck x{n} (wait-{take_wait}, active-{take_active}; active={gs.energy_active}, wait={gs.energy_wait})"
        if reason:
            msg += f" {reason}"
        gs.log.append(msg)
    except Exception:
...
def _move_energy_zone_to_under_self(gs: 'GameState', pos: str, n: int, reason: str = '') -> int:
    """Move up to n energy from the energy zone under the stage member at pos."""
    pos = str(pos or '').upper()
    slot = gs.stage.get(pos) if pos in ('L', 'C', 'R') else None
    if not slot or not getattr(slot, 'cardnumber', ''):
        return 0
    ...
    active = int(getattr(gs, 'energy_active', 0) or 0)
    wait = int(getattr(gs, 'energy_wait', 0) or 0)
    take = min(want, active + wait)
    if take <= 0:
        return 0
    take_wait = min(wait, take)
    take_active = take - take_wait
    gs.energy_wait = wait - take_wait
    gs.energy_active = active - take_active
    slot.energy_under = int(getattr(slot, 'energy_under', 0) or 0) + take
    msg = f"[COST] {pos}: energy zone -> under x{take} (wait-{take_wait}, active-{take_active}; under={int(getattr(slot, 'energy_under', 0) or 0)})"
    if reason:
        msg += f" {reason}"
    gs.log.append(msg)
    return int(take)
...
def refresh(gs: GameState) -> None:
    ...
    gs.energy_wait = 0
def _refresh_notice_card_summary(gs: GameState, moved_cards: List[str], reason: str = '', mode: str = '', need: int = 0, cur: int = 0) -> Dict[str, Any]:
    """Build a UI/public-safe summary for a main-deck refresh.

    Rules: refresh moves all waiting-room cards to the main deck.  The UI shows
    only the moved public waiting-room cards, never hidden deck contents.
    """
    cards_db = getattr(gs, '_cards_db', None)
    moved = [str(cn) for cn in (moved_cards or []) if str(cn or '').strip()]
    live_counts: Dict[str, int] = {}
    no_bh = 0
    for cn in moved:
        ci = _get_card(cards_db, cn) if isinstance(cards_db, dict) else None
        try:
            if ci is not None and not _ci_has_blade_heart_payload(ci):
                no_bh += 1
        except Exception:
            pass
        try:
            if ci is not None and _is_live_ci(ci):
                live_counts[cn] = int(live_counts.get(cn, 0) or 0) + 1
        except Exception:
            pass
    live_cards = [{'cardnumber': cn, 'count': int(n)} for cn, n in sorted(live_counts.items())]
    return {
        'kind': 'refresh_notice',
        'reason': str(reason or ''),
        'mode': str(mode or ''),
        'need': int(need or 0),
        'deck_count_before': int(cur or 0),
        'returned_total': int(len(moved)),
        'returned_no_bladeheart_count': int(no_bh),
        'returned_live_cards': live_cards,
    }


def _append_refresh_notice(gs: GameState, moved_cards: List[str], reason: str = '', mode: str = '', need: int = 0, cur: int = 0) -> None:
    try:
        seq = int(getattr(gs, 'refresh_notice_seq', 0) or 0) + 1
        gs.refresh_notice_seq = seq
        notice = _refresh_notice_card_summary(gs, moved_cards, reason=reason, mode=mode, need=need, cur=cur)
        notice['seq'] = int(seq)
        notices = [dict(x) for x in (getattr(gs, 'refresh_notices', []) or []) if isinstance(x, dict)]
        notices.append(notice)
        gs.refresh_notices = notices[-8:]
    except Exception as e:
        try:
            gs.log.append(f'[WARN] refresh notice failed: {e}')
        except Exception:
            pass
...
def _rule_refresh_main_deck(gs: GameState, rng: Optional[random.Random], reason: str = '') -> bool:
    ...
    _append_refresh_notice(gs, moved, reason=reason, mode='empty_deck', need=0, cur=0)
...
def _rule_refresh_for_top_access(gs: GameState, rng: Optional[random.Random], need: int, reason: str = '') -> bool:
    ...
    gs.deck = keep + pool
    gs.deck_refreshed_this_turn = True
    _append_refresh_notice(gs, moved, reason=reason, mode='top_access_shortage', need=need, cur=cur)
...
def _stage_distinct_member_group_name_count(gs: 'GameState', cards_db: Dict[str, CardInfo]) -> int:
    ...
    try:
        for pos in ('L', 'C', 'R'):
            ...
            if not ci or _is_live_ci(ci):
                continue
            groups.update(_card_group_names(ci))
    except Exception:
        pass
    return int(len(groups))

def _stage_member_group_name_set(gs: 'GameState', cards_db: Dict[str, CardInfo]) -> Set[str]:
    """Return non-empty group names from members currently on stage."""
    groups: Set[str] = set()
    try:
        for pos in ('L', 'C', 'R'):
            slot = (getattr(gs, 'stage', {}) or {}).get(pos)
...
def _card_group_names(ci: Optional[CardInfo]) -> Set[str]:
    """Return non-empty group names for a card.

    Keep this conservative: blank group data is not treated as a wildcard for
    "different group name" effects.
    """
    out: Set[str] = set()
    if not ci:
        return out
    for raw in (str(getattr(ci, 'group', '') or ''),):
    ...
    nm = str(getattr(ci, 'name', '') or getattr(ci, 'cardname', '') or '').strip()
    if cn in {'PL!-bp5-111', 'PL!-bp5-222', 'PL!-bp5-333'} or nm in {'綺羅ツバサ', '優木あんじゅ', '統堂英玲奈'}:
        out.add('A-RISE')
    elif cn.startswith('PL!SP-'):
        out.add('Liella!')
    elif cn.startswith('PL!HS-'):
        out.add('蓮ノ空')
    elif cn.startswith('PL!S-'):
        out.add('Aqours')
    elif cn.startswith('PL!N-'):
        out.add('虹ヶ咲')
    elif cn.startswith('PL!-'):
        out.add("μ's")
    return out

def _green_cards_with_group_different_from_stage_members(gs: 'GameState', cards_db: Dict[str, CardInfo]) -> List[str]:
    """Cards in waiting room whose group name does not overlap any stage member group."""
    stage_groups = _stage_member_group_name_set(gs, cards_db)
    if not stage_groups:
        return []
    out: List[str] = []
    for cn in list(getattr(gs, 'green_room', []) or []):
        ci = _get_card(cards_db, cn)
        groups = _card_group_names(ci)
        if groups and groups.isdisjoint(stage_groups):
            out.append(str(cn))
    return out

def _empty_stage_positions(gs: 'GameState') -> List[str]:
    out: List[str] = []
    try:
        for pos in ('L', 'C', 'R'):
            if not (getattr(gs, 'stage', {}) or {}).get(pos):
                out.append(pos)
    except Exception:
        pass
    return out
...
def _record_stage_member_enter(gs: 'GameState', cn: str, pos: str = '', origin: str = '') -> None:
    canon = _canon_cardno(str(cn or ''))
    if not canon:
        return
    try:
        gs.stage_enter_count_this_turn = int(getattr(gs, 'stage_enter_count_this_turn', 0) or 0) + 1
    except Exception:
        gs.stage_enter_count_this_turn = 1
    try:
        cur = [str(x) for x in list(getattr(gs, 'stage_entered_cardnumbers_this_turn', []) or []) if str(x or '').strip()]
    except Exception:
        cur = []
    cur.append(canon)
    try:
        gs.stage_entered_cardnumbers_this_turn = cur
    except Exception:
        pass
    gs.log.append(f'[STATE] stage enter this turn count={int(getattr(gs, "stage_enter_count_this_turn", 0) or 0)} cn={canon} pos={str(pos or "").upper()} origin={str(origin or "")}')

def _stage_group_or_unit_entered_this_turn(gs: 'GameState', cards_db: Dict[str, CardInfo], tag: str) -> bool:
    tag = str(tag or '').strip()
    if not tag:
        return False
    entered = [_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_entered_cardnumbers_this_turn', []) or [])]
    entered = [x for x in entered if x]
    for cn in entered:
        ci = _get_card(cards_db, cn)
        if ci and _is_member_ci(ci) and _ci_matches_group_or_unit(ci, tag):
            return True
    return False

def _entered_stage_member_positions_this_turn(gs: 'GameState', cards_db: Dict[str, CardInfo], exclude_tag: str = '') -> List[str]:
    entered = [_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_entered_cardnumbers_this_turn', []) or [])]
    entered = [x for x in entered if x]
    if not entered:
        return []
    exclude_tag = str(exclude_tag or '').strip()
    out: List[str] = []
    for pos, slot in (getattr(gs, 'stage', {}) or {}).items():
        pos_u = str(pos or '').upper()
        if pos_u not in ('L', 'C', 'R') or not slot:
            continue
        cn = _canon_cardno(str(getattr(slot, 'cardnumber', '') or ''))
        if not cn or cn not in entered:
            continue
        ci = _get_card(cards_db, cn)
        if not ci or not _is_member_ci(ci):
            continue
        if exclude_tag and _ci_matches_group_or_unit(ci, exclude_tag):
            continue
        if pos_u not in out:
            out.append(pos_u)
    return out

def _entered_or_moved_stage_member_positions_this_turn(gs: 'GameState', cards_db: Dict[str, CardInfo], tag: str = '') -> List[str]:
    entered = {_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_entered_cardnumbers_this_turn', []) or [])}
    moved = {_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_moved_cardnumbers_this_turn', []) or [])}
    touched = {x for x in (entered | moved) if x}
    if not touched:
        return []
    tag = str(tag or '').strip()
    out: List[str] = []
    for pos, slot in (getattr(gs, 'stage', {}) or {}).items():
        ...
        if pos_u not in ('L', 'C', 'R') or not slot:
            continue
        cn = _canon_cardno(str(getattr(slot, 'cardnumber', '') or ''))
        if not cn or cn not in touched:
            continue
        ci = _get_card(cards_db, cn)
        if not ci or not _is_member_ci(ci):
            continue
...
def _record_stage_area_movement(gs: 'GameState', cards: List[str], src_pos: str = '', dst_pos: str = '', reason: str = '', source_cn: str = '') -> None:
    """Record that one or more stage members moved between stage areas this turn."""
    moved: List[str] = []
    for cn0 in list(cards or []):
        canon = _canon_cardno(str(cn0 or ''))
        if canon and canon not in moved:
            moved.append(canon)
    if not moved:
        return
    try:
        ...
        pass
    try:
        cur = [str(x) for x in list(getattr(gs, 'stage_moved_cardnumbers_this_turn', []) or []) if str(x or '').strip()]
    except Exception:
        cur = []
    for cn in moved:
        if cn not in cur:
            cur.append(cn)
    try:
        gs.stage_moved_cardnumbers_this_turn = cur
    except Exception:
        pass
    try:
    ...
    log.append({
        ...
        'cards': list(moved),
        'src_pos': str(src_pos or '').upper(),
        'dst_pos': str(dst_pos or '').upper(),
        'reason': str(reason or 'stage-area-move'),
        'source_cn': _canon_cardno(source_cn),
    })
    try:
        gs.stage_movement_log_this_turn = log
    except Exception:
        pass

def _stage_position_of_card(gs: 'GameState', cn: str) -> str:
    canon = _canon_cardno(cn)
    if not canon:
        return ''
    for pos, slot in (getattr(gs, 'stage', {}) or {}).items():
        if slot and _canon_cardno(str(getattr(slot, 'cardnumber', '') or '')) == canon:
...
def _collect_stage_movement_auto_triggers(gs: 'GameState', cards_db: Dict[str, CardInfo], moved_cards: List[str]) -> List[Dict[str, Any]]:
    ...
    seen = set()
    for cn0 in list(moved_cards or []):
        cn = _canon_cardno(cn0)
        if not cn or cn in seen:
            continue
        seen.add(cn)
        pos = _stage_position_of_card(gs, cn)
        if not pos:
            continue
        ci = _get_card(cards_db, cn)
        if not ci:
        ...
        for ab in list(getattr(ci, 'abilities', []) or []):
            ...
            for cl in list(ab.get('clauses', []) or []):
                ...
                if not m:
                    ...
                    if m and str(getattr(gs, 'phase', '') or '').upper() != 'MAIN':
                        continue
                if not m:
                    continue
...
def _enqueue_stage_movement_auto_triggers(gs: 'GameState', cards_db: Dict[str, CardInfo], moved_cards: List[str]) -> int:
    triggers = _collect_stage_movement_auto_triggers(gs, cards_db, moved_cards)
    if not triggers:
        return 0
    opts = [_auto_trigger_option_text(t) for t in triggers if _auto_trigger_option_text(t)]
    gs.pending.append({
        'kind': 'auto_order',
        'text': 'エリア移動時の自動効果が発生：解決するカードを選択（1つずつ）',
        'options': opts,
        'queue': list(triggers),
    })
...
def _stage_has_moved_group_or_unit_member_this_turn(gs: 'GameState', cards_db: Dict[str, CardInfo], tag: str = '') -> bool:
    """Return True if a currently staged member of tag moved areas this turn."""
    if not bool(getattr(gs, 'stage_moved_this_turn', False)):
        return False
    tag = str(tag or '').strip()
    moved_set = {_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_moved_cardnumbers_this_turn', []) or [])}
    moved_set = {x for x in moved_set if x}
    if not moved_set:
        return False
    for slot in (getattr(gs, 'stage', {}) or {}).values():
        if not slot or not getattr(slot, 'cardnumber', ''):
            continue
        canon = _canon_cardno(getattr(slot, 'cardnumber', '') or '')
        if canon not in moved_set:
            continue
        ci = _get_card(cards_db, canon)
        if not ci or not _is_member_ci(ci):
            continue
        if not tag or _ci_matches_group_or_unit(ci, tag):
            return True
    return False

def _moved_stage_member_positions_this_turn(gs: 'GameState', cards_db: Dict[str, CardInfo], tag: str = '', exclude_pos: str = '') -> List[str]:
    if not bool(getattr(gs, 'stage_moved_this_turn', False)):
        return []
    tag = str(tag or '').strip()
    exclude_pos = str(exclude_pos or '').upper()
    moved_set = {_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_moved_cardnumbers_this_turn', []) or [])}
    moved_set = {x for x in moved_set if x}
    if not moved_set:
        return []
    out: List[str] = []
    for pos, slot in (getattr(gs, 'stage', {}) or {}).items():
        pos0 = str(pos or '').upper()
        if exclude_pos and pos0 == exclude_pos:
            continue
        if not slot or not getattr(slot, 'cardnumber', ''):
            continue
        canon = _canon_cardno(getattr(slot, 'cardnumber', '') or '')
...
def _self_moved_by_group_effect_this_turn(gs: 'GameState', cards_db: Dict[str, CardInfo], source_cn: str, group: str) -> bool:
    src = _canon_cardno(source_cn)
    group = str(group or '').strip()
    if not src or not bool(getattr(gs, 'stage_moved_this_turn', False)):
        return False
    logs = [x for x in list(getattr(gs, 'stage_movement_log_this_turn', []) or []) if isinstance(x, dict)]
    for ev in logs:
        cards = {_canon_cardno(str(x or '')) for x in list(ev.get('cards', []) or [])}
        if src not in cards:
            continue
        eff_src = _canon_cardno(str(ev.get('source_cn', '') or ''))
        ci_eff = _get_card(cards_db, eff_src) if eff_src else None
        if ci_eff and (not group or _ci_matches_group_or_unit(ci_eff, group)):
            return True
    moved_set = {_canon_cardno(str(x or '')) for x in list(getattr(gs, 'stage_moved_cardnumbers_this_turn', []) or [])}
    if src in moved_set:
        ci_self = _get_card(cards_db, src)
        if ci_self and (not group or _ci_matches_group_or_unit(ci_self, group)):
            return True
    return False

def _body_always_moved_stage_group_hand_cost_reduction_from_blob(gs: 'GameState', cards_db: Dict[str, CardInfo], blob: str) -> int:
    """Parse BODY 常時: hand cost -N while a staged group/unit member moved areas this turn."""
    try:
        b = _norm_digits_jp(str(blob or '')).replace(' ', '').replace('\n', '')
        m = re.search(r'自分のステージにいる『([^』]+)』のメンバーがこのターンにエリアを移動しているかぎり、手札にあるこのメンバーカードのコストは(\d+)減る', b)
        if not m:
            return 0
        tag = str(m.group(1) or '').strip()
        red = int(m.group(2) or 0)
        if tag and red > 0 and _stage_has_moved_group_or_unit_member_this_turn(gs, cards_db, tag):
            return red
        return 0
    except Exception:
        return 0

def _body_always_other_hand_cards_cost_reduction_from_blob(gs: 'GameState', blob: str) -> int:
    """Parse BODY 常時: cost -N per other card in hand."""
    try:
        b = _norm_digits_jp(str(blob or '')).replace(' ', '').replace('\n', '')
        m = re.search(r'手札にあるこのメンバーカードのコストは、このカード以外の自分の手札1枚につき、(\d+)少なくなる', b)
        if not m:
            return 0
        per = int(m.group(1) or 0)
        if per <= 0:
            return 0
        other = max(0, len(list(getattr(gs, 'hand', []) or [])) - 1)
        return int(per) * int(other)
    except Exception:
        return 0


def _body_always_stage_group_hand_cost_reduction_from_blob(gs: 'GameState', cards_db: Dict[str, CardInfo], blob: str) -> int:
    """Parse BODY 常時: cost -N per matching group/unit member on stage."""
    try:
        b = _norm_digits_jp(str(blob or '')).replace(' ', '').replace('\n', '')
        m = re.search(r'手札にあるこのメンバーカードのコストは、自分のステージにいる『([^』]+)』のメンバー1人につき、(\d+)少なくなる', b)
        if not m:
            return 0
        tag = str(m.group(1) or '').strip()
        per = int(m.group(2) or 0)
        if not tag or per <= 0:
            return 0
        cnt = 0
        for slot in (getattr(gs, 'stage', {}) or {}).values():
            ...
            if ci and _is_member_ci(ci) and _ci_matches_group_or_unit(ci, tag):
                cnt += 1
        return int(per) * int(cnt)
    except Exception:
        return 0


def _body_always_wait_group_hand_cost_reduction_from_blob(gs: 'GameState', cards_db: Dict[str, CardInfo], blob: str) -> int:
    """Parse BODY 常時: cost -N while a matching group/unit member is wait on stage."""
    try:
        b = _norm_digits_jp(str(blob or '')).replace(' ', '').replace('\n', '')
        m = re.search(r'自分のステージにウェイト状態の『([^』]+)』のメンバーがいるかぎり、手札にあるこのメンバーカードのコストは(\d+)減る', b)
        if not m:
            return 0
        tag = str(m.group(1) or '').strip()
        red = int(m.group(2) or 0)
...
def _card_effective_play_cost_from_hand(gs: 'GameState', cards_db: Dict[str, CardInfo], cn: str) -> int:
    ...
    for _eff, blob in _iter_body_always_effects(ci):
        ...
        reduction += int(_body_always_other_hand_cards_cost_reduction_from_blob(gs, blob) or 0)
        ...
        for _eff, blob in _iter_body_always_effects(ci):
            try:
                ...
                if 'エリアすべてに『' in blob and 'のメンバーが登場しており、かつ名前が異なる場合' in blob and 'ライブの合計スコアを+1する' in blob:
                    ...
                    continue
                elif 'このメンバーの下にエネルギーカードが' in blob and '枚以上置かれているかぎり' in blob and 'ライブの合計スコアを+' in blob:
                    m = re.search(r'このメンバーの下にエネルギーカードが(\d+)枚以上置かれているかぎり、ライブの合計スコアを\+(\d+)する', blob)
                    need = int(m.group(1)) if m else 0
                    add = int(m.group(2)) if m else 0
                    if need and add and int(getattr(slot, 'energy_under', 0) or 0) >= need:
                        bonus += add
            except Exception:
                pass
        return int(bonus)
...
def cheer_hearts_from_resolve(gs: GameState, cards_db: Dict[str, CardInfo]) -> Dict[str, int]:
    ...
    convert_target = str(getattr(gs, 'yell_heart_convert_target_color_this_live', '') or '').strip().lower()
    if convert_target:
        pool = _convert_pool_to_single_color(pool, convert_target)
    elif bool(getattr(gs, 'vivid_world_blue_mode_this_live', False)):
...
def _enqueue_choose_top_keep_one(gs: 'GameState', k: int, label: str = '', source_cn: str = '', set_idx: Optional[int] = None, followup_ops: Optional[List[Dict[str, Any]]] = None) -> None:
    ...
    if len(top) == 1:
        ...
        gs.pending.insert(0, {
            ...
            'text': f'{label}：公開されたカードから、デッキの一番上に残すカードを1枚選んでください。選ばなかったカードは控え室に置きます。',
            'options': [keep],
            'top_cards': [keep],
            'label': str(label or ''),
            'source_cn': str(source_cn or ''),
            ...
            'default_live_score_followup': False,
        })
        return
    gs.pending.insert(0, {
        'kind': 'choose_top_keep_one',
...
def _resolve_choose_top_keep_one(gs: 'GameState', p: Dict[str, Any], choice_str: str, cards_db: Dict[str, CardInfo]) -> bool:
    ...
    if not bool((p or {}).get('default_live_score_followup', False)):
        gs.log.append(f'[ACT] choose_top_keep_one: kept {keep}; rest {len(rest)} -> waiting room')
        return True
...
def _convert_pool_nonblue_plus_any_to_blue(pool: Dict[str, int]) -> Dict[str, int]:
    return _convert_pool_to_single_color(pool, 'blue')

def _convert_pool_to_single_color(pool: Dict[str, int], target_color: str) -> Dict[str, int]:
    target = str(target_color or '').strip().lower()
    if target not in {'pink', 'red', 'yellow', 'green', 'blue', 'purple'}:
        return {str(k).lower(): int(v or 0) for k, v in (pool or {}).items()}
    out = {str(k).lower(): int(v or 0) for k, v in (pool or {}).items()}
    moved = 0
    for k in ('pink', 'red', 'yellow', 'green', 'blue', 'purple', 'all'):
        if k == target:
            continue
        ...
        if k in out:
            out.pop(k, None)
    if moved > 0:
        out[target] = int(out.get(target, 0) or 0) + moved
    return out
    ...
    for pos in ("L", "C", "R"):
        ...
        for ab in ci.abilities:
            ...
            for cl in clauses:
                ...
                if need_e <= 0:
                    need_e = 1
                m_eff_generic = _match_effect_template(eff)
                # Prefer the generic effect route when the post-cost effect itself is
                # matchable.  The old blade-only fallback counts blade icons anywhere
                # in the sentence, so mixed/targeted effects such as
                # 「他の『蓮ノ空』メンバー1人は、<桃><ブレード>を得る」 were
                # incorrectly treated as self blade +1.
                if m_eff_generic:
                    pr = {
                        "kind": "live_start_pay_effect",
                        "pos": pos,
...
def _clear_end_of_live_buffs(gs: GameState, cards_db: Optional[Dict[str, CardInfo]] = None) -> None:
    ...
    try:
        ...
        pass
    try:
        gs.yell_reveal_count_delta_this_live = 0
        ...
        gs.yell_flow_completed_this_live = False
    except Exception:
        pass
def cmd_play(gs: GameState, cards_db: Dict[str, CardInfo], hand_idx: int, pos: str) -> None:
    ...
    _record_stage_member_enter(gs, c.cardnumber if c else cn, pos=pos, origin='hand')
    try:
    ...
    for ab in _iter_triggered_abilities(ci, '登場'):
        ...
        for cl in clauses:
            ...
            if ('以下から' in eff) and ('選ぶ' in eff):
                return True
    return False
def handle_enter_auto(gs: GameState, cards_db: Dict[str, CardInfo], pos: str, cn: str, rng: Optional[random.Random] = None, baton_old_cn: str = '', baton_old_cost: int = 0, entry_origin: str = 'hand') -> None:
    ...
    for ab in _iter_triggered_abilities(ci, '登場'):
        ...
        for cl in clauses:
            ...
            if cost:
                ...
                if n > 0:
                    ...
                    _enqueue_discard_from_hand(
                        gs,
                        n,
                        label=_pretty_optional_effect_prompt_text('登場', canon, cost, eff) + '\nコストとして控え室に置く手札を選んでください。',
                        ctx=ctx,
                        after_effect_template=eff,
                        after_ctx=ctx,
                        after_source_cn=canon,
            ...
            ctx = {
                'pos': pos.upper(),
                'source_cn': canon,
                'auto_effect_detail': f'【{canon}】登場時効果\n効果：{eff}',
            ...
            if try_apply_effect_template(gs, rng, cards_db, eff, ctx):
                skipped_reason = str(ctx.get('_effect_skipped_reason', '') or '').strip()
                if skipped_reason:
                    gs.log.append(f"[SKIP] {canon}[登場]: {skipped_reason}")
                else:
...
def _collect_auto_triggers_on_member_leave_stage(gs: GameState, cards_db: Dict[str, CardInfo], left_pos: str, left_cn: str, baton_new_cn: str = '') -> List[Dict[str, Any]]:
    ...
    return out
def _collect_auto_triggers_on_member_enter(gs: GameState, cards_db: Dict[str, CardInfo], entered_pos: str, entered_cn: str, baton_old_cn: str = '', baton_old_cost: int = 0, entry_origin: str = 'hand') -> List[Dict[str, Any]]:
    ...
    enter_count = int(getattr(gs, 'stage_enter_count_this_turn', 0) or 0)
    for src_pos, slot in (getattr(gs, 'stage', {}) or {}).items():
        pos_u = str(src_pos or '').upper()
        if pos_u not in ('L', 'C', 'R') or not slot:
            continue
        src_cn = _canon_cardno(str(getattr(slot, 'cardnumber', '') or ''))
        ...
        for ab in list(getattr(ci_src, 'abilities', []) or []):
            ...
            for cl in list(ab.get('clauses', []) or []):
                ...
                inner = str(m.group('inner') or '').strip()
                if enter_count != need or not inner or not _match_effect_template(inner):
                    continue
                out.append({
                    'kind': 'stage_enter_count_auto',
                    'source_cn': src_cn,
                    'pos': pos_u,
                    'effect': inner,
                    'effect_text': eff_norm,
                    'label': f'{src_cn}[登場{need}回目]',
                    'entered_pos': str(entered_pos or 'C').upper(),
                    'entered_cn': entered_cn,
                })
    return out
...
def _exec_auto_trigger(gs: GameState, cards_db: Dict[str, CardInfo], trig: Dict[str, Any]) -> None:
    ...
    if kind == 'enter_auto':
        ...
        handle_enter_auto(
            ...
            pos,
            cn,
            baton_old_cn=str(trig.get('baton_old_cn', '') or ''),
            baton_old_cost=int(trig.get('baton_old_cost', 0) or 0),
            entry_origin=str(trig.get('entry_origin', 'hand') or 'hand'),
        )
        return
    ...
    if kind == 'stage_movement_auto':
        ...
        if eff:
            ...
            detail = _auto_effect_detail_for_condition({'source_cn': src_cn}, full_eff or eff, 'このメンバーがエリアを移動した', timing='移動時')
            ctx = {'source_cn': src_cn, 'auto_effect_detail': detail}
            if pos:
    ...
    if kind == 'stage_enter_count_auto':
        ...
        if eff:
            try:
                ...
                rng2 = random.Random(31)
            detail = _auto_effect_detail_for_condition({'source_cn': src_cn}, full_eff or eff, 'このターンの登場回数条件を満たした', timing='登場時')
            ctx = {'source_cn': src_cn, 'auto_effect_detail': detail}
            if pos:
                ctx.update({'pos': pos, 'src_pos': pos})
            try_apply_effect_template(gs, rng2, cards_db, eff, ctx)
            gs.log.append(f"[AUTO] {src_cn or '?'}[登場回数]: applied {eff}")
        return
    ...
    if kind == 'live_start_convert_revealed_colors_to_single_color_until_end_of_live':
        ...
        if target_key in {'pink', 'red', 'yellow', 'green', 'blue', 'purple'}:
            gs.yell_heart_convert_target_color_this_live = target_key
            gs.vivid_world_blue_mode_this_live = bool(target_key == 'blue')
            gs.log.append(f'[AUTO] live-start: cheer non-{target_key}/all -> {target_key} until end of live')
        else:
            gs.log.append(f'[WARN] live-start color-convert unsupported target={target_color_jp}')
        return
    if kind == 'live_start_yell_reveal_count_delta_if_other_stage_members_at_least':
        src_cn = str((trig or {}).get('source_cn', '') or '')
        src_pos = str((trig or {}).get('pos', '') or '').upper()
        need = int((trig or {}).get('condition_other_member_count', 0) or 0)
        delta = int((trig or {}).get('yell_reveal_count_delta', 0) or 0)
        other_count = 0
        try:
            for pos, slot in (getattr(gs, 'stage', {}) or {}).items():
                if src_pos and str(pos or '').upper() == src_pos:
                    continue
                if not slot or not getattr(slot, 'cardnumber', ''):
                    continue
                ci = _get_card(cards_db, getattr(slot, 'cardnumber', '') or '')
            ...
            other_count = 0
        if other_count < need:
            gs.log.append(f'[SKIP] {src_cn}[ライブ開始時]: other members {other_count} < {need} -> yell reveal count unchanged')
            return
        before = int(getattr(gs, 'yell_reveal_count_delta_this_live', 0) or 0)
        gs.yell_reveal_count_delta_this_live = int(before + delta)
        gs.log.append(f'[AUTO] {src_cn}[ライブ開始時]: other members {other_count} >= {need} -> yell reveal count {delta:+d} until end of live')
        return
    ...
    if kind == 'live_start_reduce_any_and_score_if_success_score_at_least':
        ...
        delta = int((trig or {}).get('score_delta', 0) or 0)
        try:
            total_now = int(_success_zone_score_total(gs, cards_db) or 0)
        except Exception:
            total_now = int(_own_success_zone_score_sum(gs, cards_db) or 0)
        if score_th and total_now >= score_th:
            cond_status = {'state': 'met', 'text': f'条件達成: 成功置き場のスコア合計 {total_now} ≥ {score_th}'}
        elif reduce_th and total_now >= reduce_th:
            cond_status = {'state': 'met', 'text': f'条件達成: 成功置き場のスコア合計 {total_now} ≥ {reduce_th}'}
        else:
            need_min = min([x for x in (reduce_th, score_th) if int(x or 0) > 0] or [0])
            cond_status = {'state': 'unmet', 'text': f'条件未達成: 成功置き場のスコア合計 {total_now} < {need_min}'}
        gs.pending.append({
        ...
        return
    if kind == 'live_start_required_any_reduction_per_success_count':
        src_cn = str((trig or {}).get('source_cn', '') or '')
        per = int((trig or {}).get('required_any_reduction_per', 0) or 0)
        own_n = len(list(getattr(gs, 'success_zone', []) or []))
        total_red = int(per * own_n)
        gs.pending.append({
            'kind': 'live_start_required_any_reduction_per_success_count',
            'source_cn': src_cn,
            'set_idx': (trig or {}).get('set_idx', None),
            'required_any_reduction_per': per,
            'own_success_count': own_n,
            'text': f'【{src_cn}】ライブ開始時：自分の成功ライブカード置き場{own_n}枚 × 必要ハート<(任意)>-{per} → 合計-{total_red}。',
            'options': ['ok'],
        })
        return
    if kind == 'live_start_score_and_required_increase_per_success_count':
        src_cn = str((trig or {}).get('source_cn', '') or '')
        per_score = int((trig or {}).get('score_delta_per', 0) or 0)
        per_icons = _ordered_heart_counts(dict((trig or {}).get('required_increase_per', {}) or {}))
        own_n = len(list(getattr(gs, 'success_zone', []) or []))
        total_score = int(per_score * own_n)
        icon_text = ', '.join([f'{k}+{int(v or 0) * own_n}' for k, v in per_icons.items()]) or 'なし'
        gs.pending.append({
            'kind': 'live_start_score_and_required_increase_per_success_count',
            'source_cn': src_cn,
            'set_idx': (trig or {}).get('set_idx', None),
            'score_delta_per': per_score,
            'required_increase_per': dict(per_icons),
            'own_success_count': own_n,
            'text': f'【{src_cn}】ライブ開始時：自分の成功ライブカード置き場{own_n}枚 × スコア+{per_score} / 必要ハート増加({icon_text})。',
            'options': ['ok'],
        })
        return
    ...
    if kind in ('choose_green_card_with_group_different_from_stage_members_on_live_success',):
        src_cn = str((trig or {}).get('source_cn', '') or '').strip()
        count = max(1, int((trig or {}).get('count', 1) or 1))
        cands = _green_cards_with_group_different_from_stage_members(gs, cards_db)
        stage_groups = sorted(_stage_member_group_name_set(gs, cards_db))
        if not cands:
            gs.log.append(f"[SKIP] LIVE: {src_cn}[ライブ成功時] no waiting-room cards with group different from stage groups {stage_groups}")
            gs.pending.append({
                'kind': 'message_ack',
                'source_cn': src_cn,
                'text': f'{src_cn}[ライブ成功時]：ステージのグループ名 {stage_groups or ["未設定"]} と異なるグループ名を持つ控え室カードがないため、手札に加えません。',
                'options': ['ok'],
            })
            return
        if count != 1:
            gs.log.append(f"[WARN] LIVE: {src_cn}[ライブ成功時] different-stage-group retrieve currently resolves one card at a time (requested {count})")
        gs.pending.append({
            'kind': 'choose_card_from_green',
            'text': f'{src_cn}[ライブ成功時]：ステージのグループ名 {stage_groups} と異なるグループ名を持つ控え室カードを{count}枚手札に加えます。',
            'options': list(cands),
            'candidates': list(cands),
            'display_cards': list(cands),
            'source_cn': src_cn,
        ...
        gs.log.append(f"[PENDING] LIVE: {src_cn}[ライブ成功時] choose green cards with different group from stage groups candidates={len(cands)} picks={count}")
        return
    ...
    if kind in ('add_live_success_score_bonus_if_revealed_live_or_stage_heart_kinds_or_moved',):
        ...
        moved_ok = bool(_stage_has_moved_group_or_unit_member_this_turn(gs, cards_db, ''))
        if got_live >= need_live or got_kinds >= need_kinds or moved_ok:
            why = f"revealed LIVE={got_live}/{need_live}, stage heart kinds={got_kinds}/{need_kinds}, moved_this_turn={moved_ok}"
            _add_live_success_score_bonus(gs, src_cn, bonus, detail=why)
            return
        gs.pending.append({
            'kind': 'confirm_effect',
            'text': f'{src_cn}[ライブ成功時] エール公開ライブは{got_live}/{need_live}枚、ステージのハート種類数は{got_kinds}/{need_kinds}種類、記録済みのエリア移動はありません。このカードのスコアを+{bonus}する条件を手動で満たす場合は「使う」、満たさない場合は「スキップ」を選んでください。',
            'options': ['apply', 'skip'],
            'after_live_success_score_bonus': {'source_cn': src_cn, 'bonus': bonus, 'detail': f'manual position-change condition; revealed LIVE={got_live}/{need_live}; stage heart kinds={got_kinds}/{need_kinds}; moved_this_turn=False'},
            'source_cn': src_cn,
        })
...
def cmd_yell(gs: GameState, rng: random.Random, cards_db: Dict[str, CardInfo]) -> None:
    ...
    if _enqueue_live_start_prompts(gs, cards_db) > 0:
        ...
        return
    base_n = stage_blade(gs, cards_db)
    n = _current_yell_reveal_count(gs, base_n)
    if n != base_n:
        gs.log.append(f"[YELL] reveal count modifier: {base_n} -> {n}")
    if n <= 0:
        if base_n <= 0:
            gs.log.append("[YELL] 0 (no blade on active stage members)")
        else:
            gs.log.append("[YELL] 0 (reveal count reduced to 0)")
        gs.yell_flow_completed_this_live = True
        return
    revealed = []
    for i in range(n):
        if not gs.deck:
    ...
    gs.log.append(f"[YELL-DEBUG] deck_before_draw n={len(getattr(gs, 'deck', []) or [])} top={list(getattr(gs, 'deck', []) or [])[:6]}")
    if draw_n > 0:
        got, drawn_cards = _draw_cards_for_yell(gs, draw_n, rng, reason='yell')
    else:
        got, drawn_cards = 0, []
    gs.log.append(f"[YELL-DEBUG] drawn_cards={drawn_cards} deck_after_draw n={len(getattr(gs, 'deck', []) or [])} hand_n={len(getattr(gs, 'hand', []) or [])}")
...
def _put_wait_energy_from_deck(gs: GameState, n: int, reason: str = '') -> int:
    ...
    return int(add)
def _put_active_energy_from_deck(gs: GameState, n: int, reason: str = '') -> int:
    n = int(n or 0)
    if n <= 0:
        return 0
    rem = int(_energy_remaining_in_deck(gs))
    if rem <= 0:
        ...
        return 0
    add = min(rem, n)
    gs.energy_active += add
    _clamp_energy_zone(gs)
    gs.log.append(f"[AUTO] energy deck -> ACTIVE +{add}" + (f" ({reason})" if reason else ""))
    return int(add)
...
def _parse_required_heart_icon_counts_from_text(icon_blob: str) -> Dict[str, int]:
    """Parse required-heart icon text, including <任意>, into color -> count."""
    out: Dict[str, int] = {}
    try:
        for m in re.finditer(r'<(?:\(([^)]+)\)|([^<>]+))>', str(icon_blob or '')):
            ...
            if raw == '任意':
                ...
                key = _HEART_ICON_COLOR_MAP.get(raw, '')
            if key:
                out[key] = int(out.get(key, 0) or 0) + 1
    except Exception:
        return {}
    return _ordered_heart_counts(out)


def _parse_live_start_required_any_reduction_per_success_count(ci: Optional[CardInfo]) -> Optional[int]:
    """Parse LIVE-start: success-zone card count * <任意> required-heart reduction."""
    try:
        if not ci or not getattr(ci, 'abilities', None):
            return None
        for ab in _iter_triggered_abilities(ci, 'ライブ開始時'):
            ...
            for cl in clauses:
                ...
                if not eff:
                    continue
                eff_norm = _normalize_icon_token_text(eff).replace('\n', '')
                m = re.match(r'^自分の成功ライブカード置き場にあるカード1枚につき、このカードを成功させるための必要ハートは(?P<anys>(?:<\(任意\)>)+)少なくなる。$', eff_norm)
                if m:
                    return len(re.findall(r'<\(任意\)>', str(m.group('anys') or '')))
    except Exception:
        return None
    return None


def _parse_live_start_score_and_required_increase_per_success_count(ci: Optional[CardInfo]) -> Optional[Tuple[int, Dict[str, int]]]:
    """Parse LIVE-start: each success-zone card gives score +N and required hearts +icons."""
    try:
        if not ci or not getattr(ci, 'abilities', None):
            return None
        for ab in _iter_triggered_abilities(ci, 'ライブ開始時'):
            ...
            for cl in clauses:
                ...
                if not eff:
                    continue
                eff_norm = _normalize_icon_token_text(eff).replace('\n', '')
                m = re.match(r'^自分の成功ライブカード置き場にあるカード1枚につき、このカードのスコアを\+(?P<score>\d+)し、必要ハートを(?P<icons>(?:<\([^)]+\)>)+)増やす。$', eff_norm)
                if not m:
                    continue
                per_score = int(m.group('score') or 0)
                icons = _parse_required_heart_icon_counts_from_text(str(m.group('icons') or ''))
                if per_score > 0 or icons:
                    return (per_score, icons)
    except Exception:
        return None
    return None


def _live_start_success_count_required_any_reduction(cn_live, gs: GameState, cards_db: Dict[str, CardInfo], set_idx: Optional[int] = None) -> int:
    if not _live_start_set_idx_resolved(gs, set_idx):
        return 0
    ci = _get_card(cards_db, cn_live)
    per = _parse_live_start_required_any_reduction_per_success_count(ci)
    if per is None:
        return 0
    return int(per) * len(list(getattr(gs, 'success_zone', []) or []))


def _live_start_success_count_score_and_required_increase(cn_live, gs: GameState, cards_db: Dict[str, CardInfo], set_idx: Optional[int] = None) -> Tuple[int, Dict[str, int]]:
    if not _live_start_set_idx_resolved(gs, set_idx):
        return 0, {}
    ci = _get_card(cards_db, cn_live)
    parsed = _parse_live_start_score_and_required_increase_per_success_count(ci)
    if not parsed:
        return 0, {}
    per_score, per_icons = parsed
    cnt = len(list(getattr(gs, 'success_zone', []) or []))
    if cnt <= 0:
        return 0, {}
    return int(per_score or 0) * cnt, _ordered_heart_counts({str(k): int(v or 0) * cnt for k, v in dict(per_icons or {}).items()})
...
def _build_live_start_trigger_from_effect(gs: GameState, cards_db: Dict[str, CardInfo], eff: str, source_cn: str, label: str, ctx: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Generalized from VIVID WORLD / Dazzling Game.
    m = re.match(r'^ライブ終了時まで、エールによって公開される自分のカードが持つ(?P<sources>(?:<\([^)]+\)>、?)+)<\(ALL\)>は、すべて<\((?P<target>[^)]+)\)>になる。$', eff_norm)
    if m:
    ...
    # Generalized from PL!SP-bp2-010 Wien Margarete.
    m = re.match(r'^自分のステージにこのメンバー以外のメンバーが(?P<count>\d+)人以上いる場合、ライブ終了時まで、エールによって公開される自分のカードの枚数が(?P<minus>\d+)枚減る。$', eff_norm)
    if m:
        return {
            'kind': 'live_start_yell_reveal_count_delta_if_other_stage_members_at_least',
            'source_cn': str(source_cn or ''),
            'set_idx': ctx.get('set_idx', None),
            'pos': str((ctx or {}).get('pos', '') or ''),
            'label': str(label or ''),
            'condition_other_member_count': int(m.group('count') or 0),
            'yell_reveal_count_delta': -int(m.group('minus') or 0),
        }
    ...
    m = re.match(r'^自分の成功ライブカード置き場にあるカードのスコアの合計が(?P<reduce_th>\d+)以上の場合、このライブを成功させるための必要ハートを<\(任意\)>減らす。スコアの合計が(?P<score_th>\d+)以上の場合、さらにこのカードのスコアを\+(?P<delta>\d+)する。$', eff_norm)
    if m:
        reduce_th = int(m.group('reduce_th') or 0)
        score_th = int(m.group('score_th') or 0)
        try:
            # Use the same effective score helper as the actual resolver.  This
            # includes success-zone BODY score modifiers such as Angelic Angel,
            # so the auto-order choice popup and the post-resolution popup agree.
            total_now = int(_own_success_zone_score_sum(gs, cards_db) or 0)
        except Exception:
            total_now = 0
        if score_th and total_now >= score_th:
    ...
    if m:
        return {
            ...
            'set_idx': ctx.get('set_idx', None),
            'label': str(label or ''),
            'required_any_reduction_per': len(re.findall(r'<\(任意\)>', str(m.group('anys') or ''))),
        }
    # Generalized from A song for You! You? You!!.
    m = re.match(r'^自分の成功ライブカード置き場にあるカード1枚につき、このカードのスコアを\+(?P<score>\d+)し、必要ハートを(?P<icons>(?:<\([^)]+\)>)+)増やす。$', eff_norm)
    if m:
        return {
            'kind': 'live_start_score_and_required_increase_per_success_count',
            ...
            'label': str(label or ''),
            'score_delta_per': int(m.group('score') or 0),
            'required_increase_per': _ordered_heart_counts(_parse_required_heart_icon_counts_from_text(str(m.group('icons') or ''))),
        }
...
def _build_live_success_trigger_from_effect(gs: GameState, cards_db: Dict[str, CardInfo], eff: str, source_cn: str, label: str, ctx: Dict[str, Any], pos: str = '') -> Optional[Dict[str, Any]]:
    ...
    # Waiting-room card whose group name differs from every stage member's group.
    # Example: LL-bp5-002 Bring the LOVE!
    m = re.match(r'^自分の控え室にある、自分のステージにいるすべてのメンバーと異なるグループ名を持つカード(?P<count>\d+)枚を手札に加える。?$', eff_norm)
    if m:
        return {
            'kind': 'choose_green_card_with_group_different_from_stage_members_on_live_success',
...
def _ci_matches_group_or_unit(ci: Optional[CardInfo], label: str) -> bool:
    ...
    if (g == lab) or (u == lab) or (lab in g if g else False) or (lab in u if u else False):
        return True
    cn0 = str(getattr(ci, 'cardnumber', '') or '').strip()
    if lab in _card_group_names(ci):
        return True
    if lab == "μ's" and cn0.startswith('PL!-'):
        return True
    if lab == 'Aqours' and cn0.startswith('PL!S-'):
        return True
    if lab == '虹ヶ咲' and cn0.startswith('PL!N-'):
        return True
    if lab == 'Liella!' and cn0.startswith('PL!SP-'):
        return True
    if lab == '蓮ノ空' and cn0.startswith('PL!HS-'):
        return True
...
def _live_start_required_any_reduction_if_success_score(cn_live, gs: GameState, cards_db: Dict[str, CardInfo], set_idx: Optional[int] = None) -> int:
    ...
    try:
        red_count = int(_live_start_success_count_required_any_reduction(cn_live, gs, cards_db, set_idx=set_idx) or 0)
        if red_count > 0:
            return red_count
    except Exception:
        pass
...
def _effective_live_required_hearts(cn_live, ci, gs: GameState, cards_db: Optional[Dict[str, CardInfo]] = None, set_idx: Optional[int] = None) -> Dict[str, int]:
    ...
    try:
        _sc_score, _sc_inc = _live_start_success_count_score_and_required_increase(cn_live, gs, cards_db=(cards_db or {}), set_idx=set_idx)
        for _col, _n in dict(_sc_inc or {}).items():
            if int(_n or 0) > 0:
                req[str(_col)] = int(req.get(str(_col), 0) or 0) + int(_n or 0)
    except Exception:
        pass
    try:
    ...
    return _ordered_heart_counts(req)
...
def _extra_live_score_delta_for_attempt(cn_live, gs: GameState, cards_db: Dict[str, CardInfo], set_idx: Optional[int] = None, lives_count: Optional[int] = None) -> int:
    ...
    if _parse_live_start_score_and_required_increase_per_success_count(ci_live) is not None:
        _m = int(_live_start_score_bonus_for_set_idx(gs, set_idx, source_cn=cn_live))
        if _m > 0:
            return _m
        try:
            return int(_live_start_success_count_score_and_required_increase(cn_live, gs, cards_db, set_idx=set_idx)[0])
        except Exception:
            return 0
...
def _enqueue_yell_revealed_body_auto_triggers(gs: GameState, cards_db: Dict[str, CardInfo], revealed: Optional[List[str]] = None, draw_icon_count: int = 0, drew_count: int = 0, drawn_cards: Optional[List[str]] = None) -> int:
    ...
    if cur_revealed:
        _draw_notice = _yell_draw_notice_payload(cards_db, cur_revealed, draw_icon_count=draw_icon_count, drew_count=drew_count, drawn_cards=drawn_cards)
        gs.pending.append({
            'kind': 'show_revealed_cards_ack',
            'label': 'エール公開カード確認',
            'yell_reveal_ack': True,
...
def _live_attempt_full_heart_counts(counts: Dict[str, Any], include_all: bool = True) -> Dict[str, int]:
    """Return heart counts with canonical keys and integer values for UI summaries."""
    order = ('pink', 'red', 'yellow', 'green', 'blue', 'purple', 'any', 'all') if include_all else ('pink', 'red', 'yellow', 'green', 'blue', 'purple', 'any')
    src = counts or {}
    out: Dict[str, int] = {}
    for k in order:
        try:
...
def _live_attempt_alloc_spent_counts(alloc: Dict[str, Any]) -> Dict[str, int]:
    spent: Dict[str, int] = {}
    for k, v in dict(alloc or {}).items():
        kk = str(k or '')
        if not kk.startswith('use_'):
            continue
        col = kk[len('use_'):].lower().strip()
        try:
            n = int(v or 0)
        except Exception:
            n = 0
        if col and n > 0:
            spent[col] = int(spent.get(col, 0) or 0) + n
    return _live_attempt_full_heart_counts(spent, include_all=True)


def _live_attempt_card_name(cards_db: Dict[str, CardInfo], cn: str) -> str:
    ci = _get_card(cards_db, cn)
    return str(getattr(ci, 'name', '') or getattr(ci, 'cardname', '') or cn or '')


def _live_attempt_failure_trace(lives: List[str], cards_db: Dict[str, CardInfo], owned: Dict[str, int], live_set_indices: Optional[List[int]], gs: GameState) -> Dict[int, Dict[str, Any]]:
    """Rebuild deterministic per-LIVE success/failure trace for the summary popup."""
    out: Dict[int, Dict[str, Any]] = {}
    pool_trace: Dict[str, int] = {str(k).lower(): int(v or 0) for k, v in dict(owned or {}).items()}
    pool_trace.setdefault('all', 0)
    failed = False
    idxs = list(live_set_indices or [])
    for i, cn in enumerate(list(lives or [])):
        ci = _get_card(cards_db, cn)
        set_idx = idxs[i] if i < len(idxs) else None
        req = _effective_live_required_hearts(cn, ci, gs, cards_db, set_idx=set_idx)
        if failed:
            out[i] = {'success': False, 'alloc': {}, 'reason': 'not reached'}
            continue
        ok, alloc = can_satisfy_req(req, pool_trace)
        out[i] = {'success': bool(ok), 'alloc': dict(alloc or {}), 'reason': '' if ok else str((alloc or {}).get('reason', '不足'))}
        if ok:
...
def _build_live_attempt_summary_pending(
    gs: GameState,
    cards_db: Dict[str, CardInfo],
    *,
    lives: List[str],
    live_set_indices: List[int],
    ok_all: bool,
    base_hearts: Dict[str, int],
    yell_hearts: Dict[str, int],
    owned_hearts: Dict[str, int],
    alloc_map: Dict[str, Dict[str, Any]],
    score_rows: List[Dict[str, Any]],
    card_score_total: int,
    stage_score_bonus: int,
    yell_score_icon_bonus: int,
    live_total_score: int,
) -> Dict[str, Any]:
    """Build the acknowledgement payload shown between LIVE_ATTEMPT and LIVE_SUCCESS timing."""
    lives = list(lives or [])
    live_set_indices = list(live_set_indices or [])
    score_rows = [dict(x) for x in list(score_rows or [])]
    score_by_i: Dict[int, Dict[str, Any]] = {i: score_rows[i] for i in range(min(len(score_rows), len(lives)))}
    if ok_all:
        trace: Dict[int, Dict[str, Any]] = {i: {'success': True, 'alloc': dict(alloc_map.get(cn, {}) or {}), 'reason': ''} for i, cn in enumerate(lives)}
    else:
        trace = _live_attempt_failure_trace(lives, cards_db, owned_hearts, live_set_indices, gs)
    live_cards: List[Dict[str, Any]] = []
    for i, cn in enumerate(lives):
        ci = _get_card(cards_db, cn)
        set_idx = live_set_indices[i] if i < len(live_set_indices) else None
        original_req = dict((getattr(ci, 'required_hearts', {}) if ci else {}) or {})
        effective_req = _effective_live_required_hearts(cn, ci, gs, cards_db, set_idx=set_idx)
        tr = dict(trace.get(i, {}) or {})
        sr = dict(score_by_i.get(i, {}) or {})
        base_score = int(sr.get('base', getattr(ci, 'score', 0) if ci else 0) or 0)
        score_delta = int(sr.get('delta', 0) or 0)
        score_eff = int(sr.get('score', base_score + score_delta) or 0)
        live_cards.append({
            'cardnumber': str(cn or ''),
            'cardname': _live_attempt_card_name(cards_db, cn),
            'success': bool(ok_all and tr.get('success', False)) if ok_all else bool(tr.get('success', False)),
            'required_original': _live_attempt_full_heart_counts(original_req, include_all=False),
            'required_effective': _live_attempt_full_heart_counts(effective_req, include_all=False),
            'spent_hearts': _live_attempt_alloc_spent_counts(dict(tr.get('alloc', {}) or {})),
            'failure_reason': str(tr.get('reason', '') or ''),
            'score_base': int(base_score),
    ...
    required_original_full = _live_attempt_full_heart_counts(required_original_total, include_all=False)
    required_effective_full = _live_attempt_full_heart_counts(required_effective_total, include_all=False)
    spent_full = _live_attempt_full_heart_counts(spent_total, include_all=True)
    ...
    return {
        ...
        'live_attempt_summary': {
            ...
            'required_hearts_total': {
                'original': required_original_full,
                'effective': required_effective_full,
                'spent': spent_full,
                'original_count': int(sum(int(required_original_full.get(k, 0) or 0) for k in required_keys)),
                'effective_count': int(sum(int(required_effective_full.get(k, 0) or 0) for k in required_keys)),
                'spent_count': int(sum(int(spent_full.get(k, 0) or 0) for k in ('pink', 'red', 'yellow', 'green', 'blue', 'purple', 'all'))),
            },
            ...
            'owned_hearts': {
                ...
                'yell': yell_full,
                'total': owned_full,
                'count_total': int(sum(int(owned_full.get(k, 0) or 0) for k in owned_keys)),
            },
            'score': {
                'card_total': int(card_score_total or 0),
                'stage_bonus': int(stage_score_bonus or 0),
                'yell_score_icons': int(yell_score_icon_bonus or 0),
                'live_total': int(live_total_score or 0),
                'before_live_success_effects': True,
            },
        },
    }
...
def cmd_attempt(gs: GameState, cards_db: Dict[str, CardInfo]) -> None:
    ...
    # Build score components once for both success and failure summaries.
    summary_card_score_total, summary_score_rows = _compute_attempt_score_breakdown(lives, cards_db, int(getattr(gs, 'turn', 0) or 0), gs, live_set_indices=live_idxs)
    summary_stage_score_bonus = 0
    try:
        for pos, slot in (gs.stage or {}).items():
            if not slot:
                continue
            summary_stage_score_bonus += int(_slot_always_score_bonus(gs, cards_db, pos, slot) or 0)
    except Exception:
        summary_stage_score_bonus = 0
    summary_yell_score_icon_bonus = int(_yell_revealed_score_icon_bonus(gs, cards_db) or 0)
    summary_live_total_score = int(summary_card_score_total) + int(summary_stage_score_bonus) + int(summary_yell_score_icon_bonus)
    ...
    if ok_all:
        ...
        result_txt = "FAIL"
    try:
        gs.pending.append(_build_live_attempt_summary_pending(
            gs, cards_db,
            lives=list(lives),
            live_set_indices=list(live_idxs),
            ok_all=bool(ok_all),
            base_hearts=base,
            yell_hearts=cheer,
            owned_hearts=owned,
            alloc_map=dict(alloc_map or {}),
            score_rows=list(summary_score_rows or []),
            card_score_total=int(summary_card_score_total or 0),
            stage_score_bonus=int(summary_stage_score_bonus or 0),
            yell_score_icon_bonus=int(summary_yell_score_icon_bonus or 0),
            live_total_score=int(summary_live_total_score or 0),
        ))
        gs.log.append('[PENDING] live attempt summary acknowledgement')
    except Exception as e:
        try:
            gs.log.append(f'[WARN] live attempt summary popup failed: {e}')
        except Exception:
            pass
...
def cmd_activate_to_green(gs: GameState, cards_db: Dict[str, CardInfo], pos: str, rng: Optional[random.Random] = None) -> None:
    ...
    for ab in _iter_activated_abilities(ci):
        ...
        for cl in clauses:
            ...
            if not eff:
                # effが空でもコストだけ処理してcontinue。
                # Split clauses can put costs such as self-WAIT or energy->deck
                # into a standalone clause before the actual effect clause.
                need_energy_to_deck_empty = _parse_energy_to_deck_cost(cost)
                if need_energy_to_deck_empty > 0:
                    if not pay_energy_to_deck(gs, need_energy_to_deck_empty, reason=f'activate {ci.cardnumber} empty-clause'):
                        gs.log.append(f"[ERR] activate: insufficient energy to return to energy deck x{need_energy_to_deck_empty} (active={gs.energy_active} wait={gs.energy_wait})")
                        return
                if _cost_requires_self_wait(cost) and not _cost_requires_self_to_green(cost):
            ...
            if req_discard_n > 0:
                ...
                gs.pending.append({
                    ...
                    'text': f'コストとして、手札を{req_discard_n}枚控え室に置く',
                    'options': list(gs.hand),
                    'after_effect_template': eff,
                    'after_ctx': {'pos': pos, 'source_cn': ci.cardnumber, 'effect_timing': '起動効果'},
                    'after_source_cn': ci.cardnumber,
            ...
            need_energy_to_deck = _parse_energy_to_deck_cost(cost)
            if need_energy_to_deck > 0:
                if not pay_energy_to_deck(gs, need_energy_to_deck, reason=f'activate {ci.cardnumber}'):
                    gs.log.append(f"[ERR] activate: insufficient energy to return to energy deck x{need_energy_to_deck} (active={gs.energy_active} wait={gs.energy_wait})")
                    return
            ...
            if flags.get('once_per_turn'):
                try:
                    ...
                    try:
                        ...
                        pass
            ctx = {'pos': pos, 'source_cn': ci.cardnumber, 'effect_timing': '起動効果'}
...
def cmd_resolve_pending(gs: GameState, cards_db: Dict[str, CardInfo], idx: int, choice: str, rng: Optional[random.Random] = None) -> None:
        ...
        try:
            if str(getattr(gs, 'phase', '') or '') == 'LIVE_PERF' and rng is not None and not list(getattr(gs, 'pending', []) or []):
                if not list(getattr(gs, 'set_zone', []) or []):
                    ...
                    if bool(getattr(gs, 'yell_flow_completed_this_live', False)):
    ...
    if kind == 'confirm_yell_revealed_all_to_green_then_extra_yell':
        ...
        if low in ('pay', 'yes', 'y', '1', 'true', 'apply', 'use', '使う'):
            # These reroll effects say the blade-heart gained from that YELL is lost.
            # Per 8.3.11-8.3.13, draw icons have already resolved before YELL-time
            # automatic abilities are played at check timing, so do not undo draws.
            _note_yell_bladeheart_loss_for_cards(gs, cards_db, list(cands), reason=str(p.get('source_cn','') or 'reroll-yell'))
            moved = []
            for cn0 in cands:
            ...
            if moved:
                gs.log.append(f"[AUTO] {str(p.get('source_cn','') or '?')}[エール時]: moved all revealed cards to green {moved}; lost their blade-hearts; additional yell {extra_n}")
                _append_effect_event(gs, 'reroll_yell', source_cn=str(p.get('source_cn','') or ''), detail=f'moved all revealed cards and performed additional yell {extra_n}', moved=moved, extra_count=extra_n)
    ...
    if kind == 'position_change':
        ...
        moved_cards = []
        try:
            if src_slot and getattr(src_slot, 'cardnumber', ''):
                moved_cards.append(str(getattr(src_slot, 'cardnumber', '') or ''))
            if dst_slot and getattr(dst_slot, 'cardnumber', ''):
                moved_cards.append(str(getattr(dst_slot, 'cardnumber', '') or ''))
        except Exception:
            moved_cards = []
    ...
    if kind == 'live_attempt_summary_ack':
        gs.log.append('[ACK] live_attempt_summary_ack')
        _enqueue_auto_order_from_deferred()
        return
    ...
    if kind == 'live_start_required_any_reduction_per_success_count':
        ...
        src = str(p.get('source_cn', '') or '')
        set_idx = p.get('set_idx', None)
        per = int(p.get('required_any_reduction_per', 0) or 0)
        cnt = len(list(getattr(gs, 'success_zone', []) or []))
        total_red = int(per * cnt)
        _mark_live_start_set_idx_resolved(gs, set_idx)
        gs.log.append(f"[AUTO] {src}[ライブ開始時]: success-zone cards={cnt} -> required(any) -{total_red}")
        _r = p.get('_resume') if isinstance(p, dict) else None
        if _r:
            gs.pending.append(_r)
        return
    if kind == 'live_start_score_and_required_increase_per_success_count':
        ...
        cnt = len(list(getattr(gs, 'success_zone', []) or []))
        per_score = int(p.get('score_delta_per', 0) or 0)
        per_icons = _ordered_heart_counts(dict(p.get('required_increase_per', {}) or {}))
        total_score = int(per_score * cnt)
        icon_totals = _ordered_heart_counts({str(k): int(v or 0) * cnt for k, v in per_icons.items()})
        _mark_live_start_set_idx_resolved(gs, set_idx)
        gs.log.append(f"[AUTO] {src}[ライブ開始時]: success-zone cards={cnt} -> score +{total_score}, required +{icon_totals}")
        _r = p.get('_resume') if isinstance(p, dict) else None
        if _r:
            gs.pending.append(_r)
        return
    ...
    if kind == 'pay_or_skip':
        ...
        if cost_kind == 'self_wait_and_discard_from_hand':
            ...
            _enqueue_discard_from_hand(
                gs,
                cost_n,
                label=f'コストとして、手札を{cost_n}枚控え室に置く',
                ctx=ctx0,
        ...
        if cost_kind == 'self_wait':
            ...
            if _r:
                gs.pending.append(_r)
            return
        if cost_kind == 'energy_under_self':
            pos = str(ctx0.get('pos', '') or '').upper()
            if cost_n <= 0:
                gs.log.append("[ERR] pay_or_skip: invalid under-energy cost_n")
                return
            if int(getattr(gs, 'energy_active', 0) or 0) + int(getattr(gs, 'energy_wait', 0) or 0) < cost_n:
                gs.log.append(f"[ERR] pay_or_skip: not enough energy to place under (need {cost_n})")
                return
            moved_under = _move_energy_zone_to_under_self(gs, pos, cost_n, reason=f'optional effect {src or ""}'.strip())
            if moved_under < cost_n:
                gs.log.append(f"[ERR] pay_or_skip: under-energy moved only {moved_under}/{cost_n}")
                return
            applied = False
            if after_eff:
                try:
                    ...
                    rng = random.Random()
                applied = bool(try_apply_effect_template(gs, rng, cards_db, after_eff, ctx0))
            blades = str(p.get('stage_blades_after_under', '') or '')
            if blades:
                blade_n = int(_count_blade_icons_from_tagblob(blades) or 0)
                if blade_n > 0:
                    for _pos, _slot in (getattr(gs, 'stage', {}) or {}).items():
                        if not _slot or not getattr(_slot, 'cardnumber', ''):
                            continue
                        try:
                            _slot.temp_blade = int(getattr(_slot, 'temp_blade', 0) or 0) + blade_n
                            _slot.temp_until = 'end_of_live'
                        except Exception:
                            pass
                    gs.log.append(f"[AUTO] {src}: stage members gain blade +{blade_n} until end_of_live")
            gs.log.append(f"[AUTO] {src}: energy_under_self -> {'applied' if applied else 'done'} {after_eff}")
            _r = p.get('_resume') if isinstance(p, dict) else None
            if _r:
                gs.pending.append(_r)
            return
    ...
    if kind == 'choose_effects':
        ...
        if choice0 not in remaining:
            ...
            gs.pending.append(p)
            return
        if not bool(p.get('cost_paid', False)):
            cost_kind0 = str(p.get('cost_kind', '') or '').strip()
            cost_n0 = int(p.get('cost_n', 0) or 0)
            if cost_kind0 == 'energy' and cost_n0 > 0:
                if not pay_energy(gs, cost_n0):
                    gs.log.append(f"[ERR] choose_effects: not enough active energy (need {cost_n0}, have {getattr(gs, 'energy_active', 0)})")
                    gs.pending.append(p)
                    return
                p['cost_paid'] = True
    ...
    if kind == 'choose_member_from_green_multi_up_to':
        ...
        return
    if kind == 'choose_live_from_green_to_deck_nth':
        low_choice = str(choice_str or '').strip().lower()
        src = str(p.get('source_cn', '') or '')
        deck_n = int(p.get('deck_n', 0) or 0)
        if low_choice in ('skip', '__skip__', 'none', 'no', 'n', '0', 'false', 'スキップ'):
            gs.log.append(f"[SKIP] {src}: waiting LIVE to deck {deck_n}th skipped")
            return
        cn = _canon_cardno(choice_str)
        pick_cn = None
        if cn in gs.green_room:
            ...
            for x in _cardno_variants(cn):
                if x in gs.green_room:
                    ...
                    break
        if not pick_cn:
            gs.log.append(f"[ERR] waiting LIVE to deck {deck_n}th: not in waiting room {cn}")
            return
        ci2 = _get_card(cards_db, pick_cn)
        if not _is_live_ci(ci2):
            gs.log.append(f"[ERR] waiting LIVE to deck {deck_n}th: not LIVE {pick_cn}")
            return
        gs.green_room.remove(pick_cn)
        insert_idx = max(0, min(len(gs.deck), deck_n - 1))
        gs.deck.insert(insert_idx, pick_cn)
        gs.log.append(f"[ACT] {src}: waiting LIVE {pick_cn} -> deck {deck_n}th (index={insert_idx})")
        return
    if kind == 'choose_opponent_stage_count_for_reveal_live_score':
        try:
            opp_n = max(0, min(3, int(str(choice_str or '0').strip() or '0')))
        except Exception:
            gs.log.append(f"[ERR] choose_opponent_stage_count_for_reveal_live_score: invalid choice {choice_str}")
            return
        own_n = int(p.get('own_stage_count', 0) or 0)
        total_n = max(0, own_n + opp_n)
        score_n = int(p.get('score_n', 1) or 1)
        src = str(p.get('source_cn', '') or '')
        rng2 = rng or random.Random(int(getattr(gs, 'seed', 0) or 0) + int(getattr(gs, 'turn', 0) or 0) + 71)
        _rule_refresh_for_top_access(gs, rng2, total_n, reason='reveal_by_all_stage_members_live_score')
        revealed = []
        for _ in range(min(total_n, len(gs.deck))):
            revealed.append(gs.deck.pop(0))
        gs.green_room.extend(revealed)
        live_n = 0
        for cn0 in revealed:
            ...
            if ci0 and _is_live_ci(ci0):
                live_n += 1
        if live_n > 0:
            _add_live_start_score_bonus(gs, live_n * score_n, set_idx=p.get('set_idx', None), source_cn=_source_cn_or_default(src, 'この能力'))
        gs.log.append(f'[AUTO] {src}: revealed {len(revealed)} by stage members own={own_n} opponent={opp_n}; LIVE={live_n}; score +{live_n * score_n}')
        gs.pending.append({
            'kind': 'show_revealed_cards_ack',
            'label': '公開カード確認',
            'text': f'{src or "この能力"}：自分{own_n}人＋相手{opp_n}人ぶん、デッキ上{len(revealed)}枚を公開して控え室に置きました。ライブカード{live_n}枚のためスコア+{live_n * score_n}です。',
            'display_cards': list(revealed),
            'options': ['ok'],
        })
        return
    if kind == 'choose_reveal_until_kind':
        raw = str(choice_str or '').strip()
        cost_n = int(p.get('cost_n', 0) or 0)
        want_live = raw.upper() == 'LIVE'
        want_member_cost = raw.upper().startswith('MEMBER_COST_GE')
        if not (want_live or want_member_cost):
            gs.log.append(f"[ERR] choose_reveal_until_kind: invalid choice {choice_str}")
            return
        src = str(p.get('source_cn', '') or '')
        rng2 = rng or random.Random(int(getattr(gs, 'seed', 0) or 0) + int(getattr(gs, 'turn', 0) or 0) + 73)
        revealed = []
        picked = ''
        while True:
            if not gs.deck:
                _rule_refresh_main_deck(gs, rng2, reason='reveal_until_choice')
            if not gs.deck:
                break
            cn0 = gs.deck.pop(0)
            revealed.append(cn0)
            ...
            elif want_member_cost:
                try:
                    cst = int(getattr(ci0, 'cost', 0) or 0) if ci0 else 0
                except Exception:
                    cst = 0
                ok = bool(ci0 and _is_member_ci(ci0) and cst >= cost_n)
            if ok:
                picked = cn0
                break
        if picked:
            rest = [x for x in revealed if x != picked]
            gs.hand.append(picked)
            gs.green_room.extend(rest)
            gs.log.append(f'[AUTO] {src}: reveal until target -> hand {picked}; rest {len(rest)} waiting')
        else:
            gs.green_room.extend(revealed)
            gs.log.append(f'[AUTO] {src}: reveal until target found none; revealed {len(revealed)} waiting')
        target_label = 'ライブカード' if want_live else f'コスト{cost_n}以上のメンバーカード'
        gs.pending.append({
            'kind': 'show_revealed_cards_ack',
            'label': '公開カード確認',
            'text': f'{src or "この能力"}：{target_label}が公開されるまで公開しました。' + (f'\n{picked}を手札に加え、ほか{max(0, len(revealed)-1)}枚を控え室に置きました。' if picked else f'\n対象は公開されず、公開した{len(revealed)}枚を控え室に置きました。'),
            'display_cards': list(revealed),
            'options': ['ok'],
        })
        return
    if kind == 'choose_number_reveal_top_member_cost_compare':
        try:
            chosen_n = int(str(choice_str or '0').strip() or '0')
        except Exception:
            gs.log.append(f"[ERR] choose_number_reveal_top_member_cost_compare: invalid choice {choice_str}")
            return
        src = str(p.get('source_cn', '') or '')
        pos = str(p.get('pos', '') or '').upper()
        blade_n = int(p.get('blade_n', 0) or 0)
        rng2 = rng or random.Random(int(getattr(gs, 'seed', 0) or 0) + int(getattr(gs, 'turn', 0) or 0) + 79)
        _rule_refresh_for_top_access(gs, rng2, 1, reason='number_reveal_top_cost_compare')
        top_cn = gs.deck.pop(0) if gs.deck else ''
        ci_top = _get_card(cards_db, top_cn) if top_cn else None
        try:
            actual_cost = int(getattr(ci_top, 'cost', 0) or 0) if ci_top else 0
        except Exception:
            actual_cost = 0
        is_member = bool(ci_top and _is_member_ci(ci_top))
        moved_to_hand = False
        gained_blade = False
        if top_cn and is_member and actual_cost >= chosen_n:
            gs.hand.append(top_cn)
            moved_to_hand = True
        elif top_cn:
            gs.green_room.append(top_cn)
        if top_cn and actual_cost <= chosen_n and blade_n > 0:
            slot = gs.stage.get(pos) if pos in ('L', 'C', 'R') else None
            if slot:
                slot.temp_blade += blade_n
                slot.temp_until = 'end_of_live'
                gained_blade = True
        gs.log.append(f'[AUTO] {src}: choose number={chosen_n}; revealed {top_cn}; cost={actual_cost}; hand={moved_to_hand}; blade={gained_blade}')
        gs.pending.append({
            'kind': 'show_revealed_cards_ack',
            'label': 'デッキトップ公開',
            'text': f'{src or "この能力"}：数{chosen_n}を選び、{top_cn or "なし"}を公開しました。' + (f'\nコスト{actual_cost}のメンバーなので手札に加えました。' if moved_to_hand else f'\n手札には加えず、{top_cn or "カードなし"}を控え室に置きました。') + (f'\nコストが選んだ数以下のためブレード+{blade_n}しました。' if gained_blade else ''),
            'display_cards': [top_cn] if top_cn else [],
            'options': ['ok'],
        })
        return
    if kind in ('choose_live_from_green','choose_member_from_green'):
        ...
        if remaining_picks > 0:
            _enqueue_choose_from_green(gs, cards_db, kind=want_kind, n=remaining_picks,
                                       ...
                                       score_lim=p.get('score_lim', None))
            return
        after_eff = str(p.get('after_effect_template', '') or '').strip()
        if after_eff:
    ...
    if kind == 'choose_card_from_green':
        ...
        if _r:
            gs.pending.append(_r)
        return
    if kind == 'live_storage_live_to_deck_top_gain_icons':
        low = str(choice_str or '').strip().lower()
        src = str(p.get('source_cn', '') or '')
        if low in ('skip', '__skip__', 'no', 'n', '0', 'false', 'スキップ'):
            gs.log.append(f'[SKIP] {src}: live storage topdeck skipped')
            return
        cn = _canon_cardno(choice_str)
        pick = None
        for x in list(getattr(gs, 'set_zone', []) or []):
            if _canon_cardno(x) == cn:
                ...
                break
        if not pick:
            gs.log.append(f'[ERR] live_storage_live_to_deck_top_gain_icons: not in live storage {choice_str}')
            return
        ci = _get_card(cards_db, pick)
        group = str(p.get('group', '') or '')
        if not (ci and _is_live_ci(ci) and (not group or _ci_matches_group_or_unit(ci, group)) and not _ci_has_live_start_ability(ci)):
            gs.log.append(f'[ERR] live_storage_live_to_deck_top_gain_icons: invalid target {pick}')
            return
        try:
            gs.set_zone.remove(pick)
        except Exception:
            pass
        gs.deck.insert(0, pick)
        pos = str(p.get('pos', '') or '').upper()
        slot = gs.stage.get(pos) if pos in ('L', 'C', 'R') else None
        ...
        if slot:
            ...
            if b or hearts:
                slot.temp_until = 'end_of_live'
            gs.log.append(f'[ACT] {src}: live storage {pick} -> deck top; {pos} gains icons {icons_blob}')
        else:
            gs.log.append(f'[ACT] {src}: live storage {pick} -> deck top; icon gain skipped (source pos missing)')
        return
    ...
    if kind == 'hand_to_deck_top':
        ...
        for i, hcn in enumerate(list(gs.hand)):
            if _canon_cardno(hcn) == cn:
                ...
                break
        if pick_i is None:
            gs.log.append(f'[ERR] hand_to_deck_top: chosen not in hand {cn}')
            return
        moved = gs.hand.pop(pick_i)
        gs.deck.insert(0, moved)
        picked = list(p.get('picked', []) or []) + [moved]
        rem -= 1
        gs.log.append(f'[ACT] hand_to_deck_top: {moved} -> deck top (remaining={rem})')
        if rem > 0:
            kind_need = str(p.get('want_kind', 'ANY') or 'ANY')
            group_need = str(p.get('want_group', '') or '')
            cands = _hand_candidates_by_kind(gs, cards_db, kind=kind_need, group=group_need)
            gs.pending.append({
                'kind': 'hand_to_deck_top',
                'text': str(p.get('text', '') or f'手札からデッキの一番上に置くカードを選ぶ（残り{rem}）'),
                'options': list(cands),
                'remaining': rem,
                'picked': picked,
                'want_kind': kind_need,
    ...
    if kind == 'choose_player_for_deck_top_action':
        ...
        if choice_low in ('self', 'me', 'own', 'you', '自分'):
            ...
            if action == 'topk_reorder_keep_any':
                ...
                _enqueue_reorder_from_topk_keep_any(gs, k, rng, ctx={'source_cn': src_cn})
                return
    ...
    if kind == 'choose_from_topk':
        ...
        pick_count = int(p.get('pick_count', 1) or 1)
        min_pick_count = int(p.get('min_pick_count', pick_count) or 0)
        max_pick_count = int(p.get('max_pick_count', pick_count) or pick_count)
        if min_pick_count < 0:
            min_pick_count = 0
        if max_pick_count < min_pick_count:
            max_pick_count = min_pick_count
        picked = list(p.get('picked', []) or [])
        raw_multi = [s.strip() for s in str(choice_str or '').split(',') if s.strip()]
        ...
        if choice_str.strip().lower() in ('skip', 'スキップ', '__skip__'):
            if optional:
                ...
                gs.deck = picked + pool + gs.deck
            return
        if pick_count > 1 and optional and len(raw_multi) == 1 and min_pick_count <= 0:
            cn_one = _canon_cardno(raw_multi[0])
            if cn_one in ('DONE', 'done', 'CONFIRM', 'confirm', '確定', '__DONE__', '__done__'):
                if picked:
                    gs.hand.extend(picked)
                    gs.green_room.extend(pool)
                    gs.log.append(f"[ACT] topk chose {len(picked)} optional cards -> hand; rest {len(pool)} -> waiting room")
                else:
                    gs.green_room.extend(pool)
                    gs.log.append(f"[ACT] topk chose 0 optional cards; all {len(pool)} -> waiting room")
                return
        if pick_count > 1 and raw_multi and (',' in str(choice_str or '') or not optional or min_pick_count > 0):
            pool_copy = list(pool)
            picked_multi = []
            candidates = list(p.get('candidates', pool) or pool)
            candidate_canon = [_canon_cardno(c) for c in candidates]
            unique_by_group = bool(p.get('unique_by_group', False))
            used_group_keys = set()
            max_allowed = min(max_pick_count, len(candidates), len(pool_copy))
            if len(raw_multi) < min_pick_count or len(raw_multi) > max_allowed:
                gs.log.append(f"[ERR] topk choose_n: invalid pick count {len(raw_multi)} (need {min_pick_count}..{max_allowed})")
                gs.pending.append(p)
                return
            for raw_cn in raw_multi:
                cn = _canon_cardno(raw_cn)
                if candidates and cn not in candidate_canon:
                    gs.log.append(f'[ERR] topk choose_n: {cn} not in filtered candidates')
                    gs.pending.append(p)
                    return
                pick_idx = None
                for i, x in enumerate(pool_copy):
                    if _canon_cardno(x) == cn:
                        ...
                        break
                if pick_idx is None:
                    gs.log.append(f"[ERR] topk choose_n: invalid choice {cn}")
                    gs.pending.append(p)
                    return
                if unique_by_group:
                    ci_pick = _get_card(cards_db, cn)
                    raw_group = str(getattr(ci_pick, 'group', '') or '').replace('／', '/')
                    group_keys = [s.strip() for s in raw_group.split('/') if s.strip() and s.strip() != '-']
                    if not group_keys:
                        gs.log.append(f"[ERR] topk choose_n: {cn} has no group name")
                        gs.pending.append(p)
                        return
                    dup_keys = [g for g in group_keys if g in used_group_keys]
                    if dup_keys:
                        gs.log.append(f"[ERR] topk choose_n: duplicate group name {dup_keys[0]} for {cn}")
                        gs.pending.append(p)
                        return
                    used_group_keys.update(group_keys)
                picked_multi.append(pool_copy.pop(pick_idx))
            gs.hand.extend(picked_multi)
            gs.green_room.extend(pool_copy)
            gs.log.append(f"[ACT] topk chose {len(picked_multi)} cards -> hand; rest {len(pool_copy)} -> waiting room")
            return
        cn = _canon_cardno(choice_str)
        pick_idx = None
        candidates = list(p.get('candidates', pool) or pool)
        ...
        if pick_count <= 1:
            gs.green_room.extend(pool)
            if str(p.get('after_pick', '') or '') == 'stage_or_hand_empty_area':
                empty_positions = [pp for pp in ('L', 'C', 'R') if not (getattr(gs, 'stage', {}) or {}).get(pp)]
                opts = ['hand'] + empty_positions
                gs.pending.append({
                    'kind': 'topk_stage_or_hand',
                    'text': f'{pick_cn} を手札に加えるか、メンバーのいないエリアに登場させます。',
                    'options': opts,
                    'card': pick_cn,
                    'source_kind': str(p.get('kind', '') or ''),
                    'empty_positions': empty_positions,
                })
                gs.log.append(f"[PENDING] topk stage_or_hand: picked={pick_cn}; rest {len(pool)} -> waiting room")
                return
            if str(p.get('after_pick', '') or '') == 'gain_icons_if_picked_group':
                gs.hand.append(pick_cn)
                group = str(p.get('after_group', '') or '')
                icons = str(p.get('after_icons', '') or '')
                pos = str(p.get('pos', '') or '').upper()
                src_cn = str(p.get('source_cn', '') or '')
                ci_pick = _get_card(cards_db, pick_cn)
                gained = []
                if ci_pick and _ci_matches_group_or_unit(ci_pick, group):
                    slot = gs.stage.get(pos) if pos in ('L', 'C', 'R') else None
                    if slot:
                        blade_n = int(_count_blade_icons_from_tagblob(icons) or 0)
                        if blade_n:
                            slot.temp_blade = int(getattr(slot, 'temp_blade', 0) or 0) + blade_n
                            slot.temp_until = 'end_of_live'
                            gained.append(f'ブレードx{blade_n}')
                        for col, cnt in _parse_heart_icons(icons).items():
                            _grant_temp_heart(slot, col, cnt)
                            gained.append(f'{col}ハートx{cnt}')
                    else:
                        gs.log.append(f'[WARN] {src_cn}: cannot grant icons; source slot {pos or "-"} not found')
                gain_label = ', '.join(gained) if gained else 'なし'
                gs.log.append(f"[ACT] topk chose {pick_cn} -> hand; rest {len(pool)} -> waiting room; icon gain={gain_label}")
                return
            if str(p.get('after_pick', '') or '') == 'opponent_wait_cost_blade_by_picked':
                gs.hand.append(pick_cn)
                ci_pick = _get_card(cards_db, pick_cn)
                cost_n = int(getattr(ci_pick, 'cost', 0) or 0) if ci_pick else 0
                blade_lim = int(p.get('blade_lim', 0) or 0)
                src_cn = str(p.get('source_cn', '') or '')
                gs.log.append(f"[ACT] topk chose {pick_cn} -> hand; rest {len(pool)} -> waiting room; opponent wait cost<={cost_n} blade<={blade_lim}")
                _enqueue_opponent_wait_notice(
                    gs,
                    {'source_cn': src_cn},
                    f'コスト{cost_n}以下で、かつ元々持つ<(ブレード)>の数が{blade_lim}つ以下のメンバーをすべてウェイトにする',
                )
                return
            if str(p.get('after_pick', '') or '') == 'gain_blades_to_source':
                gs.hand.append(pick_cn)
                pos = str(p.get('pos', '') or '').upper()
                src_cn = str(p.get('source_cn', '') or '')
                blade_n = int(p.get('blade_n', 0) or 0)
                ...
                if slot and blade_n:
                    slot.temp_blade = int(getattr(slot, 'temp_blade', 0) or 0) + blade_n
                    slot.temp_until = 'end_of_live'
                gs.log.append(f"[ACT] topk chose {pick_cn} -> hand; rest {len(pool)} -> waiting room; {src_cn or pos} gains blade x{blade_n}")
                return
            gs.hand.append(pick_cn)
            gs.log.append(f"[ACT] topk chose {pick_cn} -> hand; rest {len(pool)} -> waiting room")
            return
        picked.append(pick_cn)
        if len(picked) >= pick_count or not pool:
            gs.hand.extend(picked)
            gs.green_room.extend(pool)
            gs.log.append(f"[ACT] topk chose {len(picked)} cards -> hand; rest {len(pool)} -> waiting room")
            return
        p2 = dict(p)
        p2['pool'] = list(pool)
        p2['options'] = list(pool)
        p2['display_cards'] = list(pool)
        candidates2 = list(candidates)
        for ci, cand in enumerate(candidates2):
            if _canon_cardno(cand) == cn:
                candidates2.pop(ci)
                break
        p2['candidates'] = list(candidates2)
        p2['picked'] = list(picked)
        if optional and min_pick_count <= 0:
            p2['options'] = list(pool) + ['done']
            p2['text'] = f"デッキ上からカードを選ぶ：{pick_count}枚まで手札に加え、残りを控え室に置く（{len(picked)}/{pick_count}枚選択済み。終了する場合は確定）"
        else:
            p2['text'] = f"デッキ上からカードを選ぶ：{pick_count}枚を手札に加え、残りを控え室に置く（{len(picked)}/{pick_count}枚選択済み）"
        gs.pending.append(p2)
        gs.log.append(f"[PENDING] topk choose_n: picked={len(picked)}/{pick_count} remain={len(pool)}")
        return
    if kind == 'topk_stage_or_hand':
        cn = _canon_cardno(str(p.get('card', '') or ''))
        if not cn:
            gs.log.append('[ERR] topk_stage_or_hand: card missing')
            return
        choice = str(choice_str or '').strip().upper()
        if choice in ('HAND', '手札'):
            gs.hand.append(cn)
            gs.log.append(f'[ACT] topk_stage_or_hand: {cn} -> hand')
            return
        if choice not in ('L', 'C', 'R'):
            gs.log.append(f'[ERR] topk_stage_or_hand: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        if (getattr(gs, 'stage', {}) or {}).get(choice):
            gs.log.append(f'[ERR] topk_stage_or_hand: stage {choice} is not empty')
            gs.pending.append(p)
            return
        ci_pick = _get_card(cards_db, cn)
        if not ci_pick or not _is_member_ci(ci_pick):
            gs.log.append(f'[ERR] topk_stage_or_hand: not member {cn}')
            gs.hand.append(cn)
            return
        gs.stage[choice] = StageSlot(cardnumber=(getattr(ci_pick, 'cardnumber', '') or cn), active=True)
        _record_stage_member_enter(gs, getattr(ci_pick, 'cardnumber', '') or cn, pos=choice, origin='deck')
        gs.log.append(f'[ACT] topk_stage_or_hand: {cn} -> stage {choice} (no play cost)')
        return
    if kind == 'hand_member_to_former_area_then_energy_under':
        src = str(p.get('source_cn', '') or '')
        pos = str(p.get('target_pos', '') or '').upper()
        cost_max = int(p.get('cost_max', 0) or 0)
        energy_n = int(p.get('energy_n', 1) or 1)
        exclude_name = str(p.get('exclude_name', '') or '').strip()
        cn = _canon_cardno(choice_str)
        opts = [_canon_cardno(str(x or '')) for x in list(p.get('options', []) or [])]
        if not cn or cn not in opts:
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        if pos not in ('L', 'C', 'R'):
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: invalid target area {pos}')
            return
        if (getattr(gs, 'stage', {}) or {}).get(pos):
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: stage {pos} is not empty')
            gs.pending.append(p)
            return
        hand_idx = None
        for i, hcn in enumerate(list(getattr(gs, 'hand', []) or [])):
            if _canon_cardno(hcn) == cn:
                ...
                break
        if hand_idx is None:
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: {cn} is not in hand')
            return
        ci_pick = _get_card(cards_db, cn)
        if not ci_pick or not _is_member_ci(ci_pick):
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: not member {cn}')
            return
        name_pick = str(getattr(ci_pick, 'name', '') or getattr(ci_pick, 'cardname', '') or '')
        if exclude_name and exclude_name in name_pick:
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: excluded name {name_pick}')
            gs.pending.append(p)
            return
        try:
            actual_cost = int(_card_effective_play_cost_from_hand(gs, cards_db, cn) or getattr(ci_pick, 'cost', 0) or 0)
        except Exception:
            actual_cost = int(getattr(ci_pick, 'cost', 0) or 0)
        if actual_cost > cost_max:
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: cost {actual_cost}>{cost_max} for {cn}')
            gs.pending.append(p)
            return
        if int(getattr(gs, 'energy_active', 0) or 0) + int(getattr(gs, 'energy_wait', 0) or 0) < energy_n:
            gs.log.append(f'[ERR] hand_member_to_former_area_then_energy_under: not enough energy to place under (need {energy_n})')
            gs.pending.append(p)
            return
        gs.hand.pop(hand_idx)
        placed_cn = str(getattr(ci_pick, 'cardnumber', '') or cn)
        gs.stage[pos] = StageSlot(cardnumber=placed_cn, active=True)
        _record_stage_member_enter(gs, placed_cn, pos=pos, origin='hand')
        moved = _move_energy_zone_to_under_self(gs, pos, energy_n, reason=f'effect {src or ""}'.strip())
        gs.log.append(f'[ACT] {src or "effect"}: hand {placed_cn} -> stage {pos}; energy under +{moved}')
        if moved < energy_n:
            gs.log.append(f'[WARN] hand_member_to_former_area_then_energy_under: moved only {moved}/{energy_n}')
        try:
            triggers = _collect_auto_triggers_on_member_enter(gs, cards_db, entered_pos=pos, entered_cn=placed_cn, entry_origin='hand')
        except Exception:
            triggers = []
        if len(triggers) >= 2:
            opts2 = []
    ...
    if kind == 'hand_member_to_empty_area':
        ...
        if str(choice_str or '').strip().lower() in ('skip', 'done'):
            gs.log.append(f'[ACT] {src or "effect"}: hand member entry skipped')
            return
        cn = _canon_cardno(choice_str)
        opts = [_canon_cardno(str(x or '')) for x in list(p.get('options', []) or []) if str(x or '').lower() not in ('skip', 'done')]
        if not cn or cn not in opts:
            gs.log.append(f'[ERR] hand_member_to_empty_area: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        hand_idx = None
        pick_cn = ''
        for i, hcn in enumerate(list(getattr(gs, 'hand', []) or [])):
            if _canon_cardno(hcn) == cn:
                ...
                break
        if hand_idx is None:
            gs.log.append(f'[ERR] hand_member_to_empty_area: {cn} is not in hand')
            return
        ci_pick = _get_card(cards_db, pick_cn or cn)
        if not ci_pick or not _is_member_ci(ci_pick):
            gs.log.append(f'[ERR] hand_member_to_empty_area: not member {pick_cn or cn}')
            return
        if group_name and not _ci_matches_group_or_unit(ci_pick, group_name):
            gs.log.append(f'[ERR] hand_member_to_empty_area: group mismatch {pick_cn or cn} for {group_name}')
            gs.pending.append(p)
            return
        try:
            actual_cost = int(_card_effective_play_cost_from_hand(gs, cards_db, pick_cn or cn) or getattr(ci_pick, 'cost', 0) or 0)
        except Exception:
            actual_cost = int(getattr(ci_pick, 'cost', 0) or 0)
        if actual_cost > cost_max:
            gs.log.append(f'[ERR] hand_member_to_empty_area: cost {actual_cost}>{cost_max} for {pick_cn or cn}')
            gs.pending.append(p)
            return
        empty_positions = _empty_stage_positions(gs)
        if not empty_positions:
            gs.log.append('[ERR] hand_member_to_empty_area: no empty stage area')
            return
        if len(empty_positions) == 1:
            pos = empty_positions[0]
            gs.pending.append({
                'kind': 'hand_member_to_empty_area_place',
                'text': f'{src or "この能力"}：{pick_cn or cn} を {pos} に登場させます。',
                'options': [pos],
                'source_cn': src,
                'card': pick_cn or cn,
            })
            cmd_resolve_pending(gs, cards_db, len(gs.pending) - 1, pos, rng)
            return
        gs.pending.append({
            'kind': 'hand_member_to_empty_area_place',
            'text': f'{src or "この能力"}：{pick_cn or cn} の登場先を選んでください。',
            'options': list(empty_positions),
            'source_cn': src,
            'card': pick_cn or cn,
        })
        gs.log.append(f'[PENDING] {src or "effect"}: choose empty area for hand {pick_cn or cn} ({empty_positions})')
        return
    if kind == 'hand_member_to_empty_area_place':
        src = str(p.get('source_cn', '') or '')
        cn = _canon_cardno(str(p.get('card', '') or ''))
        pos = str(choice_str or '').strip().upper()
        if pos not in ('L', 'C', 'R') or (getattr(gs, 'stage', {}) or {}).get(pos):
            gs.log.append(f'[ERR] hand_member_to_empty_area_place: invalid or occupied area {choice_str}')
            gs.pending.append(p)
            return
        hand_idx = None
        pick_cn = ''
        for i, hcn in enumerate(list(getattr(gs, 'hand', []) or [])):
            if _canon_cardno(hcn) == cn:
                ...
                break
        if hand_idx is None:
            gs.log.append(f'[ERR] hand_member_to_empty_area_place: {cn} is not in hand')
            return
        try:
            gs.hand.pop(hand_idx)
        except Exception:
            try:
                gs.hand.remove(pick_cn or cn)
            except Exception:
                pass
        ci_pick = _get_card(cards_db, pick_cn or cn)
        placed_cn = str(getattr(ci_pick, 'cardnumber', '') or pick_cn or cn)
        gs.stage[pos] = StageSlot(cardnumber=placed_cn, active=True)
        _record_stage_member_enter(gs, placed_cn, pos=pos, origin='hand')
        gs.log.append(f'[ACT] {src or "effect"}: hand {placed_cn} -> stage {pos} (effect entry; no play cost)')
        try:
            triggers = _collect_auto_triggers_on_member_enter(gs, cards_db, entered_pos=pos, entered_cn=placed_cn, entry_origin='hand')
        except Exception:
            triggers = []
        ...
        _enqueue_auto_order_from_deferred()
        return
    if kind == 'green_member_to_former_area':
        src = str(p.get('source_cn', '') or '')
        pos = str(p.get('target_pos', '') or '').upper()
        cost_max = int(p.get('cost_max', 0) or 0)
        group_name = str(p.get('group_name', '') or '').strip()
        ...
        opts = [_canon_cardno(str(x or '')) for x in list(p.get('options', []) or [])]
        if not cn or cn not in opts:
            gs.log.append(f'[ERR] green_member_to_former_area: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        if pos not in ('L', 'C', 'R'):
            gs.log.append(f'[ERR] green_member_to_former_area: invalid target area {pos}')
            return
        if (getattr(gs, 'stage', {}) or {}).get(pos):
            gs.log.append(f'[ERR] green_member_to_former_area: stage {pos} is not empty')
            gs.pending.append(p)
            return
        green_idx = None
        pick_cn = ''
        ...
        if green_idx is None:
            gs.log.append(f'[ERR] green_member_to_former_area: {cn} is not in waiting room')
            return
        ci_pick = _get_card(cards_db, pick_cn or cn)
        if not ci_pick or not _is_member_ci(ci_pick):
            gs.log.append(f'[ERR] green_member_to_former_area: not member {pick_cn or cn}')
            return
        if group_name and not _ci_matches_group_or_unit(ci_pick, group_name):
            gs.log.append(f'[ERR] green_member_to_former_area: group mismatch {pick_cn or cn} for {group_name}')
            gs.pending.append(p)
            return
        try:
            actual_cost = int(getattr(ci_pick, 'cost', 0) or 0)
            ...
            actual_cost = 0
        if actual_cost > cost_max:
            gs.log.append(f'[ERR] green_member_to_former_area: cost {actual_cost}>{cost_max} for {pick_cn or cn}')
            gs.pending.append(p)
            return
        try:
            gs.green_room.pop(green_idx)
        except Exception:
            try:
                gs.green_room.remove(pick_cn or cn)
            except Exception:
                pass
        placed_cn = str(getattr(ci_pick, 'cardnumber', '') or pick_cn or cn)
        gs.stage[pos] = StageSlot(cardnumber=placed_cn, active=True)
        _record_stage_member_enter(gs, placed_cn, pos=pos, origin='green')
        gs.log.append(f'[ACT] {src or "effect"}: waiting room {placed_cn} -> stage {pos} (no play cost)')
        try:
            triggers = _collect_auto_triggers_on_member_enter(gs, cards_db, entered_pos=pos, entered_cn=placed_cn, entry_origin='green')
        except Exception:
            triggers = []
        if len(triggers) >= 2:
            opts2 = []
        ...
        _enqueue_auto_order_from_deferred()
        return
    if kind == 'green_member_to_empty_area':
        src = str(p.get('source_cn', '') or '')
        cost_max = int(p.get('cost_max', 0) or 0)
        group_name = str(p.get('group_name', '') or '').strip()
        cn = _canon_cardno(choice_str)
        opts = [_canon_cardno(str(x or '')) for x in list(p.get('options', []) or [])]
        if not cn or cn not in opts:
            gs.log.append(f'[ERR] green_member_to_empty_area: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        pick_cn = ''
        for gcn in list(getattr(gs, 'green_room', []) or []):
            if _canon_cardno(gcn) == cn:
                pick_cn = str(gcn or '')
                break
        ci_pick = _get_card(cards_db, pick_cn or cn)
        if not ci_pick or not _is_member_ci(ci_pick):
            gs.log.append(f'[ERR] green_member_to_empty_area: not member {pick_cn or cn}')
            return
        if group_name and not _ci_matches_group_or_unit(ci_pick, group_name):
            gs.log.append(f'[ERR] green_member_to_empty_area: group mismatch {pick_cn or cn} for {group_name}')
            gs.pending.append(p)
            return
        try:
            actual_cost = int(getattr(ci_pick, 'cost', 0) or 0)
            ...
            actual_cost = 0
        if actual_cost > cost_max:
            gs.log.append(f'[ERR] green_member_to_empty_area: cost {actual_cost}>{cost_max} for {pick_cn or cn}')
            gs.pending.append(p)
            return
        empty_positions = _empty_stage_positions(gs)
        if not empty_positions:
            gs.log.append('[ERR] green_member_to_empty_area: no empty stage area')
            return
        if len(empty_positions) == 1:
            pos = empty_positions[0]
            gs.pending.append({
                'kind': 'green_member_to_empty_area_place',
                'text': f'{src or "この能力"}：{pick_cn or cn} を {pos} に登場させます。',
                'options': [pos],
                'source_cn': src,
                'card': pick_cn or cn,
        ...
        gs.log.append(f'[PENDING] {src or "effect"}: choose empty area for {pick_cn or cn} ({empty_positions})')
        return
    if kind == 'green_member_to_empty_area_place':
        src = str(p.get('source_cn', '') or '')
        cn = _canon_cardno(str(p.get('card', '') or ''))
        pos = str(choice_str or '').strip().upper()
        if pos not in ('L', 'C', 'R') or (getattr(gs, 'stage', {}) or {}).get(pos):
            gs.log.append(f'[ERR] green_member_to_empty_area_place: invalid or occupied area {choice_str}')
            gs.pending.append(p)
            return
        pick_idx = None
        pick_cn = ''
        for i, gcn in enumerate(list(getattr(gs, 'green_room', []) or [])):
            if _canon_cardno(gcn) == cn:
                ...
                break
        if pick_idx is None:
            gs.log.append(f'[ERR] green_member_to_empty_area_place: {cn} is not in waiting room')
            return
        try:
    ...
    if kind == 'look_top_3way_step':
        ...
        if step == 'hand':
            ...
            if len(pool) >= 2:
                remaining = list(pool)
                gs.pending.append({
                    **_topdeck_modal_common_from_ctx(p.get('ctx', {}) if isinstance(p, dict) else {}),
                    'source_cn': str(p.get('source_cn', '') or (p.get('ctx', {}) or {}).get('source_cn', '') or ''),
                    'effect_text': str(p.get('effect_text', '') or p.get('detail_text', '') or ''),
                    'detail_text': str(p.get('detail_text', '') or p.get('effect_text', '') or ''),
                    'trigger': str(p.get('trigger', '') or (p.get('ctx', {}) or {}).get('trigger', '') or ''),
                    'kind': 'look_top_3way_step',
    ...
    if kind == 'reorder_topk_keep_any':
        ...
        return
    if kind == 'reorder_topk_all':
        pool = list(p.get('pool', []) or [])
        kept = list(p.get('kept', []) or [])
        if not pool and not kept:
            gs.log.append('[ERR] reorder_topk_all: state missing')
            return
        cn = _canon_cardno(choice_str)
        pick_idx = None
        for i, x in enumerate(pool):
            if _canon_cardno(x) == cn:
                ...
                break
        if pick_idx is None:
            gs.log.append(f"[ERR] reorder_topk_all: invalid choice {cn}")
            gs.deck = kept + pool + gs.deck
            return
        pick_cn = pool.pop(pick_idx)
        kept.append(pick_cn)
        if not pool:
            gs.deck = kept + gs.deck
            gs.log.append(f"[ACT] reorder_topk_all: placed {len(kept)} cards on top")
            return
        gs.pending.append({
            **_topdeck_modal_common_from_ctx(p.get('ctx', {}) if isinstance(p, dict) else {}),
            'source_cn': str(p.get('source_cn', '') or (p.get('ctx', {}) or {}).get('source_cn', '') or ''),
            ...
            'trigger': str(p.get('trigger', '') or (p.get('ctx', {}) or {}).get('trigger', '') or ''),
            'kind': 'reorder_topk_all',
            'text': f'次にデッキ上に置くカードを選択（残り{len(pool)}枚）',
            'options': list(pool),
            'pool': list(pool),
            'kept': list(kept),
        })
        gs.log.append(f"[PENDING] reorder_topk_all: picked {pick_cn}; remaining {len(pool)}")
        return
    if kind == 'confirm_mill_top_to_green':
        low = str(choice_str or '').strip().lower()
        src = str(p.get('source_cn', '') or '')
        if low in ('skip', '__skip__', 'no', 'n', '0', 'false', 'cancel', '使わない', 'いいえ', 'スキップ'):
            gs.log.append(f'[SKIP] {src}: optional mill top skipped')
            return
        if low not in ('apply', 'yes', 'y', '1', 'true', 'use', 'do', 'go', 'ok', 'confirm', '使う', 'はい'):
            gs.log.append(f'[ERR] confirm_mill_top_to_green: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        ...
        try:
            ...
            rng2 = random.Random()
        _rule_refresh_for_top_access(gs, rng2, k, reason='confirm_mill_top_to_green')
        moved = []
        for _ in range(min(k, len(gs.deck))):
            moved.append(gs.deck.pop(0))
        gs.green_room.extend(moved)
        gs.log.append(f'[ACT] {src}: optional mill top {len(moved)}/{k} cards to waiting room')
        return
    if kind == 'choose_heart_color_reveal_topk_all_match_group_pick_gain_blades':
        color = str(choice_str or '').strip().lower()
        if color not in {'pink', 'red', 'yellow', 'green', 'blue', 'purple'}:
            gs.log.append(f'[ERR] choose_heart_color_reveal_topk: invalid color {choice_str}')
            gs.pending.append(p)
            return
        k = int(p.get('k', 0) or 0)
        need_n = int(p.get('need_n', 0) or 0)
        group = str(p.get('group', '') or '')
        src = str(p.get('source_cn', '') or '')
        pos = str(p.get('pos', '') or '').upper()
        try:
            ...
            rng2 = random.Random()
        _rule_refresh_for_top_access(gs, rng2, k, reason='choose_heart_color_reveal_topk')
        pool = [gs.deck.pop(0) for _ in range(min(k, len(gs.deck)))]
        matches = []
        group_candidates = []
        for cn in pool:
            ...
            if not ci:
                continue
            if (_is_member_ci(ci) and _ci_member_heart_count(ci, color) > 0) or (_is_live_ci(ci) and _ci_live_required_heart_count(ci, color) > 0):
                matches.append(cn)
            if _ci_matches_group_or_unit(ci, group):
                group_candidates.append(cn)
        if len(matches) < need_n or not group_candidates:
            gs.green_room.extend(pool)
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '公開結果確認',
                'text': f'{src or "この能力"}：指定色 {color} の条件は{len(matches)}/{need_n}枚、対象『{group}』候補は{len(group_candidates)}枚でした。公開した{len(pool)}枚を控え室に置きます。',
                'display_cards': list(pool),
                'options': ['ok'],
            })
            gs.log.append(f'[AUTO] {src}: reveal top {len(pool)} color={color} condition failed -> waiting room')
            return
        blade_n = int(_count_blade_icons_from_tagblob(str(p.get('blades', '') or '')) or 0)
        gs.pending.append({
            'kind': 'choose_from_topk',
            'text': f'{src or "この能力"}：条件達成。公開カードから『{group}』のカード1枚を手札に加えます。',
            'options': list(group_candidates),
            'pool': list(pool),
            'display_cards': list(pool),
            'display_pool_all': list(pool),
            'candidates': list(group_candidates),
            'pick_count': 1,
            'after_pick': 'gain_blades_to_source',
            'blade_n': blade_n,
            'source_cn': src,
            'pos': pos,
        })
        gs.log.append(f'[PENDING] {src}: color={color} condition met -> choose 『{group}』 card; blade x{blade_n}')
        return
    if kind == 'choose_revealed_for_heart_colors_to_stage_named':
        pool = list(p.get('pool', []) or [])
        candidates = list(p.get('candidates', []) or [])
        cn = _canon_cardno(choice_str)
        if cn not in [_canon_cardno(x) for x in candidates]:
            gs.log.append(f'[ERR] choose_revealed_for_heart_colors: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        ci_pick = _get_card(cards_db, cn)
        colors = list(_card_heart_colors_for_cost_result(ci_pick))
        for col in ('pink', 'red', 'yellow', 'green', 'blue', 'purple'):
            if _ci_live_required_heart_count(ci_pick, col) > 0 and col not in colors:
                colors.append(col)
        ...
        if target_slot:
            ...
            for pp, slot in (getattr(gs, 'stage', {}) or {}).items():
                ...
                nm = str(getattr(ci, 'name', '') or getattr(ci, 'cardname', '') or '') if ci else ''
                if target_name and target_name in nm:
                    stage_opts.append(pp)
        if not stage_opts:
            gs.green_room.extend(pool)
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '効果処理結果',
                'text': f'{str(p.get("source_cn", "") or "この能力")}：{_card_display_name_with_no(cards_db, cn)} を選びましたが、付与先の「{target_name}」がステージに見つからなかったため、公開カードをすべて控え室に置きました。',
                'display_cards': list(pool),
                'options': ['ok'],
            })
            gs.log.append(f'[AUTO] choose revealed {cn}; target {target_name} missing; all {len(pool)} -> waiting room')
            return
        gs.pending.append({
            'kind': 'choose_stage_named_for_picked_hearts',
            'text': f'{_card_display_name_with_no(cards_db, cn)} が持つ色のハートを付与するステージ上の「{target_name}」を選んでください。',
            'options': list(stage_opts),
            'pool': list(pool),
            'picked_card': cn,
            'picked_colors': list(colors),
            'target_name': target_name,
            'source_cn': str(p.get('source_cn', '') or ''),
        })
        gs.log.append(f'[PENDING] choose stage target for revealed {cn}; colors={colors}')
        return
    if kind == 'choose_stage_named_for_picked_hearts':
        pool = list(p.get('pool', []) or [])
        cn = _canon_cardno(str(p.get('picked_card', '') or ''))
        colors = list(p.get('picked_colors', []) or [])
        pos = str(choice_str or '').strip().upper()
        if pos not in ('L', 'C', 'R') or pos not in [str(x).upper() for x in list(p.get('options', []) or [])]:
            gs.log.append(f'[ERR] choose_stage_named_for_picked_hearts: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        slot = (getattr(gs, 'stage', {}) or {}).get(pos)
        if slot:
            for col in colors:
                _grant_temp_heart(slot, col, 1)
        gs.green_room.extend(pool)
        gs.log.append(f'[ACT] choose revealed {cn}; all {len(pool)} revealed -> waiting room; {pos} gains hearts={colors}')
        return
    if kind == 'confirm_repeat_mill_top1_gain_blade_wait_if_live':
        low = str(choice_str or '').strip().lower()
        src = str(p.get('source_cn', '') or '')
        if low in ('skip', '__skip__', 'no', 'n', '0', 'false', 'cancel', '使わない', 'いいえ', 'スキップ'):
            gs.log.append(f'[SKIP] {src}: repeat mill stopped')
            return
        if low not in ('apply', 'yes', 'y', '1', 'true', 'use', 'do', 'go', 'ok', 'confirm', '使う', 'はい'):
            gs.log.append(f'[ERR] confirm_repeat_mill_top1: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        ...
        blade_n = int(_count_blade_icons_from_tagblob(str(p.get('blades', '') or '')) or 0)
        try:
            rng2 = random.Random(int(getattr(gs, 'seed', 0) or 0) + int(getattr(gs, 'turn', 0) or 0) + remaining)
        except Exception:
            rng2 = random.Random()
        _rule_refresh_for_top_access(gs, rng2, 1, reason='repeat_mill_top1')
        moved = ''
        if gs.deck:
            moved = gs.deck.pop(0)
            gs.green_room.append(moved)
        ...
        if slot and blade_n:
            slot.temp_blade = int(getattr(slot, 'temp_blade', 0) or 0) + blade_n
            slot.temp_until = 'end_of_live'
        ci_moved = _get_card(cards_db, moved) if moved else None
        if slot and ci_moved and _is_live_ci(ci_moved):
            slot.active = False
        gs.log.append(f'[ACT] {src}: repeat mill moved {moved or "-"}; blade x{blade_n}; live_wait={bool(ci_moved and _is_live_ci(ci_moved))}')
        if remaining > 1:
            p2 = dict(p)
            p2['remaining'] = remaining - 1
            moved_label = _card_display_name_with_no(cards_db, moved) if moved else 'なし'
            wait_text = '公開されたカードがライブカードだったため、このメンバーをウェイトにしました。' if bool(ci_moved and _is_live_ci(ci_moved)) else '公開されたカードはライブカードではありませんでした。'
            p2['text'] = f'{src or "この能力"}：{moved_label} を控え室に置き、ブレードを{blade_n}つ得ました。{wait_text}\nさらにデッキ上1枚を控え室に置きますか？（残り{remaining - 1}回）'
            p2['display_cards'] = [moved] if moved else []
            gs.pending.append(p2)
        else:
            moved_label = _card_display_name_with_no(cards_db, moved) if moved else 'なし'
            wait_text = '公開されたカードがライブカードだったため、このメンバーをウェイトにしました。' if bool(ci_moved and _is_live_ci(ci_moved)) else '公開されたカードはライブカードではありませんでした。'
            gs.pending.append({
                'kind': 'show_revealed_cards_ack',
                'label': '効果処理結果',
                'text': f'{src or "この能力"}：{moved_label} を控え室に置き、ブレードを{blade_n}つ得ました。{wait_text}',
                'display_cards': [moved] if moved else [],
                'options': ['ok'],
            })
        return
    ...
    if kind == 'choose_top_keep_one':
        ...
        if not ok:
            return
        return
    if kind == 'choose_stage_member_under_energy_return':
        low = str(choice_str or '').strip().lower()
        src = str(p.get('source_cn', '') or '')
        if low in ('skip', '__skip__', 'no', 'n', '0', 'false', 'cancel', '使わない', 'いいえ', 'スキップ'):
            gs.log.append(f'[SKIP] {src}: under-energy return skipped')
            return
        pos = str(choice_str or '').strip().upper()[:1]
        if pos not in ('L', 'C', 'R') or not (getattr(gs, 'stage', {}) or {}).get(pos):
            gs.log.append(f'[ERR] choose_stage_member_under_energy_return: invalid choice {choice_str}')
            gs.pending.append(p)
            return
        slot = (gs.stage or {}).get(pos)
        under = int(getattr(slot, 'energy_under', 0) or 0) if slot else 0
        if under <= 0:
            gs.log.append(f'[ERR] choose_stage_member_under_energy_return: {pos} has no under-energy')
            gs.pending.append(p)
            return
        gs.pending.append({
            'kind': 'choose_under_energy_return_count',
            'text': f'{src or "この能力"}：{pos} のメンバーの下にあるエネルギーを何枚エネルギーデッキに置きますか？',
            'options': [str(i) for i in range(0, under + 1)],
            'source_cn': src,
            'target_pos': pos,
            'icons_per_energy': str(p.get('icons_per_energy', '') or ''),
            'max_count': int(under),
        })
        gs.log.append(f'[PENDING] {src}: choose under-energy return count pos={pos} max={under}')
        return
    if kind == 'choose_under_energy_return_count':
        src = str(p.get('source_cn', '') or '')
        pos = str(p.get('target_pos', '') or '').upper()
        try:
            n = int(str(choice_str or '0').strip())
            ...
            n = -1
        max_count = int(p.get('max_count', 0) or 0)
        if n < 0 or n > max_count:
            gs.log.append(f'[ERR] choose_under_energy_return_count: invalid count {choice_str}')
            gs.pending.append(p)
            return
        if n <= 0:
            gs.log.append(f'[SKIP] {src}: under-energy return count=0')
            return
        slot = (gs.stage or {}).get(pos) if pos in ('L', 'C', 'R') else None
        if not slot or int(getattr(slot, 'energy_under', 0) or 0) < n:
            gs.log.append(f'[ERR] choose_under_energy_return_count: not enough under-energy at {pos}')
            gs.pending.append(p)
            return
        slot.energy_under = int(getattr(slot, 'energy_under', 0) or 0) - n
        icons = str(p.get('icons_per_energy', '') or '')
        b = int(_count_blade_icons_from_tagblob(icons) or 0) * n
        if b:
            slot.temp_blade = int(getattr(slot, 'temp_blade', 0) or 0) + b
            slot.temp_until = 'end_of_live'
        hearts_one = _parse_heart_icons(icons)
        hearts_total = {}
        for col, cnt in hearts_one.items():
            add = int(cnt or 0) * n
            hearts_total[col] = add
            _grant_temp_heart(slot, col, add)
        gs.log.append(f'[AUTO] {src}: {pos} under-energy -> energy deck x{n}; gain per {icons} (blade+{b}, hearts={hearts_total})')
        return
        ...
        if choice_str.lower() in ("pay", "yes", "y", "1", "true"):
            ...
            if cost_kind == 'discard_live_from_hand' and cost_n > 0:
                ...
                _enqueue_discard_from_hand(
                    gs,
                    cost_n,
                    label=f'手札のライブカードを{cost_n}枚控え室に置く',
                    ctx=after_ctx,
                    after_effect_template=eff,
                    after_ctx=after_ctx,
                ...
                try:
                    gs.pending[-1]['options'] = list(live_in_hand)
                except Exception:
                    pass
                return
...
# 2) Pick 1 LIVE from green room to hand
    ...
    if kind == 'choose_heart_color_for_moved_stage_members':
        ...
        col = _HEART_JP_MAP.get(jp, '')
        if not col:
            gs.log.append(f'[ERR] choose_heart_color_for_moved_stage_members: invalid color {choice_str!r}')
            gs.pending.append(p)
            return
        targets = [str(x or '').upper() for x in list(p.get('target_positions', []) or []) if str(x or '').upper() in ('L','C','R')]
        applied = []
        for pos2 in targets:
            slot2 = gs.stage.get(pos2)
            if not slot2:
    ...
    if kind == 'choose_heart_color_for_stage_positions':
        ...
        col = _HEART_JP_MAP.get(jp, '')
        if not col:
            gs.log.append(f'[ERR] choose_heart_color_for_stage_positions: invalid color {choice_str!r}')
            gs.pending.append(p)
            return
        targets = [str(x or '').upper() for x in list(p.get('target_positions', []) or []) if str(x or '').upper() in ('L','C','R')]
        applied = []
...
def cmd_next(gs: GameState, rng: random.Random, cards_db: Dict[str, CardInfo], indices: Optional[List[int]] = None) -> None:
    ...
    if gs.phase == "LIVE_PERF":
        ...
        cmd_yell(gs, rng, cards_db)
        if not bool(getattr(gs, 'yell_flow_completed_this_live', False)):
            gs.log.append('[INFO] YELL deferred; remain in LIVE_PERF until pending prompts resolve')
            return
        gs.phase = "LIVE_ATTEMPT"
        gs.log.append(f"[PHASE] LIVE_ATTEMPT (attempt) turn={gs.turn}")
        return
