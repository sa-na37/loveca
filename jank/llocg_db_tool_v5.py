...
def split_trigger_blocks(effect_text_norm: str) -> List[Dict[str, Any]]:
    """
    Split effect_text_norm into blocks with:
      - ability_type: 自動 / 起動 / 常時 (if present; else UNKNOWN)
      - trigger: for 自動, sub-triggers like 登場時 / ライブ開始時 / ...; otherwise "BODY"
      - conditions: headers that are NOT triggers (e.g., センター/ライト/レフト, ターンn回, etc.)
      - text: block body

    Notes:
      - Wiki sometimes uses different bracket glyphs: ＜＞, 〈〉, 《》. We normalize to <>.
      - Headers inside <> that don't look like event triggers are treated as conditions, not triggers.
      - Multiple condition headers can stack and apply to the following text lines.
    """
    if not isinstance(effect_text_norm, str) or not effect_text_norm.strip():
        return []

    # normalize bracket variants to ASCII <>
    t = effect_text_norm
    t = t.replace("＜", "<").replace("＞", ">")
    t = t.replace("〈", "<").replace("〉", ">")
    t = t.replace("《", "<").replace("》", ">")
    ...
    eventish_re = re.compile(r"(開始時|終了時|した時|時$)")
...
def split_cost_effect_clauses(block_text: str) -> List[Dict[str, Any]]:
    ...
    Also trims leading punctuation on the effect side that is usually an artifact of line-breaking.
