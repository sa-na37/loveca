...
def split_cost_effect_clauses(block_text: str) -> List[Dict[str, Any]]:
    ...
    for cl in clauses:
        ...
        # If current is icon-only and previous ends with a comma-like joiner, merge into previous
        if icon_run_re.match(cur_eff) and prev_eff and prev_eff.endswith(("、", "，", ",", "に", "を", "は", "が", "の", "と", "で", "へ", "から")):
            prev["effect_text"] = prev_eff + cur_eff
            prev["raw"] = (prev.get("raw", "") + "\\n" + cl.get("raw", "")).strip()
            continue
