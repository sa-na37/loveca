...
def split_trigger_blocks(effect_text_norm: str) -> List[Dict[str, Any]]:
    ...
    Split effect_text_norm into blocks with:
      - ability_type: 自動 / 起動 / 常時 (if present; else inferred)
      - trigger: event trigger like 登場時 / ライブ開始時 / ... (may exist even when ability header is omitted)
      - conditions: non-trigger headers (センター/ライト/レフト, ターンn回, etc.)
      - text: block body

    Why "自動" may become UNKNOWN:
      Many wiki entries omit an explicit "<自動>" header and start directly with "<登場時>" etc.
      In that case, we infer ability_type="自動" when we see a trigger header.

    Notes:
      - Normalize bracket variants to "<...>".
      - Headers that don't look like event triggers are treated as conditions.
    """
