...
def split_trigger_blocks(effect_text_norm: str) -> List[Dict[str, Any]]:
    """
    Split effect_text_norm into blocks with two-level headers:
      - ability_type: 自動 / 起動 / 常時 (if present)
      - trigger: for 自動, sub-triggers like 登場(時) / ライブ開始時 / ライブ終了時 / ...; otherwise "BODY"

    Wiki pages often format like:
      <自動>
      <登場時>
      ...
      <起動>
      ...
    We interpret any "<...>" line (excluding icons like "<(桃)>") as a header token and classify it.

    Returns list of {"ability_type": str, "trigger": str, "text": str}
    """
    if not isinstance(effect_text_norm, str) or not effect_text_norm.strip():
        return []
    lines = [ln.strip() for ln in effect_text_norm.splitlines() if ln.strip()]

    ability_headers = {"自動", "起動", "常時"}
    # Known / frequent event triggers (extendable)
    known_triggers = {
        "登場", "登場時", "登場した時",
        "ライブ開始時", "ライブ中", "ライブ終了時",
        "アタック時", "アタックした時",
    ...
    def classify_header(h: str) -> Tuple[str, str]:
        ...
        if h in ability_headers:
            return ("ability", h)
        # heuristics for trigger
        if h in known_triggers:
            return ("trigger", h)
        if "開始時" in h or "終了時" in h or h.endswith("時"):
            return ("trigger", h)
        # fallback: treat as trigger (keeps information)
        return ("trigger", h)
    ...
    def flush():
        nonlocal cur_lines
        if cur_lines:
            blocks.append({"ability_type": cur_ability, "trigger": cur_trigger, "text": "\n".join(cur_lines).strip()})
        cur_lines = []
    ...
    for ln in lines:
        m = trig_re.match(ln)
        if m and not ln.startswith("<("):  # exclude icons
            kind, val = classify_header(m.group(1))
            flush()
            if kind == "ability":
                cur_ability = val
                cur_trigger = "BODY"
...
def split_cost_effect_clauses(block_text: str) -> List[Dict[str, Any]]:
    """
    Split block text into clauses. A clause may contain cost/effect split by '：' (Japanese colon) or ':'.
    Returns list of {"optional": bool, "cost_text": str, "effect_text": str, "raw": str}
    """
    ...
    for ln in lines:
        ...
        if sep:
            ...
            clauses.append({"optional": optional, "cost_text": left, "effect_text": right, "raw": ln})
        else:
            clauses.append({"optional": ("してもよい" in ln), "cost_text": "", "effect_text": ln, "raw": ln})
    return clauses
...
def cmd_mine(csv_path: Path, outdir: Path, top: int = 150) -> Dict[str, Path]:
    ...
    for _, r in df.iterrows():
        ...
        for b in blocks:
            ...
            trigger_rows.append({"ability_type": ability, "ability_type": ability,
                        "trigger": trig, "cardnumber": cardno, "cardname": name, "card_type": ctype, "source_url": url})
    ...
    if cost_rows:
        ...
        gcols = ["ability_type", "trigger", "optional", "cost_template", "filters_group", "filters_unit", "filters_card_kind", "filters_cost_le", "filters_cost_ge"]
    ...
    if eff_rows:
        ...
        gcols = ["ability_type", "trigger", "optional", "effect_template", "filters_group", "filters_unit", "filters_card_kind", "filters_cost_le", "filters_cost_ge"]
