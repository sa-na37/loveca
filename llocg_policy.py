...
def compute_stage_stats(p: PlayerState) -> Dict[str, int]:
    """Compute simplified 'stats' used for LIVE set eligibility.

    Project definition: stats = (sum of base hearts on stage members) + (sum of blades on stage members).
    We only count ACTIVE stage members.
    ...
    hearts_total = 0
    blade_total = 0
    # duck-typed: p.stage_active_members() exists in play_sim
    members = []
    try:
        members = p.stage_active_members()
    except Exception:
        # fallback to scanning p.stage
        for m in getattr(p, "stage", {}).values():
            if m is not None and getattr(m, "state", None) == "ACTIVE":
                members.append(m)

    for sm in members:
        c = getattr(sm, "card", None)
        if c is None:
            continue
        # base_hearts is dict color->int
        bh = getattr(c, "base_hearts", {}) or {}
        hearts_total += sum(int(v or 0) for v in bh.values())
        blade_total += int(getattr(c, "blade", 0) or 0)

    return {
        "hearts_total": int(hearts_total),
        "blade_total": int(blade_total),
        "stats_total": int(hearts_total + blade_total),
    }


def is_live_makeable(live_card: Card, stats: Dict[str, int]) -> bool:
    """Return True if the LIVE is eligible to be set under current stage stats.

    Current heuristic: a LIVE is 'makeable' if total required hearts (including 'any') <= stats_total.
    This matches the project's interim rule for set-phase selection (not the final success check).
    """
    req = getattr(live_card, "required_hearts", {}) or {}
    need_total = 0
    for k, v in req.items():
        try:
            need_total += int(v or 0)
        except Exception:
            pass
    return need_total <= int(stats.get("stats_total", 0) or 0)




def live_difficulty(live_card, stats=None) -> int:
    """Heuristic difficulty score for a LIVE card.

    Lower means 'easier'. We use total required hearts as the primary proxy.
    This is only used for ordering makeable candidates; the actual success
    check is performed elsewhere.
    """
    if live_card is None:
        return 10**9
    # Try multiple field names defensively.
    req = getattr(live_card, "req_hearts", None) or getattr(live_card, "req", None) or getattr(live_card, "requires", None)
    if isinstance(req, dict):
        ...
        for k, v in req.items():
            try:
                ...
                continue
        return total
    # If requirement is missing, treat as hard.
    return 10**6
...
def choose_live_set_cards(p: Player, card_db: Dict[str, Card], turn: int, log=None, card_label=None, **kwargs) -> List[str]:
    """
    ...
    # Reserve key curve members so we don't discard the only copy needed for the next turn's stage plan.
    # Heuristic (Aqours三神): T1 keep at least one 9-cost member; T2 keep at least one 11-cost; T3 keep at least one 13-cost.
    reserve_cost = {1: 9, 2: 11, 3: 13}.get(turn)
    reserve_total = 0
    if reserve_cost is not None:
        reserve_total = sum(
            1 for cn in p.hand
            if (card_db.get(cn) and card_db.get(cn).type == "MEMBER" and int(getattr(card_db.get(cn), "cost", 0) or 0) == int(reserve_cost))
        )
    ...
    def is_forbidden_set(cn: str) -> bool:
        ...
        # Never discard the only copy of the reserved curve-cost member on early turns.
        if reserve_cost is not None and c.type == "MEMBER":
            try:
                ccost = int(getattr(c, "cost", 0) or 0)
            except Exception:
                ccost = 0
            if ccost == int(reserve_cost) and reserve_total <= 1:
                return True
    ...
    hand_counts = Counter(p.hand)
    chosen: List[str] = []

    # 1) choose 0-1 best makeable LIVE (conservative to avoid starving next-turn hand)
    ...
    # 2) fill with non-live cards (priority: 2-cost (T1-2 most), then 4-cost, then others; 11-cost is near-forbidden)
    def nonlive_priority(cn: str) -> int:
        ...
        if c.type == "MEMBER":
            ...
            if cost == 20:
                return -10**9  # don't set 20 for now
            if cost == 11:
                # avoid accidentally setting the only copy before it is needed for T3 stage curve
                if turn <= 2 and hand_counts.get(cn, 0) <= 1:
                    return -10**9
                return 10  # extremely low (almost forbidden)
            if cost == 2:
                return 9000 if turn <= 2 else 7000
            if cost == 4:
                return 6000
            # other members: low-moderate
            return 1000
...
def policy_selfcheck() -> None:
    """Fast sanity checks to catch NameError/attribute drift early."""
    # Ensure key callables exist.
    for name in ("choose_live_set_cards", "play_member_best_effort", "compute_stage_stats", "is_live_makeable", "live_difficulty"):
        if name not in globals():
            raise RuntimeError(f"policy missing symbol: {name}")
